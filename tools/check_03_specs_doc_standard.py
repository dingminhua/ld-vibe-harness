...
CHINESE_HEADING_RE = re.compile(r"^[一二三四五六七八九十百千万]+[、，.．]\s*")
ROMAN_HEADING_RE = re.compile(r"^[IVXLCDM]+[、.．]\s+", re.IGNORECASE)
UNNUMBERED_ALLOWED_HEADINGS = {"章节索引"}
...
def build_parser():
    parser = argparse.ArgumentParser(description="检查 specs Markdown 文档是否符合 03-Specs 文档规范的章节编号要求。")
    parser.add_argument("paths", nargs="*", default=[str(SPECS_DIR)], help="要检查的 Markdown 文件或目录，默认检查 specs/。")
    return parser
