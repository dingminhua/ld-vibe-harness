...
用户视角表达也属于稳定交互契约。新增或修改 30/31 时，应测试 30 的 `简洁确认块`、`确认项`、`选择影响`、`异常反馈`、交还摘要、用户下一步待办、失败信息包，以及 31 的 `用户要做什么`、`正常表现`、`失败时给 AI 什么`、逐项验收推进规则、raw diagnostic 不裸露在主界面、no-Hook 分支显示为“手动可用 / 可用但不自动拦截”、支持 Hook 但未 lifecycle 验收显示为“自动接入待验收”、环境自动拦截与提交消息检查分块展示、受控正反例写明 scratch target 语义、最终交还使用本次验收通过 / 失败 / 未验证、不复用 `install_verification.py` 的进入 31 下一步提示、按钮文案收集 Human 观察而不是要求 Human 自判通过 / 失败。具体图标、展示表格、卡片模板、按钮文案和 scratch 文件名属于 `code/docs/` 或运行时输出的实践契约，不应作为 specs 正文状态闭集测试。负例应覆盖删除这些用户可理解判断标准、把 `unsupported_target_environment` 或 `environment_hook_integrated=false` 直接作为用户主选项、把两类 Hook 混成一个“Hook 已通过”、31 收尾重新使用 `--require-environment-integrated` 作为通过条件、以及重新把 no-Hook 写成降级或不安装。
