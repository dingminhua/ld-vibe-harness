...
> 相关规范：`specs-v2/00-LD-Vibe-Harness理念与纲要.md`、`specs-v2/02-术语规范.md`、`specs-v2/03.04-工作模型文档规范.md`、`specs-v2/05.01-工作字段内容格式规范.md`、`specs-v2/06-工作流程基础规范.md`、`specs-v2/07-Code实现规范.md`、`specs-v2/08-Web信息同步规范.md`、`specs-v2/09-事实源边界与承载规范.md`、`specs-v2/20-工作模型集合索引.md`
...
| 内容 | 权威位置 |
|---|---|
| Task 工作模型规范 | `specs-v2/27-Task-任务.md` |
| Task 实例 | `ldvh-base/tasks/` |
| Task 字段内容格式 | `specs-v2/05.01-工作字段内容格式规范.md` |
...
3. Task 涉及修改 specs-v2、docs 正文、ldvh-base、Rules / Instructions、Skill、Agent、Code、Web 或其他高影响事实源；
...
Human Gate 的具体环境实体由 04 系列环境适配映射和运行投影记录承接。本文只规定 Task 语境下需要确认的事实和影响范围。
...
字段内容格式按 `specs-v2/05.01-工作字段内容格式规范.md` 执行。字段缺失、类型错误、状态非法、引用不存在、关闭条件不满足或文件命名不匹配时，Code 应报告诊断，不得静默通过。
...
updated: 2026-06-09
description: |
  将旧 specs/27-Task-任务.md 迁入 specs-v2，并按 v2 工作模型文档规范校准字段、状态和 Human Gate。
source: 用户直接指示
source_intent:
parent_task:
sub_tasks: []
...
| 确定性执行要求 | Task 字段、状态、引用、文件命名、验收清单、关闭条件和文档同步检查应由 Code 校验或记录缺口 | `specs-v2/07-Code实现规范.md`、Task 校验 Code、正反样例 | 校验实现 | 字段契约、状态机、引用关系、关闭条件或 affected_docs 规则变化时 |
