...
LD Vibe Harness 的基础设施规范由 01-09 承载。本文只定义基础设施在五类构成要素中的位置，不重复定义各基础设施规范的细则。

当前基础设施规范规划如下：

1. specs 文档结构、编号、文档类型和引用纪律由 `specs/01-specs文档结构规范.md` 定义；
2. 项目文件夹规划由 `specs/02-LDVH文件夹说明.md` 定义；
3. 事实源边界与承载规则由 `specs/03-事实源边界与承载规范.md` 定义；
4. Trae Solo 环境机制由后续 04 号基础设施规范定义；
5. Harness 工具边界由后续 05 号基础设施规范定义。

发生冲突时，以对应基础设施规范的权威领域为准。

---

## 七、Human Gate 与检查要求

涉及 LD Vibe Harness 理念、价值标准、五类构成要素、基础设施规范权威领域、生产对象模型或行动模型的高影响变更，应评估 Human Gate。
...
1. specs 文档编号、文档类型、子文档、实践文档、引用纪律、写作骨架和治理检查细则（见 `specs/01-specs文档结构规范.md`）；
2. 项目文件夹规划和目录用途细则（见 `specs/02-LDVH文件夹说明.md`）；
3. 事实源边界、事实承载、结构化优先和工具输出回写细则（见 `specs/03-事实源边界与承载规范.md`）；
4. 各 Harness 生产对象的完整实践细则、对象实例和对象关系细节（见 10-39 Harness 生产对象）；
5. 行动模型要素的具体设计规则和流程细则（见 40-69 Harness 行动模型）；
6. 其他基础设施规范的具体格式契约、工具实现规则和机制适配细则（见 04-09 基础设施规范）；
7. ld-web/ 的具体实现；
8. ld-tools/ 的具体实现；
9. Harness 工具自身的产品需求和开发实践，以及管辖项目资料、报告和操作性实践（见 docs）；
10. 外部资料、第三方文档和外部案例原文（见 references）；
11. 操作性实践说明、案例和工具使用手册（见 docs）。

---

## 九、待补齐事项

1. 04-09 基础设施规范的具体文件拆分和命名待迁移时确定；
