...
**Version: v4.1.0** — 完善阶段增量发布。能力结构已建立，部分功能仍在收敛中，完善阶段将持续推进并在后续版本收敛稳定保证。

完整历史见 [CHANGELOG.md](CHANGELOG.md)，发布见 [GitHub Releases](https://github.com/dingminhua/ld-vibe-harness/releases)。

## 前言：LDVH 为什么存在

Vibe Coding 让 Human 用自然语言表达目标，由 AI 理解意图、推理方案并生成实现——门槛低、反馈快、产出多。但 AI 生成代码的本质是概率性推理而非确定性编译，天然存在上下文依赖、概率性输出和无状态等问题。

Vibe Coding 擅长单个功能和局部修复，但真实开发是从概念出发，经一个个功能、一次次修改、一步步迭代打磨出来的。缺乏对需求、状态、验证、决策、经验和上下文的稳定衔接，就会导致产出越多、负担越重，最终陷入问题回归、判断失据、质量漂移和闭环断裂。

**LD Vibe Harness 要解决的**：Human 与 AI 协同推进长期项目时，如何让需求、状态、验证、决策、经验和上下文保持稳定衔接，避免项目从"开始还不错"变成"难以继续做"；并让每次推进形成的经验、规则和能力持续沉淀复用，使项目越做越稳、越做越可控、越做越高效。

**设计哲学的前提假设：AI 执行者不可靠。** AI 的输出会出错、会漂移、会遗忘；LDVH 不把长期项目的稳定推进寄托于单次 AI 的自觉或单次对话的约束，也不接受单一视角的自我确认。

基于这一假设，LDVH 以两条基本路径推动项目前进：

- **制度化** — 以规范源、事实源、行动模板与 Git Gate 等制度，把边界、依据、流程和机械把关固化下来，使 AI 在制度约束下推进，而非依赖其自觉。
- **多角度对抗** — 不以单一视角的自我判断作为结论，让不同角色、不同视角与机械检查相互挑战、复核与纠偏，使判断在对抗中逼近可靠，而非在自证中确认。

制度与对抗都需要成本。为在可靠与效率之间取得平衡，LDVH 建议对 AI 执行者的模型资源实行分级配置：大部分普通求解工作交由普通模型完成；仅少数核心、关键或高风险的决策，才调用更高级的模型处理。

> LD Vibe Harness 是 LaoDing 基于长期 AI 协作开发经验所作的一种探索，简称为 LDVH。设计哲学的完整定义见 [specs/00-理念与构成](specs/00-理念与构成.md)。
...
`git pull` 后在任一已接入环境告诉 AI「LDVH 更新了」即可：由 AI 按环境接入规范判断变化类型、只重放受影响部署件，并经 [specs/09 §6.5](specs/09-环境接入规范.md) 重验受影响范围，不沿用旧验证结论。

完整的更新判定、`git-hooks-status` 只读复核与 `environment-sync` 受控同步规则见 [specs/09 环境接入规范](specs/09-环境接入规范.md)（§5.9 对齐检查、§11 更新授权），执行步骤见 [specs/33 环境接入行动模板](specs/33-环境接入行动模板.md)。程序不猜厂商路径、不默认 WorkBuddy；`platform`/`skill_path` 缺省或空时一律 `invalid_request`。
