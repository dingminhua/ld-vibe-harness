...
| 7 | 已完成 | 新增 `specs/10-管辖项目配置规范.md`、`specs/attachments/10.Att.01-管辖项目配置字段表.md` 和根配置 `LDVH-GOVERNED-PROJECTS.yaml`，迁入管辖项目静态解析能力 | `parse_governed_projects_config`、`validate_governed_projects_config`、`resolve_governed_subject` 与配置/target/worktree/multi-target/CLI 测试 | validator 0 diagnostics；tests/code 与 `_migration/tests` 通过 |
