...
安装向导的关键表格必须使用稳定图标帮助 Human 快速扫描。路径确认表项目列使用 `📦 LDVH 本体路径`、`🗂️ 目标工作区根目录`、`🧾 配置文件完整路径`；安装前检查项目列使用 `📦 LDVH 本体`、`🧾 工作区配置`、`⚠️ 配置层级`、`📁 事实源目录`、`🔎 配置解析`、`🔌 环境入口`、`🪝 Git Hook`；安装前检查结果列必须使用 `✅ 通过`、`⚠️ 注意`、`⚠️ 需安装`、`⚠️ 需升级`、`⛔ 阻断`、`➖ 不适用`；安装方案预览项目列使用 `🧾 工作区配置`、`🗑️ 项目内旧配置`、`📁 事实源目录`、`🔌 环境插件`、`🪝 管辖项目 Git Hook`、`🧪 验证`、`↩️ 回滚`。图标只用于提高可读性，不得替代状态文字、路径、限制、Human Gate 或验证结论。

安装前检查只能输出系统已检查事实，不得把事实伪装成 Human 选项。目录已存在、配置已存在、项目内配置不受支持、当前实现不支持用户级配置目录、旧插件和旧路径状态、外部 Hook 状态等，都应写入检查结果、需授权动作或后置项；不得继续询问 Human 是否创建已存在目录，或把项目内配置、用户级配置目录放进主选项。环境插件未安装时必须标为 `⚠️ 需安装` 并安排安装方案；环境插件已安装但指向旧路径、旧版本或 stale V2 path 时必须标为 `⚠️ 需升级` 并安排升级方案；每个已选择管辖项目都必须检查 Git `commit-msg` Hook 状态，未安装时必须标为 `⚠️ 需安装` 并安排安装方案，已安装但不是当前 V3 managed hook 或指向旧路径时必须标为 `⚠️ 需升级` 并安排升级方案；不得把这类情况表达成“不安装插件”“不处理插件”或“Git Hook 后置可不做”。
...
安装选项只包含当前可执行且需要 Human 取舍的决策。`LDVH-GOVERNED-PROJECTS.yaml` 的配置位置不是选项，只能是目标工作区根目录；项目根目录、用户级目录和 LDVH 本体目录都不是支持位置。AI 必须用表格展示 LDVH 本体路径、目标工作区根目录、配置文件完整路径、说明、限制和建议。项目内已存在 `LDVH-GOVERNED-PROJECTS.yaml` 时，只能记录为 legacy / conflict，并要求 Human 先删除、迁移或明确保留其中一个；不得把它作为主选项。

安装方案预览必须合并所有 Human Gate：写入对象、写入位置、将执行动作、不会执行的事项、后置项、验证命令、失败处理和回滚方式都必须在此步展示。若环境插件未安装，方案预览必须安排安装；若环境插件已安装但过时或指向 stale V2 path，方案预览必须安排升级；插件安装或升级的写入对象、影响范围、验证和回滚入口必须明确。完整安装方案必须同时覆盖 AI 环境 Hook 和管辖项目 Git Hook；每个已选择管辖项目都必须列出 repo 路径、`core.hooksPath` / active hook 状态、将执行的 install / upgrade / no-op 动作、验证命令和卸载 / rollback 命令。若 Git Hook 因目标不明确、非管辖、混合 target 或 Human 未确认而不能执行，安装不得声明完成，只能记录为 gated / blocking。若目标工作区根目录到目标项目路径链上已经存在项目内 `LDVH-GOVERNED-PROJECTS.yaml`，安装方案预览必须报告配置层级冲突，并要求先删除、迁移或明确保留其中一个配置文件；不得继续进入执行。最终确认前，AI 只能只读检查、展示结果和收集选择，不得写入配置、创建目录、安装插件、修改 Hook、修改用户环境或声明 integrated。

最终确认只展示两个主选项：`1 执行方案` 和 `2 不执行，停止安装`。Human 选择不执行时，AI 必须停止在方案预览或最终确认状态，不得写入；Human 要求调整方案时，AI 必须回到对应前一步，但不得把返回修改作为第三个主选项；只有 Human 选择执行后，AI 才能按方案执行已确认的仓库内动作。
...
| Gate | 写入、覆盖、删除或迁移环境入口，安装、升级、禁用或卸载 LDVH 插件 / 扩展包，安装、升级、禁用、卸载或迁移管辖项目 Git Hook，创建或修改目标工作区根目录下的 `LDVH-GOVERNED-PROJECTS.yaml`，创建、删除、迁移或重命名管辖项目 `ldvh-base/` 及事实源子目录，确认 LDVH 本体路径和目标工作区根目录，处理配置层级冲突，声明环境入口 integrated，目标环境 Hook 能力不明，多项目或混合非管辖 target，缺少用户告知清单，或缺少安装方案预览和最终确认，均必须暂停或进入 Human Gate。 |
