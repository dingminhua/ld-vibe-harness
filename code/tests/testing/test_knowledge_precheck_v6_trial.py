"""Focused tests for the v6 action-semantic trigger calibration paired trial runner.

The v6 gateway compares live F2 cards byte-exactly against the frozen baseline
card set and projects the activation card set for the calibration-enhanced arm.
The positive F2/F3 paths are driven by replaying the frozen
``source-observations`` fixtures (read-only inputs) through
:class:`ReadOnlyKnowledgeGateway`, while the runner-owned orchestration steps
(prepare / trigger / exclude / finalize / blind / score) are exercised through
the trial module directly.
"""
...
from ldvh.testing.knowledge_precheck_v6_trial import (
    TrialRunnerError,
...
ARTIFACT_ROOT = PROJECT_ROOT / "docs/metrics/knowledge-precheck-v6"
OBJECT_SET_FINGERPRINT = "43678e7d3e10caf25af7ea88e2a404b255b552f57a35c20124403953b17961af"
...
def test_v6_trial_init_creates_bundle(tmp_path: Path) -> None:
...
def test_v6_trial_bundle_protocol_valid(tmp_path: Path) -> None:
    """The initialized bundle's protocol must pass v6 validation."""
    root = _bundle(tmp_path)
...
def test_v6_trial_snapshot_dual_card_sets(tmp_path: Path) -> None:
...
def test_v6_trial_ledger_ceilings(tmp_path: Path) -> None:
    """Ledger ceilings must match the v6 constants."""
    root = _bundle(tmp_path)
    protocol = json.loads((root / "protocol.json").read_bytes())
...
def test_v6_trial_prepare_requires_fresh_member_context(tmp_path: Path) -> None:
    """prepare_attempt must reject duplicate fresh_context_id_hash."""
    root = _bundle(tmp_path)
    ctx = bytes_sha256(b"member-context")
    prepare_attempt(root=root, pair_id="A1", condition="calibration-baseline", fresh_context_id_hash=ctx)
    with pytest.raises(TrialRunnerError, match="fresh context"):
        prepare_attempt(root=root, pair_id="A1", condition="calibration-enhanced", fresh_context_id_hash=ctx)


def test_v6_trial_prepare_enforces_frozen_arm_order(tmp_path: Path) -> None:
    ...
    # Second arm of A1 must be calibration-enhanced (from the protocol order)
...
def test_v6_trial_helper_requires_pre_f2_trace(tmp_path: Path) -> None:
...
def test_v6_trial_negative_trigger_forbids_helper(tmp_path: Path) -> None:
    ...
    _write(trace_file, _trace(packet, "calibration-baseline", False))
...
def test_v6_trial_cross_family_f2_rejected(tmp_path: Path) -> None:
    ...
    attempt_id = str(packet["attempt_id"])
    trace_file = tmp_path / "trace.json"
    _write(trace_file, _trace(packet, "calibration-baseline", True))
...
def test_v6_trial_f3_before_f2_rejected(tmp_path: Path) -> None:
...
def test_v6_trial_gateway_replay_frozen_f2_baseline(tmp_path: Path) -> None:
    """The frozen adr F2 response must replay cleanly on the calibration-baseline arm."""
    root = _bundle(tmp_path)
    snapshot = json.loads((root / "source-snapshot.json").read_bytes())
    baseline = snapshot["family_cards_baseline"]
    activation = snapshot["family_cards_activation"]
...
def test_v6_trial_gateway_replay_frozen_f2_enhanced(tmp_path: Path) -> None:
    """The calibration-enhanced arm must project the activation card set at runtime."""
    root = _bundle(tmp_path)
    snapshot = json.loads((root / "source-snapshot.json").read_bytes())
    baseline = snapshot["family_cards_baseline"]
    activation = snapshot["family_cards_activation"]
    ...
    # Byte/field assertion: enhanced arm serves the projected card fields
...
def test_v6_trial_gateway_replay_frozen_f3(tmp_path: Path) -> None:
...
def test_v6_trial_gateway_enhanced_projects_from_baseline(tmp_path: Path) -> None:
    """The enhanced arm must project the frozen activation set from a live baseline response."""
    root = _bundle(tmp_path)
    snapshot = json.loads((root / "source-snapshot.json").read_bytes())
    baseline = snapshot["family_cards_baseline"]
    activation = snapshot["family_cards_activation"]
...
def test_v6_trial_technical_failure_exclude_and_rerun(tmp_path: Path) -> None:
...
def test_v6_trial_non_trigger_full_flow(tmp_path: Path) -> None:
    """A full non-trigger member run must complete without errors."""
    root = _bundle(tmp_path)
...
def test_v6_trial_scorer_context_unique(tmp_path: Path) -> None:
