...
WorkCase 主要服务 V1、V2、V4、V5、V6、V7、V8 和 V9：它承接目标、范围、成功标准、执行状态、验证证据、Human 确认、关闭判断和后续分流，让 AI 能恢复工作入口、理解当前边界、稳定推进执行、在关键 Gate 触发时等待 Human 确认、验证完成声明、按格式回写结果，并把残留风险和后续事项呈现给 Human 或分流。不对象化会增加 AI 的恢复、验证和关闭判断负担。
...
4. 当前 V3 WorkCase 顶层字段、`orchestration` 最小嵌套契约和状态条件必填的后续审计与收敛判断；该判断不得以本地 V2 成员文件作为默认来源。
...
3. Spark、ADR、Pitfall 和 Study 已进入 V3 成员规范；后续补充这些成员的字段契约、实例校验、Web/API 或行动模板能力时，必须作为 V3-owned 后续推进，不回到成员准入判断；
