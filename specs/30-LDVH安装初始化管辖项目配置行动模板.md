...
| 步骤 | 名称 | 作用 | 写入边界 |
|---|---|---|---|
| 1/5 | 流程概览 | 展示完整流程和当前入口 | 不写入 |
| 2/5 | 安装前检查 | 只读检查当前项目、配置、事实源目录、环境入口、旧插件和旧路径状态 | 不写入 |
| 3/5 | 安装选项 | 只收集真正需要 Human 决定的安装选项 | 不写入 |
...
安装前检查只能输出系统已检查事实，不得把事实伪装成 Human 选项。目录已存在、配置已存在、当前实现不支持用户级配置目录、旧插件未授权处理、外部 Hook 未授权处理等，都应写入检查结果或后置项；不得继续询问 Human 是否创建已存在目录，或把当前 Code 不支持的用户级配置目录放进主选项。

安装选项只包含当前可执行且需要 Human 取舍的决策。每个选择项必须用表格给出选项、含义、为什么选它、限制和建议。当前配置位置主选项只包含两类：工作区根目录和当前项目根目录。用户级配置目录是后置缺口，只有在 specs、Code 和 tests 均支持后才能进入主选项。
...
| 配置位置 Gate 要求 | 生成或修改管辖项目配置前必须让 Human 通过选择框 / 单选控件或等价编号二选一选择工作区根目录或当前项目根目录，且选项必须给出理由 | 本文、10、Human Gate | 配置治理 | 创建或迁移 `LDVH-GOVERNED-PROJECTS.yaml` 时 |
...
| 配置检查 | 是否按 10 解析和验证管辖项目配置，并确认配置生成位置询问使用选择框 / 单选控件或等价编号二选一，且选项给出理由、限制和建议 | 不得声明管辖项目配置已生效 |
