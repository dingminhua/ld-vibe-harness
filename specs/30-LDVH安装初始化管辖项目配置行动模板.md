...
安装方案预览必须合并所有 Human Gate：写入对象、写入位置、将执行动作、不会执行的事项、后置项、验证命令、失败处理和回滚方式都必须在此步展示。若环境插件未安装，方案预览必须安排安装；若环境插件已安装但过时或指向 stale V2 path，方案预览必须安排升级；插件安装或升级的写入对象、影响范围、验证和回滚入口必须明确。完整安装方案必须同时覆盖 AI 环境 Hook 和管辖项目 Git Hook；每个已选择管辖项目都必须列出 repo 路径、`core.hooksPath` / active hook 状态、将执行的 install / upgrade / no-op 动作、验证命令和卸载 / rollback 命令。若已选择管辖项目不是有效 Git worktree，安装方案预览必须停止为 blocking，不得继续进入最终执行，并必须说明管辖项目必须是 Git 仓库。若 Git Hook 因目标不明确、非管辖、混合 target 或 Human 未确认而不能执行，安装不得声明完成，只能记录为 gated / blocking。若目标工作区根目录到目标项目路径链上已经存在项目内 `LDVH-GOVERNED-PROJECTS.yaml`，安装方案预览必须报告配置层级冲突，并要求先删除、迁移或明确保留其中一个配置文件；不得继续进入执行。最终确认前，AI 只能只读检查、展示结果和收集选择，不得写入配置、创建目录、安装插件、修改 Hook、修改用户环境或声明 integrated。
