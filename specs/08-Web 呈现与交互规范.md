...
字段级解析结果不构成机械校验结论。Web 直读载体时，`check_status` 只使用 `readable` / `unreadable` 表示载体是否可读、可解析；它不是 Helper 精确读取的 `mechanically_valid` 等校验结论。页面需要向 Human 传达校验状态时，必须标明其来源（如 Helper 精确读取结果），不得把字段级可解析呈现为校验通过。
