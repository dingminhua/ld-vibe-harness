...
面向 Human，当 Human 提出的失败经验满足完整准入、且经判断需要跨行动保留时，Pitfall 直接承接 HV3 的入档、完整 `draft` 快照、Human 确认及 `active`、`discarded` 节点核对。Pitfall 保存的失败机制、解决、验证和规避经验只是 HV4 的积累内容输入；当前契约不记录该经验后来是否被实际使用或复用、避免了什么损耗、产生了什么可观察效用，因而 `active` 状态、对象数量或召回机会均不能证明 HV4。Pitfall 不单独承接 HV1、HV2 或完整 HV5。
...
| `object-uid` | conditional | `pitfall-fact-type::5. Pitfall 类型定义` |
