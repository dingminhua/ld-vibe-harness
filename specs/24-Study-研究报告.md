...
Study 的价值，是把已形成稳定阅读价值的调研、分析、核验或方案比较结果沉淀为 Markdown 事实对象，减少 AI 在后续讨论、决策和执行中重新整理资料、引用结论或判断来源边界的负担。

Study 主要服务 V1、V2、V3、V7 和 V9：它帮助 AI 快速定位稳定资料入口，充分理解研究结论和适用边界，正确判断来源可靠性、方案差异和待决事项，按 Markdown 事实对象格式承载来源、结论和证据，并把可吸收结论分流到 WorkCase、ADR、Pitfall、Spark、docs、specs 或其它事实源。Study 是报告承载，不是讨论入口、执行承接、长期决策或经验库。
...
Study / 研究报告承载已经形成稳定阅读价值的调研、分析、核验或方案比较结果。它解决的问题是：临时调研材料和研究过程可以继续变化，但某些报告已经成为后续讨论、决策、计划或 Spark 演变的关键依据，需要进入 Git 可追踪事实源。
...
| 检查类别 | 检查内容 | 不满足时 |
|---|---|---|
| 准入检查 | 是否说明研究问题、输入边界、关键发现和减少的 AI 负担 | 保留为临时材料、Spark 或待补齐事项 |
...
2. 将调研结果从临时材料、聊天或 WorkCase 输出提升为 Study；
