...
本文只定义 ADR 对象模型。ADR 相关 Rules、Skill、Agent、Tools 辅助和 Web 展示实践可按需由 §十二附件型实践子文档承接；落地初始化、落地审计和合规检查按 06 §4.6 执行。
...
3. ADR YAML schema 待按需创建的 `11.04-Tools.md` 或主文档 Tools 辅助章节中细化；
4. ADR 相关 Rules、Skill、Agent、Tools 和 Web 实践待对应附件型实践子文档按需创建时展开；
5. 落地初始化、落地审计和合规检查按 06 §4.6 执行。
