"""Runner-owned command surface for the L1 trigger v0.3 paired trial."""
...
def helper_call(
    *,
    ...
    exchange = gateway.call(operation, request, exchange_id=exchange_id, attempt_id=attempt_id)
...
def exclude_pair(
    *,
    ...
    trigger_states = [
        (
            (root / "member-trigger-responses" / f"{attempt_id}.json").is_file(),
            (root / "trigger-traces" / f"{attempt_id}.json").is_file(),
        )
        for attempt_id in attempt_ids
    ]
    if any(raw_exists != trace_exists for raw_exists, trace_exists in trigger_states) or len(set(trigger_states)) != 1:
        raise TrialRunnerError("whole-pair technical exclusion must bind one consistent evidence stage")
    has_trigger_evidence = trigger_states[0][0]

    snapshot = _read_object(root / "source-snapshot.json", "source snapshot")
    evidence_by_attempt: dict[str, dict[str, Any]] = {}
    invalid_structured_responses = 0
    for attempt_id in attempt_ids:
        packet = _read_object(root / "model-inputs" / f"{attempt_id}.json", "model input")
        validate_model_input_packet(packet, protocol)
        exchange_files = sorted((root / "helper-exchanges").glob(f"{attempt_id}-*.json"))
        ...
        if has_trigger_evidence:
            ...
            operations = [exchange.get("operation") for exchange in exchanges]
            if operations not in (
                [],
                ["find-fact-object-candidates"],
                ["find-fact-object-candidates", "read-fact-objects"],
            ) or bool(operations) != trace["triggered"]:
                raise TrialRunnerError("technical attempt does not preserve a bounded F2/F3 sequence")
            if decision_path.is_file() != bool(trace["triggered"]):
                raise TrialRunnerError("technical attempt post-F2 decision coverage is incomplete")
            if decision_path.is_file():
                decision = parse_raw_response_evidence(
                    _read_object(decision_path, "member F3 decision response"),
                    response_kind="member-f3-decision",
                )
                decided_refs = validate_f3_decision_response(
                    decision,
                ...
                expanded_refs = [obj["object_uid"] for exchange in exchanges for obj in exchange["f3_objects"]]
                if expanded_refs != decided_refs:
                    raise TrialRunnerError("technical attempt F3 expansion differs from the raw member decision")
        ...
        if not is_valid_structured_response(response, packet=packet, protocol=protocol):
            invalid_structured_responses += 1
        validate_technical_failure_response(
            response,
        ...
        evidence_by_attempt[attempt_id] = build_raw_response_evidence(
            attempt_id=attempt_id,
            response_kind="member-technical-failure",
            raw_response=raw_response,
        )

    if exclusion_code == "model_technical_failure" and invalid_structured_responses == 0:
        raise TrialRunnerError("model technical failure requires at least one invalid structured response")

    for attempt_id in attempt_ids:
        _write_new_json(
            root / "technical-failure-responses" / f"{attempt_id}.json",
            evidence_by_attempt[attempt_id],
        )
    finished_at = _now()
...
def record_scorer_technical_failure(
    *,
    ...
    """Retain one invalid fresh scorer response before its successful retry."""
...
def seal_bundle(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise TrialRunnerError("attempt ledger is not a complete preregistered v0.3 run")
