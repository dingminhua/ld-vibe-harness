...
| 工作流 | specs/50-79、Rules、Skill、Agent 编排 | 控制 AI 如何读取、判断、执行、停下和回写 |
...
| 50-79 | 工作流规范 | 50 为基础规范与区段索引，51-79 为具体工作流 |
| 80-99 | 保留 | 未经确认不提前占用 |
...
1. AI 需要同时记住 工作区规则、项目规则、场景规则、specs 主文档、子文档、Skill 文档和工具实现，容易遗漏真正权威位置；
...
1. 收敛工作区规则 / 项目规则 / 场景规则的入口表达，减少重复正文；
...
3. 当前 51-79 工作流 planned 清单是否仍符合 Core Loop；
