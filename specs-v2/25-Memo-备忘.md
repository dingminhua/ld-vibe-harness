...
| 内容 | 权威位置 |
|---|---|
| Memo 工作模型规范 | `specs-v2/25-Memo-备忘.md` |
| Memo 实例 | `ldvh-base/memos/` |
| Memo 字段内容格式 | `specs-v2/05.01-工作字段内容格式规范.md` |
...
| `active` → `archived` | 不再需要继续跟踪但保留历史 | 应记录归档原因 |
| `resolved` → `archived` | 分流后不再活跃 | 保留历史关系 |
...
Task 的准入、状态和字段契约由 `specs-v2/27-Task-任务.md` 定义。
...
Intent 的准入、状态和字段契约由 `specs-v2/24-Intent-意图.md` 定义。
...
Memo 的创建、状态变化、分流和归档都应留下 Change。Change 的 commit message 契约由 `specs-v2/22-Change-变更.md` 定义。
...
Human Gate 的具体环境实体由 04 系列环境适配映射和运行投影记录承接。本文只规定 Memo 语境下需要确认的事实和影响范围。
...
Web 不得在页面状态、缓存或数据库中维护独立 Memo 权威状态。受控编辑 Memo 字段时，应调用 Code 校验和受控写入链路，并遵守 Human Gate。

### 8.4 工作流程与环境适配

Memo 创建、分流和归档的具体行动流程由后续 40-59 工作流程规范承接。本文只定义 Memo 实例的事实规则和状态约束。

环境不支持完整引用校验、分流辅助或受控编辑时，应记录降级方式，例如改用人工检查、Code 校验或直接读取 Git 文件事实源；不得把未完成的环境能力表述为完整落地。
...
|---|---|---|---|---|
| 上位约束承接要求 | Memo 实例和后续工作流程应遵守本文定义的准入、状态机、字段契约、分流规则和事实源边界 | 05、03.04、本文、20 集合索引、21 ADR、24 Intent、27 Task、Human Gate | 工作模型治理 | 创建、修改、迁移、审计、分流或归档 Memo 时 |
| 入口可见要求 | AI 处理未任务化但有保留价值的信息、发现、提醒、问题或缺口时，应能定位本文 | 20 集合索引、运行入口摘要、Memo 分流流程入口 | AI 执行入口提示 | 信息保留、分流、归档或字段契约变化时 |
| 确定性执行要求 | Memo 字段、状态、分类、优先级、引用、文件命名和条件必填应由 Code 校验或记录缺口 | `specs-v2/07-Code实现规范.md`、Memo 校验 Code、正反样例 | 校验实现 | 字段契约、状态机、分类枚举、分流规则或引用关系变化时 |
