...
> 相关规范：`specs-v2/00-LD-Vibe-Harness理念与纲要.md`、`specs-v2/02-术语规范.md`、`specs-v2/03.04-工作模型文档规范.md`、`specs-v2/05.01-工作字段内容格式规范.md`、`specs-v2/07-Code实现规范.md`、`specs-v2/08-Web信息同步规范.md`、`specs-v2/09-事实源边界与承载规范.md`、`specs-v2/20-工作模型集合索引.md`、`specs-v2/21-ADR-决策.md`、`specs-v2/22-Change-变更.md`、`specs-v2/24-Intent-意图.md`、`specs-v2/25-Memo-备忘.md`、`specs-v2/26-Profile-项目画像.md`、`specs-v2/27-Task-任务.md`
...
4. 单纯的命令输出、日志片段或失败记录；
5. 已由 specs-v2、Rules / Instructions、ADR、Task 或 Code 明确约束，且没有新增经验的信息；
...
| 内容 | 权威位置 |
|---|---|
| Pitfall 工作模型规范 | `specs-v2/23-Pitfall-踩坑.md` |
| Pitfall 实例 | `ldvh-base/pitfalls/` |
| Pitfall 字段内容格式 | `specs-v2/05.01-工作字段内容格式规范.md` |
...
Task 的准入、状态和字段契约由 `specs-v2/27-Task-任务.md` 定义。Pitfall 不替代 Task 的验收标准、验证证据、关闭证据、风险判断或缺陷修复动作。
...
Intent 的准入、状态和字段契约由 `specs-v2/24-Intent-意图.md` 定义。Pitfall 不替代 Intent 的目标、约束、成功标准或完成判断。
...
ADR 的准入、状态和字段契约由 `specs-v2/21-ADR-决策.md` 定义。

### 4.5 Pitfall 与 Profile

Pitfall 可通过 `related_profiles` 记录经验适用或来源的项目画像。Profile 只帮助定位项目身份、路径和接入配置，不替代 Pitfall 的症状、根因、解决方式或适用范围。

Profile 的准入、状态和字段契约由 `specs-v2/26-Profile-项目画像.md` 定义。

### 4.6 Pitfall 与 Change

Pitfall 的创建、状态变化、核心经验改写、归档、替代和被吸收到规范、运行入口、Code、Web 或工作流程时，都应留下 Change。Change 的 commit message 契约由 `specs-v2/22-Change-变更.md` 定义。

### 4.7 Pitfall 与规范、Code、Web 和运行入口

当 Pitfall 中的规避策略需要长期生效时，应按内容性质分流：

| 需要沉淀的内容 | 承接位置 |
|---|---|
| 强制规则、字段契约、事实源边界或 Human Gate | specs-v2 正式规范 |
| 高频入口提示或硬约束摘要 | Rules / Instructions 运行投影 |
| 可复用多步骤流程 | Skill 或工作流程规范 |
| 独立、专项或并行审查视角 | Agent 运行投影或工作流程规范 |
...
6. 将 Pitfall 的规避策略吸收到 specs-v2、Rules / Instructions、Skill、Agent、Code、Web 或工作流程；
...
| `severity` | 影响程度 | string | 否 | `low`、`medium`、`high`、`critical`，默认 `medium` | Reference | AI、Code、Web |
...
| `related_profiles` | 关联 Profile ID 列表 | list[string] | 否 | 默认为空列表 | Reference | AI、Code、Web |
...
字段内容格式按 `specs-v2/05.01-工作字段内容格式规范.md` 执行。字段缺失、类型错误、状态非法、引用不存在、条件必填缺失或文件命名不匹配时，Code 应报告诊断，不得静默通过。
...
updated: 2026-06-09
symptoms: |
  AI 在迁移过程中引用旧 specs/ 文档中的规则，未先判断该内容是否已经吸收到 specs-v2。
trigger_conditions: |
  - [x] 旧文档内容比 v2 文档更完整
  - [x] 当前 v2 文档仍处于迁移期
root_cause: |
...
resolution: |
  先读取 specs-v2 的对应正文和集合索引，再把旧 specs 只作为迁移素材比较和吸收。
verification: |
...
applicability: |
  适用于 specs-v2 重建期、旧规范迁移和工作模型迁移任务。
severity: high
...
文件名变化必须同步检查引用该 Pitfall 的 Intent、Task、Memo、ADR、Profile、Change、Web 派生视图和 Code 聚合结果。
...
7. 适用范围和不适用范围；
8. Human Gate 确认记录；
9. 相关 Task、Memo、ADR、Profile、Change、docs、规范或 Code 引用。
...
3. 校验状态枚举和合法流转；
4. 校验 `severity`、`repeatability`、`superseded_by` 和引用字段；
...
| 确定性执行要求 | Pitfall 字段、状态、引用、文件命名、条件必填、标签和替代链应由 Code 校验或记录缺口 | `specs-v2/07-Code实现规范.md`、Pitfall 校验 Code、正反样例 | 校验实现 | 字段契约、状态机、引用关系、替代规则或标签规则变化时 |
| Human 交互要求 | Pitfall 创建、激活、归档、替代、核心经验改写和吸收到规范或实现时应触发 Human Gate | Human Gate、影响范围说明、确认记录 | 工作模型治理 | §5 中任一场景发生时 |
| 生命周期触发要求 | Pitfall 规范变化后，应检查 20、05.01、ADR、Memo、Task、Profile、Code、Web、运行投影和相关工作流程是否需要同步 | 集合索引维护、字段格式映射、对象关系检查、Code/Web 联动检查、人工降级检查 | 触发保障 | Pitfall 字段、状态、事实源边界、适配规则或检查要求变化时 |
...
1. Pitfall 校验 Code 待字段契约稳定后补齐正反样例；
2. Pitfall Web 展示、筛选和任务执行前提示入口待 Web 实现规划时补齐；
3. Pitfall 识别、创建、激活、归档、替代和吸收的具体工作流程待 40-59 迁入后承接；
4. `severity`、`repeatability` 和 `tags` 的枚举范围待更多实例实践后评估；
5. Pitfall 与工作流程中 Learn 阶段的关系，待 40-59 迁入后进一步校准。
