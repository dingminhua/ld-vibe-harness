...
> 文件性质：AI 统一入口 Rules 资产运行投影，不是 docs/specs 正式规范
...
环境入口、项目规则、工作区配置或会话提示只应指向 `rules/LDVH-AI-ENTRY.md`，不应复制本文正文或 docs/specs 正文。
...
LDVH 管理入口视角随 `rules/LDVH-AI-ENTRY.md` 提交，用于维护 LDVH 自身项目。该视角可以指向 `docs/specs/`、`code/`、`tests/`、`web/`、根目录配置和适配措施治理，但不得被误用为所有管辖项目的项目级规则正文。
