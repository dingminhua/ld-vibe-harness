...
| 外部资料与观察边界 | 外部资料以可复核 URL、标题和支持范围/限制摘要保留；当前观察和时效限制由正文与自然语言验证说明表达 | `urls`、正文、验证说明与重验证 | 时效治理 | 创建、更新或消费可变化结论时 |
| 报告与规则分离 | 发现、结论和建议不自动成为规则、决定、任务或事实当前性 | 本文、相邻类型来源、Human Gate | 权威治理 | 引用、吸收或提出行动时 |
| 状态与替代闭包 | 终态字段、整体替代、时间、单一直接 supersedes 关系和 DAG 同时成立 | 本文、对象集、Code checks、AI 审核 | 生命周期治理 | 状态或 supersedes 变化时 |
...
| 外部资料、证据与时效 | 创建、更新或消费可变化结论时 | `urls` 至少一项，且每项为 HTTP(S) URL、标题和支持范围或限制摘要；自然语言验证说明表明执行或观察内容、结论覆盖范围及未验证范围；重要发现可映射到对象自身说明与外部资料；重验证范围清楚 | 本文、03、对象正文、验证说明与外部 URL | URL shape 检查、正文/验证说明回读与 AI 观察审查 | 被核对的外部资料、结论和时效边界 | 暂停受影响结论消费；补充资料或说明、收紧 applicability、更新、替代或退出 |
...
Code 的共同机械边界按 05 §§10–11 执行；对 Study，只可额外检查本文明确给出的 frontmatter、`urls` 的 HTTP(S) shape、标题、摘要与去重、正文标题和 supersedes 约束。研究价值、外部资料充分性与真实性、观察时效、推断、限制、建议、自然语言同义性和外部事实当前性仍由 AI 依据当前来源审核。

最低验证样例必须覆盖：有效 HTTP(S) URL；缺 URL、标题或摘要；本机路径、代码/日志路径、会话描述、Git revision、非 HTTP(S) URL 与重复 URL；正文六个 H2 的缺失、重复、乱序和空内容；三个状态及条件字段；supersedes 的项目、类型、状态、全生命周期单一直接关系、时间、自指、缺失目标与 DAG。当前 Study parser/validator 已由 `read-fact-objects` 的只读实现从本文与统一登记派生；实现和测试只证明实际覆盖的机械范围，不得把读取通过冒充研究价值、外部资料真实性、证据充分性或当前可消费性已经成立。
...
2. 缺少可复核的外部 URL、标题或支持范围/限制摘要；自然语言验证说明不能表明执行或观察内容、结论覆盖范围和未验证范围；重要结论无法映射到对象自身说明与外部资料，或当前性要求尚未重验证；
...
6. 终态依据不足、supersedes 不是整体替代，或目标、时间、单一直接关系、DAG 不成立；
