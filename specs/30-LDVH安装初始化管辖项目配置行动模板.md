...
| 要求 | 机制 | 触发 | 证据 | 缺口处理 |
|---|---|---|---|---|
| 安装行动阶段要求 | 安装交互必须使用五个阶段表达当前阶段、已确认决策 / 结果和未发生事项；由本文、04、06、09 保障，最终确认前不得写入，不得新增安装运行时状态闭集 | 安装、接入、初始化或配置管辖项目时 | 五阶段交互输出、最终确认记录、安装交还摘要 | 缺少阶段或最终确认时停止写入，回到对应阶段重做 |
| 告知清单要求 | 安装、部署、初始化、配置或卸载前必须形成用户告知清单并交给 Human 确认；由本文、01、06、09 保障 | 涉及写入、入口、配置或卸载时 | 4/5 安装方案预览中的用户告知清单、Human 确认记录、source_refs | 缺少告知项时暂停，不进入最终确认 |
| 环境入口边界要求 | 插件、扩展包、Hook 或 manual entrypoint 只能作为入口承载，不得声明未验证 integrated；由本文、01 和环境入口审计保障 | 检查或改变环境入口时 | 环境入口审计输出、01.Att.04 分类、安装检测摘要 | 缺少真实自动触发、payload 或失败处理证据时交还未 integrated，不声明自动接入 |
| Git Hook 安装验证要求 | 完整安装必须覆盖每个已选择管辖项目的 Git `commit-msg` Hook，并在安装后通过统一验证入口或等价入口验证 status、managed hook、正例放行和负例阻断；由本文、01、Code adapter、tests 保障 | 安装、升级或修复 LDVH 时 | 每个管辖项目的 Git Hook status、managed marker、正反例验证结果 | 任一项目未通过时阻断安装完成交还，回到 Git Hook 修复 |
...
| 环境插件验收提示要求 | 环境插件安装、升级或复核必须提示插件页面、重启 App、授权 / trust、新窗口或新会话测试、正常判断标准和失败反馈；由本文、01、Code audit、tests 保障 | 安装、升级、复核或交还目标环境插件时 | 用户下一步待办、正常判断标准、失败信息包 | 缺少提示时补交交还摘要，不声明安装验收完整 |
| Git 项目准入要求 | 每个已选择管辖项目必须是有效 Git worktree；非 Git 目录必须阻断安装并说明管辖项目必须是 Git 仓库；由本文、10、Code adapter、tests 保障 | 安装、初始化或接入管辖项目时 | Git worktree 检查、Git common-dir 线索、阻断诊断 | 非 Git 目录停止安装，要求更换项目或先初始化 Git |
| 工作区配置 Gate 要求 | 生成或修改管辖项目配置前必须确认 LDVH 本体路径、目标工作区根目录和配置文件完整路径；配置位置固定为目标工作区根目录；由本文、10、Human Gate 保障 | 创建或迁移 `LDVH-GOVERNED-PROJECTS.yaml` 时 | 路径确认表、配置层级检查、Human 最终确认 | 路径或层级不清时停止写入，要求 Human 选择保留、迁移或删除冲突配置 |
| 事实源目录告知要求 | 首次启用管辖项目时必须说明 `ldvh-base/` 及五个子目录用途；由本文、10、20-24 保障 | 创建或检查管辖项目事实源目录时 | 事实源目录检查结果、用途说明、Human Gate 记录 | 用途未说明或目录创建未授权时不创建目录，不声明初始化完成 |
| 验证交还要求 | 完成声明前必须交还验证摘要、回滚或卸载入口和残留风险；由本文、09 保障 | 声称安装、初始化或配置完成时 | 验证摘要、回滚或卸载入口、残留风险、source_refs | 缺少验证或回滚信息时写为未完成交还，补齐后再声明完成 |
| 安装完成总结要求 | 安装执行完成后必须单列实际写入、未写入对象、Hook 接入后测试、不可验证范围和撤销 / 恢复入口；由本文、09、Code audit 保障 | 执行安装、初始化或配置写入后 | 安装完成总结、实际写入清单、未写入对象、Hook 测试摘要 | 缺少总结时不得只给零散测试结果，必须重做交还摘要 |
