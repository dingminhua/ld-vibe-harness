...
V3 的模板、附件、Skill、validator、Hook、WorkCase 和实例只作为重复价值与失败模式输入，不取得 V4 当前效力。本文不恢复 V3 的全局 Conventional Commit 契约、固定七结构、全局 validator/Hook、自动提交、单一主意图政策或文件类别触发的普遍 Human Gate。
...
4. 定位并读取实际适用的项目提交政策、领域规则和验证要求，记录实际检查来源、作用范围及未检查、不可读取或身份不清的缺口；
...
2. 项目来源要求拆分、目标范围无法共同说明、验证不能共同覆盖或候选无法安全隔离时，按实际来源拆分、缩小范围或交还 Human；模板不建立所有项目共用的“单一意图”政策；
...
4. 根据候选快照、Human 目标、项目 message 规则和实际验证形成提交说明。没有项目格式规则时，提交说明仍必须据实描述实际变化，不得声称未发生的验证、Gate、完成、push 或发布；
5. 在创建 commit 前重新核对实际候选快照、目标路径、message 和验证范围；任何不一致都回到本阶段处理，不得以最初计划替代当前快照。

#### C. 创建、回读与验证

1. 只有授权、目标快照、来源要求和必需验证均达到实际允许执行的范围后，才调用当次可用的 Git/Code 能力创建一个本地新 commit；
2. 失败、拒绝、无变化、Hook 修改或拒绝、部分结果或结果不可观察时，保留实际 Git 状态和诊断，不得重试破坏性操作、扩大路径、禁用既有 Hook 或绕过门禁；
