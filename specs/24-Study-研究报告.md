...
ldvh_spec:
  ...
  scope: "管辖项目中已经完成、以外部产品、实践、标准、论文、工具或方案为对象，且可独立引用的研究报告"
  basis:
...
> 文件状态：`active`。本文是 `study` 事实类型的唯一定义来源；它不使 Study 读取、创建、校验、Helper、Code、tests、行动模板或 Web 能力自动成立。任何外部材料都必须按本文重新满足来源版本、观察时点、适用边界与验证闭包，不能批量直接成为当前 active 对象。

## 1. 价值判断

Study 保存一轮已经完成、可独立引用且具有跨行动稳定阅读价值的**外部产品、实践、标准、论文、工具或方案研究**。其研究对象必须是明确的外部产品、实践、标准、论文、工具或方案；它使后续 AI 能快速恢复研究对象、输入边界、关键发现、建议和后续分流，从而避免重复研究，并把研究启发转化为项目下一步。

新建 Study 使用 `fact-object-controlled-creation`（31）；既有 Study 的事实更正、内容更新、状态变化或承接处置使用 `fact-object-lifecycle-change`（32）。模板不替代本文的外部研究准入、验证闭包或 Human Gate。
...
3. 研究问题、摘要、来源、输入边界、关键发现、建议与后续分流要求；
...
一份外部内容研究报告只有同时满足以下条件，才可以形成 Study：
...
未完成或仍需澄清的问题保持 Spark 或进入 WorkCase；已经作出的选择进入 ADR；已经发生、查明、解决并验证的失败机制进入 Pitfall；纯内部审计或分析按其稳定身份进入普通文档或相邻对象。内部审计或报告的确定原始文件在满足 25 的准入条件时可以作为 FileAsset 保存；其审计结论、决定、行动、适用性和未知范围仍由普通文档或相应事实对象承载，不因文件本体成为 FileAsset 而进入 Study。Study 回答“对哪个外部对象，在什么输入、方法、版本和观察时点下发现了什么、限制是什么”，不回答“当前必须遵守什么”“LDVH 已经决定做什么”或“内部实现应如何开展”。
...
|---|---|---|
| `study` | 已经完成、以明确外部对象为主体、可独立引用且具有跨行动稳定阅读价值的一轮外部内容研究报告 | `study-fact-type::5. Study 类型定义` |
...
研究问题由 `research_question` 表达；外部资料的用途、输入边界、关键发现、推断和限制由 `urls[].summary`、正文与 `abstract` 据实说明。不得以 URL 本身、关系或验证动作代替这些说明。
...
Study 在当前问题需要外部研究启发、准备调研实质相同或相邻的外部对象、比较外部方案或当前输入精确引用研究报告时产生召回机会。F2 Study 候选卡直接投影 `object_id`、`title`、`status`、`research_intent`、`research_question`、`abstract`、`recommendation_summary`、`relations` 和 `updated_at`，并只提示引用中是否存在版本或观察时点边界，不在候选层解释研究结论当前性。F2 投影可以同时含研究问题；它不是面向 Human 的首级详情布局。只在关键发现、限制或建议会影响当前判断时展开完整正文，不得因主题相似就向 AI 无差别注入整份报告。
...
| 外部资料、研究边界、关系与时效 | 创建、更新或消费可变化研究时 | `urls` 至少一项，且每项为 HTTP(S) URL、标题和支持范围或限制摘要；正文表达输入、观察、发现和未覆盖范围；`relations` 只使用 `inspired-by` 和 `informs` 且目标身份可解析；重读范围清楚 | 本文、03、05、对象正文与外部 URL | URL shape 检查、关系 key 与目标身份检查、正文与 URL 回读、AI 观察审查 | 被核对的外部资料、发现、关系和时效边界 | 暂停受影响发现消费；补充资料或说明、更新或退出 |
