...
行动编排设计审核是 LDVH 对具体行动编排规范主文件进行质量审查的行动编排。它回答：一个 `specs/30-59` 行动编排主文件是否能被 AI 正确读取、识别、执行、暂停、验证、回写和维护。
...
多个场景同时命中时，应先判断是否存在更具体的 active 行动编排。若没有更具体的 active 行动编排，或目标就是审核行动编排设计质量，则进入本流程。
...
| `not_applicable` | 当前对象不属于行动编排设计审核范围 |
...
|---|---|---|---|---|
| 上位约束承接要求 | 行动编排设计审核应承接 06 的行动编排通用规则和 03.03 的行动编排文档骨架、成员自描述契约、可测试性锚点 | 本文、06、03.03、Human Gate | 行动编排治理 | 新增、修改、升级、降级、删除、重命名或审查 30-59 行动编排时 |
| 入口可见要求 | AI 处理行动编排设计、行动编排变更或行动编排质量审查时应能定位本文 | Rules 入口、06、03.03、Code 派生索引 | AI 执行入口提示 | 行动编排场景路由、成员主文件、入口资产或 Code 查询入口变化时 |
| 流程复用要求 | 高频行动编排设计审核可以封装为 Skill，但 Skill 不得新增稳定规则或默认调度 Agent | Skill SOP、主控交还、人工降级检查 | 流程复用 | 行动编排设计审核反复执行且步骤稳定时 |
| 子 Agent 思考要求 | 复杂语义自洽、价值观一致性或流程职责冲突可由 Agent 辅助审查，但 Agent 输出必须回主控 | Agent 审查、主控复核、Human Gate | 子 Agent 思考 | 行动编排设计存在争议、高风险或跨流程冲突时 |
...
| 检查项 | 标准 |
|---|---|
| 对象归属 | 被审核对象属于 `specs/30-59` 行动编排主文件、候选草案或能力消费请求 |
