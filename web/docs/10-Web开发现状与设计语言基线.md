...
“推进中”Card 延续同一两区结构，但第二个区域从“成功标准”切换为“当前进展”：目标继续完整显示；结果推进主链显示四个固定推进环节并只高亮当前 `progress_step`，`plan_revising` 则显示“方案修订中”且不高亮四步。四个环节因来源已定义顺序而使用 1–4 编号，但编号不构成完成历史，前序环节不得因当前位置后移就显示完成。当前进展同时显示“已完成 N/T”、单独的取消数，以及全部 active 项的稳定 `item_id`、完整 `goal` 与状态；active 项包括 `in_progress` 和 `blocked`，以无序圆点呈现，渲染顺序不表示推进顺序。work item 结构、身份、状态条件或依赖不成立时，工作项完成数与 active 项整组显示不可判定，不使用数组位置、title、ID 或已退出字段补造线性执行信息。实际存在 `waiting_on` 或顶层 `blocking_summary` 时完整显示，二者都不替换当前推进位置。成功标准、scope、依赖、方法、完整工作项计划、Reviewer 复核、Controller 处置、Human 批准、验证说明和关闭报告与分流建议留在详情页。
