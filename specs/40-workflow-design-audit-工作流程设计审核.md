...
Code 派生集合索引、命令输出、Skill 输出、Agent 输出、Web 视图、聊天分析和 docs/studies 研究材料只能作为导航、诊断、候选建议或过程输出。审核结论形成稳定事实时，必须按事实类型选择唯一权威位置回写到目标工作流程主文件、相关工作对象、docs 报告、ADR、Spark、Pitfall 或 Git 提交记录，其他位置只保留引用或摘要。
