...
| 能力 | API | 写入目标 | 允许字段 | 约束 |
|---|---|---|---|---|
| Memo 快速创建 | `POST /api/memos` | `ldvh-base/memos/memo-{NNNN}-{slug}.yaml` | `title`、`description`、`source`、`category`、`priority` | 只能创建 `status: pending` 的新 Memo；必须校验必填字段、分类和优先级；必须写入 `status_history`；不得在创建后通过 Web 编辑 Memo 字段或状态 |
...
3. 写入后必须能重新读取目标文件并确认最小字段、`status: pending` 和 `status_history` 存在；
