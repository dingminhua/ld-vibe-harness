...
成功标准在 Card 与详情中采用同一蓝色“判断条件”语义，但使用符合各自阅读任务的密度。Card 继续使用“单一蓝色语义区＋圆点并列条目”的紧凑清单，不显示 `criterion_id`、当前结果或独立对象边界；正文使用 `12px / 20px`，保留 `4px` 圆点、首行中线对齐、`6px` 条目间距与 `10px` 圆点至正文间距。详情把每条标准呈现为与工作项结构一致的轻量对象：蓝色独立边界，标题栏显示弱化的稳定 ID 和实际存在的结果状态，标准陈述使用内页正文基准 `13px / 22px` 作为主正文，结果摘要作为对象内次级色块默认收起。成功标准对象不具有工作项依赖、方法边界或执行顺序；不得把数组位置写成编号。尚未形成当前结果时只省略状态和结果摘要，不省略标准对象或制造空结果；存在当前 `success_criterion_results` 时按 `criterion_id` 精确对照，`not_satisfied` 与 `not_verified` 保留各自警示色。呈现差异只由当前结果字段是否存在决定，不按 `phase` 猜测或补写结果。
...
“当前计划与工作项”默认呈现工作项身份、状态与工作内容；工作项主内容使用与工作项青色表面协调的青灰正文色，不回退为通用近黑正文。预期结果使用蓝色浅背景段落，方法边界使用灰色背景、边框与文字构成的完整中性色段落，两者正文默认收起并由 Human 点击标题行展开。工作项结果仅在来源字段存在时显示，使用绿色语义段落并默认展开。由于三类段落位于工作项框内，标题使用 Card 层级 `13px / 20px`，展开正文使用 Card 正文层级 `12px / 20px`；预期结果、方法边界与工作项结果标题都不放圆点或语义图标，只保留标题文字与展开箭头，具体语义由各自背景、边框和文字色承担。方法边界整体降为灰色信息层级，进一步区别于蓝色预期结果和绿色工作项结果；不得只改变文字而保留其它色系，也不得把预期结果与方法边界合并为同色、同一段或默认同时展开。
