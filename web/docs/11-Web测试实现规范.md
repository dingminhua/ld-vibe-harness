...
> 上位规范：[`specs/08-Web信息同步规范.md`](../../specs/08-Web信息同步规范.md)、[`specs/09-测试与验证规范.md`](../../specs/09-测试与验证规范.md)

## 0. 上位规范

`specs/08-Web信息同步规范.md` 是 LDVH Web 信息同步构成要素、派生视图、Human Gate UI、受控轻写入白名单和 Web 文档边界的权威规范。`specs/09-测试与验证规范.md` 是跨构成要素测试治理、验证声明、测试实现归属和测试证据事实源边界的权威规范。

本文只定义 `web/` 当前参考实现的测试入口、测试类型、测试映射、夹具组织和执行细则，不替代 specs、事实模型规范、行动编排规范、事实源对象契约或 Human Gate 条件。
...
1. 读取 `specs/08-Web信息同步规范.md`、`specs/09-测试与验证规范.md`、`web/docs/01-全局设计约束.md` 和当前页面对应文档；
...
当前 Web 参考实现只把 Spark quick create 作为正式写入入口；其它事实对象写入、WorkCase 状态推进、完整 Confirm UI 和 Human Gate 自动记录均未启用。

修改 Confirm UI 占位、受控轻写入或写入边界时，应遵守：

1. Spark quick create 继续由 `tests/web/api/sparks.test.ts` 覆盖创建、字段校验、冲突和写后校验失败；
2. 新增或扩大写入范围前必须先取得 Human Gate；
3. 每一种写入都必须补充对象类型、字段白名单、状态闭集、回写位置、`source_refs`、写后校验和失败处理测试；
4. Confirm UI 不得自动替代 Human Gate、验收通过、执行授权或风险接受；
5. Web 页面/API 不得把 Code 输出作为主数据源，也不得把页面派生态当作事实源。
...
3. 页面级测试与组件级测试的拆分边界需要在首批 `tests/web/` 用例完成后复核；
4. Web 与 Code 输出的测试对照样例需要在诊断输出结构稳定后补齐，但不得把 Code 输出升级为 Web 主数据源。
