...
`specs/07-LDVH事实模型基础规范.md` 定义事实模型通用规则、文件命名、附件型实践子文档命名和事实模型标准组成。本文依据 07 §4.2 定义 ADR 对象模型。
...
12. ADR Tools 契约式校验与执行适配、只读查询能力、受控写入能力缺口识别及其测试验收是否符合本文 §12和 `specs/06-PyTools与WebTools规范.md §4`；
