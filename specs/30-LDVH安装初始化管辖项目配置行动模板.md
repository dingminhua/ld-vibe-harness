...
> 文件状态：active；本文是 V3 第一个独立正式行动模板，吸收 V2 `33-ldvh-install-action` 的安装、初始化、首次管辖项目配置和验收交还闭环。本文不越过 Human Gate 直接安装或升级环境插件，不直接写入用户环境 Hook 系统文件，不声明任何环境入口 integrated，不恢复 Rules / Skill 顶层机制。
...
当前只迁入最小闭环：安装入口发现、安装方式判断、初始化检查、目标工作区确认、管辖项目登记、环境插件安装 / 升级方案判断、用户告知清单、验证、回写和交还。真实环境插件写入、完整部署自动化、用户级配置解析、项目内配置迁移和批量项目接入仍需 Human Gate 与最终确认。
...
安装向导的关键表格必须使用稳定图标帮助 Human 快速扫描。路径确认表项目列使用 `📦 LDVH 本体路径`、`🗂️ 目标工作区根目录`、`🧾 配置文件完整路径`；安装前检查项目列使用 `📦 LDVH 本体`、`🧾 工作区配置`、`⚠️ 配置层级`、`📁 事实源目录`、`🔎 配置解析`、`🔌 环境入口`；安装前检查结果列必须使用 `✅ 通过`、`⚠️ 注意`、`⚠️ 需安装`、`⚠️ 需升级`、`⛔ 阻断`、`➖ 不适用`；安装方案预览项目列使用 `🧾 工作区配置`、`🗑️ 项目内旧配置`、`📁 事实源目录`、`🔌 环境插件 / 外部 Hook`、`🧪 验证`、`↩️ 回滚`。图标只用于提高可读性，不得替代状态文字、路径、限制、Human Gate 或验证结论。

安装前检查只能输出系统已检查事实，不得把事实伪装成 Human 选项。目录已存在、配置已存在、项目内配置不受支持、当前实现不支持用户级配置目录、旧插件和旧路径状态、外部 Hook 状态等，都应写入检查结果、需授权动作或后置项；不得继续询问 Human 是否创建已存在目录，或把项目内配置、用户级配置目录放进主选项。环境插件未安装时必须标为 `⚠️ 需安装` 并安排安装方案；环境插件已安装但指向旧路径、旧版本或 stale V2 path 时必须标为 `⚠️ 需升级` 并安排升级方案；不得把这类情况表达成“不安装插件”或“不处理插件”。

目标工作区配置已存在且方案倾向保留时，安装前检查必须读取并展示当前配置项目清单，不得只写“保留工作区配置”。配置清单必须用表格列出 `product_name`、`product_description` 摘要、每个项目的项目 ID、名称、项目路径、Git common-dir、是否命中当前目标和检查结论；同时给出配置正确性结论，至少覆盖字段闭集、必填字段、项目 ID 唯一、路径可解析、target-first 命中和配置层级冲突。新建或重写配置时，安装方案预览必须用同等表格展示拟写入项目清单，让 Human 能比较“当前配置”和“拟写入配置”。
...
安装方案预览必须合并所有 Human Gate：写入对象、写入位置、将执行动作、不会执行的事项、后置项、验证命令、失败处理和回滚方式都必须在此步展示。若环境插件未安装，方案预览必须安排安装；若环境插件已安装但过时或指向 stale V2 path，方案预览必须安排升级；插件安装或升级的写入对象、影响范围、验证和回滚入口必须明确。若目标工作区根目录到目标项目路径链上已经存在项目内 `LDVH-GOVERNED-PROJECTS.yaml`，安装方案预览必须报告配置层级冲突，并要求先删除、迁移或明确保留其中一个配置文件；不得继续进入执行。最终确认前，AI 只能只读检查、展示结果和收集选择，不得写入配置、创建目录、安装插件、修改 Hook、修改用户环境或声明 integrated。
