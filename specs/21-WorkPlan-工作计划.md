...
1. Code 中旧 TaskPlan / Task / SubTask 读取、校验和测试夹具应继续作为历史兼容面维护，不得重新成为新建入口；
2. Web 中旧对象直达路由和详情展示应继续作为历史兼容面维护，主导航、Dashboard 和默认关注队列不得重新暴露旧对象；
3. 既有旧事实源的冻结、归档或删除策略需在工作流程或后续工作计划中单独处理。
