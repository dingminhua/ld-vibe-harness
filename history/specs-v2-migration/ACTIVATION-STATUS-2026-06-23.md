...
2. Web v2 接入需要按 05 与 08 单独规划；
3. 历史记录价值提取需要先建立清单，再按 v2 事实模型重写进入新事实源。
...
| 非根 README 清理 | 已完成 | LDVH 仓库只保留项目根 `README.md` 作为 README 入口；历史迁移目录、资料目录、Code 归档目录和 Web 目录不再保留独立 README |
