...
附件型实践子文档含义、命名规则、状态骨架和联动要求见 `specs/04-LDVH模型子文档规范.md`。ADR 对象模型已为六个子文档槽位形成显式结论；该结论表示模型实践子文档状态已声明，不等同于 ADR 对象实例事实源、Tools 测试或项目落地初始化已经完成。
...
21.01-21.06 均应保留为当前实践展开。后续新增、删除、重命名或改变附件型实践子文档状态，应按 `specs/04-LDVH模型子文档规范.md` 和 Human Gate 要求处理。若子文档状态、实体实现、测试验收或项目实际落地状态不一致，应先重新执行落地前决策和价值与要素审查，再进入初始化或整改。
...
4. 是否需要更新 ADR 的 Rules、Skill、Agent、Tools、Web 或 Contract 子文档状态；状态应符合 `specs/04-LDVH模型子文档规范.md` 的六槽位完整实践要求。
...
14. ADR 附件型实践子文档命名和六槽位状态是否符合 `specs/04-LDVH模型子文档规范.md`，是否存在对应子文档且经 Human Gate 确认。
