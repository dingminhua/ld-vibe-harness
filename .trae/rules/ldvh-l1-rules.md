...
新增机制、规则或规范内容必须服务 LDVH 价值标准。Human Gate 场景见工作区 L0 "场景入口"。提交时必须调用 `ldvh-commit` Skill，不得跳过 Skill 直接 `git commit`（触发条件见 L0 "场景入口" 强制触发条件清单）。Skill 提示词、Skill 文档和面向 AI 的 Skill 编排说明使用中文，除非外部平台字段或技术标识必须使用英文。框架建设必须服务最近一次可运行闭环；连续新增抽象而未 Dogfood 时，应暂停扩张并转入实例验证。涉及规范、Rules、Skill、Agent、Tools、事实模型、状态机、部署边界或目录边界的判断，必须先定位并读取对应规范原文，不得只凭记忆、目录现状或经验推断。
