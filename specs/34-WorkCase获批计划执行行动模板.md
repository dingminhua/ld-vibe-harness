...
本文负责组织获批 WorkCase 的执行顺序、稳定检查点、授权消费、主动终止善后和恢复分支；不负责判断语义相关性、替 Human 批准计划或关闭、定义状态/phase/字段、生成 after、创建其它事实对象类型规则、实现调度器、声称 spawn/subagent 能力存在，或把测试通过解释为 WorkCase 完成。AI 仍依据计划、依赖和当前事实选择下一个工作项；需要委派时只调用环境当次实际可用的能力。每次事实变更仍必须经 32 路由到 21 的 `update-workcase`、`begin-workcase-termination`、`complete-workcase-termination`、`close-workcase` 或 `correct-closed-workcase`；本文不提供平行写入入口。
...
执行者同时读取当前 plan、未完成 item 与依赖、fresh creation review、每项 review 的 `actual_method` 与条件性披露、冻结 `execution_authorization`、baseline fingerprint 和 Human `source_refs` 均准确的 `execution_approval`，逐项核对当前行动是否仍处于目标、影响、风险、能力与禁止项边界内。`status + phase` 是当前活动位置事实；投影只是 21 基于刚回读快照形成的非持久派生合同。21 定义全部字段、phase、转换、quality gate 与投影语义，本文只组织 Controller 流程；32 组织受控写回。本文不重述其闭集或成立条件。

每次真正发起 plan、result 或关键 item review 前：
...
若刚回读对象已为 `termination_preparing`，执行者只消费 `next_required_control_step=termination_cleanup`；旧 plan、items、授权、review、结果或 proposal 只作为中止起始现场，不恢复原计划，也不发起新的 review。
...
若 Human 已明确主动中止，Controller 先以 fresh fingerprint 调用 `begin-workcase-termination`，并立即停止旧计划的未来动作。进入 `termination_preparing` 后，只允许盘点已形成内容、按 Human 精确范围保留或丢弃、标明未验证范围、处置关系影响和据实记录被执行或跳过的质量步骤；不得继续、重开或重排旧 item。物理清理仍由 AI 在当次明确授权范围内执行，Helper 只保存事实；范围含混、可能误删他项内容、需要破坏性历史改写或入向责任无法处置时写入真实 blocked 和恢复条件，不猜测清理。
...
**Gate 2 非持久化环境阶段 Goal：** Gate 1 有效批准后，Controller 应主动尝试建立一个"依当前 LDVH 规则将当前获批 WorkCase 推进到 Gate 2"的非持久化环境阶段 Goal——若宿主环境当前可观察且可调用原生 Goal 能力，并且不存在与当前事项冲突的未完成 Goal，Controller 建立该 Goal；能力不可用、不可观察、存在冲突或调用失败时，Controller 必须在当前 WorkCase 的 `current_summary` 或等价稳定位置记录该缺口，保留该缺口并原样继续当前 LDVH 流程，不伪造 Goal，不引入适配层。它只是当次执行的注意力与续跑约束，不是 LDVH 事实对象、状态、授权、投影或第二调度器，不写入 canonical 载体或模板运行字段，也不要求任何宿主专用 API 名称或适配层。
