...
| 一 | 本文解决的问题 |
| 二 | 与 13 的关系 |
| 三 | 对象定位与准入条件（§3.1 定义、§3.2 与临时判断、§3.3 准入条件、§3.4 与 specs/Rules 边界） |
| 四 | 事实源边界 |
| 五 | 状态机（§5.1 标准状态、§5.2 合法状态流转） |
| 六 | 与其他对象的关系（§6.1 Memo→ADR、§6.2 Task/Evidence→ADR、§6.3 ADR→Task、§6.4 ADR→specs/Rules、§6.5 不满足准入） |
| 七 | Human Gate（§7.1 必须触发、§7.2 应评估、§7.3 推翻与废弃） |
| 八 | 字段契约（§8.1 基础字段、§8.2 ADR 扩展字段、§8.3 字段约束） |
| 九 | 事实源回写要求 |
| 十 | 证据留存要求 |
| 十一 | AI 协作适配 |
| 十二 | Tools 辅助适配 |
...
8. 确认 ADR 状态流转遵循本文 §五 定义的状态机；
9. 确认 ADR Human Gate 触发条件遵循本文 §七；
...
6. ADR 读取策略是否符合本文 §十一、§十二和 §十五，是否覆盖读取入口、状态适用、读取后分流和 Tools 只读查询边界；
7. ADR 写入策略是否符合本文 §七、§九、§十二和 §十五，是否覆盖 Human Gate、受控写入、人工写入、Change 记录和 Tools 降级边界；
8. ADR 创建和状态变更 Human Gate 是否符合本文 §七 定义的触发条件；
9. ADR 事实源回写是否符合本文 §九；
10. ADR 证据留存是否符合本文 §十；
11. ADR AI 协作适配是否符合本文 §十一；
12. ADR Tools 契约式校验与执行适配、只读查询能力、受控写入能力缺口识别及其测试验收是否符合本文 §十二和 `specs/12.01-Tools辅助规范.md`；
13. ADR Web 信息同步适配是否符合本文 §十三；
