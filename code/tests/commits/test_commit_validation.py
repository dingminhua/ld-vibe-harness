...
def _legacy_invalid_repair_message(before: bytes, *, fingerprint: str | None = None, human: str = "approved-by-human") -> str:
    return (
        "fix(code): 更正 legacy 历史署名\n\n"
        "关键变更:\n- 更正已知无效历史署名\n\n"
        "Session-ID: test-session\n"
        "Model-ID: gpt-5.6-luna\n"
        "Workbench-Name: Cindy\n"
        "Legacy-Repair-Object: spark-0001\n"
        f"Legacy-Repair-Fingerprint: {fingerprint or hashlib.sha256(before).hexdigest()}\n"
        "Legacy-Repair-Field: change_log[0].signature.agent_workbench\n"
        "Legacy-Repair-Old: claude-code-mcp\n"
        "Legacy-Repair-New: Cindy\n"
        f"Human-Gate: {human}"
    )


def _legacy_invalid_repair_after() -> tuple[bytes, bytes]:
    before = _VALID_SPARK.replace(b"agent_workbench: Cindy", b"agent_workbench: claude-code-mcp")
    repair_summary = (
        "受控更正历史 change_log 中的 agent_workbench 格式；修复项为: "
        "0: claude-code-mcp -> Cindy。"
        "原始错误值已由本次更正覆盖并保留本条修复记录。"
    )
    after = _VALID_SPARK.replace(b"updated_at: 2026-07-01T00:00:00+08:00", b"updated_at: 2026-08-10T09:05:09.107498Z")
    after += (
        f"  - at: 2026-08-10T09:05:09.107498Z\n    summary: '{repair_summary}'\n"
        "    signature:\n      model_id: gpt-5\n      agent_workbench: Cindy\n"
...
def test_invalid_legacy_before_image_passes_only_with_exact_control_metadata(
    contract: CommitContractProjection,
) -> None:
    before, after = _legacy_invalid_repair_after()
    result = validate_commit(
        contract,
        _input(
            contract,
            message=_legacy_invalid_repair_message(before),
            candidate_paths=("ldvh-base/sparks/spark-0001.yaml",),
            fact_candidates=(_fact_candidate(data=after, head_data=before, head_exists=True),),
            fact_schemas=(_spark_schema(),),
        ),
    )
    assert result.outcome == "passed", [f"{issue.code}: {issue.message}" for issue in result.issues]
...
def test_invalid_legacy_before_image_rejects_wrong_fingerprint(
    contract: CommitContractProjection,
) -> None:
    before, after = _legacy_invalid_repair_after()
    result = validate_commit(
        ...
        _input(
            contract,
            message=_legacy_invalid_repair_message(before, fingerprint="0" * 64),
            candidate_paths=("ldvh-base/sparks/spark-0001.yaml",),
            fact_candidates=(_fact_candidate(data=after, head_data=before, head_exists=True),),
            fact_schemas=(_spark_schema(),),
        ),
    ...
    assert result.outcome == "failed"
    assert "legacy_signature_repair_fingerprint_mismatch" in _codes(result)


def test_invalid_legacy_before_image_requires_human_authorization(
    contract: CommitContractProjection,
) -> None:
    before, after = _legacy_invalid_repair_after()
    message = _legacy_invalid_repair_message(before, human="").replace("Human-Gate: \n", "")
    result = validate_commit(
        contract,
        _input(
            contract,
    ...
    assert result.outcome == "failed"
    assert "legacy_signature_repair_human_gate_missing" in _codes(result)


def test_invalid_legacy_before_image_rejects_non_signature_difference(
    contract: CommitContractProjection,
) -> None:
    before, after = _legacy_invalid_repair_after()
    after = after.replace(b"title: \xe6\xb5\x8b\xe8\xaf\x95\xe7\x81\xab\xe8\x8a\xb1", b"title: \xe6\x94\xb9\xe5\x8a\xa8\xe7\x81\xab\xe8\x8a\xb1")
    result = validate_commit(
        contract,
        _input(
            contract,
    ...
    assert result.outcome == "failed"
    assert "legacy_signature_repair_non_signature_change" in _codes(result)
