...
3. 当前 branch/`HEAD` 状态（包括 detached、unborn、无首个 commit 或不可解析状态）、Working Tree、真实 Index、未跟踪内容和足以判断目标范围的实际差异；
4. AI 已明确声明的当次候选文件清单，以及逐文件核对的对应 Working Tree diff、staged diff 与审核结论；
5. 当次实际检查过的项目提交政策、领域规则、验证要求和能力入口，以及各自作用范围、未检查、不可读取或身份不清的缺口；
6. 已经执行的验证及其对象、版本、范围、结果和未验证部分；
7. 可能属于用户、其它事项、其它执行者或其它提交的既有变化；
8. 对受管辖目标，03 定义的 `precheck-git-commit` 当次可用，且已经准备直接传入该操作的完整 message；
9. 对受管辖目标，09 要求的 common-dir `commit-msg` Git Hook 已在当前实际 worktree 对应部署边界中静态确认为有效、可执行且无冲突；本模板只消费该状态，不安装、修改或修复 Hook；
10. 候选包含 `ldvh-base/` 中的事实载体时，各次实际事实写入所使用的来源定义 Helper 受控操作、精确写后回读，以及对当前事实集合、规则源和同一 worktree 仍有效的当次 `check-fact-integrity` 完整结果。
...
4. Git、Code、tests 和环境工具只提供实际支持的状态读取、差异、暂存、验证、原生本地 commit 创建和回读能力；静态存在不证明当次可用。对受管辖目标，09 要求的有效且可执行 `commit-msg` Git Hook 是创建 commit 的前置条件，并且必须由不带 `--no-verify` 的真实 Git commit 事件触发；静态文件存在、直接调用 Hook 或 Helper 预检都不能替代该真实触发；
5. 本模板不安装、修改、禁用、绕过或定义 Git Hook，也不要求特定 Skill、Agent、validator 子命令或平台接口。对受管辖目标，它只消费 09 已定义的源码仓库、薄 Skill、管辖配置和 common-dir Hook 接入边界，不为提交行动构建或安装 LDVH wheel、sdist、editable distribution、Python package 或插件；未来薄接入只能调用同一当前模板和相应能力，不得复制正文或项目规则；
...
3. AI 明确列出准备提交的精确文件，逐文件审核对应 diff，并将目标变化与无关、来源不明、属于其它事项或尚未获准的变化分开；对受管辖目标，若当前真实 Index 已包含任一未声明、无关或归属无法确认的 staged 内容，立即停止，不得触碰、代为 unstage、覆盖、清空或改用临时/alternate Index 隔离；
...
4. AI 明确声明的候选文件清单、逐文件 diff 审核结论、真实 Index 精确路径与 staged diff；受管辖目标中事实候选的 Helper 受控写入、精确回读与写后完整性审计依据，实际调用提交预检 Helper 时的原始请求与结果、AI 语义审核结论，以及 common-dir Hook 静态确认和原生 `commit-msg` Git Gate 的真实事件触发结果；受管辖目标未确认 Hook 或未触发时只能报告停止与接入缺口，不能报告提交闭环；
...
7. commit 后真实 Index 中剩余的 staged、Working Tree 中的 unstaged/untracked，以及全部无关变化；
...
| 提交候选范围 | 形成候选及创建 commit 前 | AI 已明确候选文件并逐文件审核对应 diff；受管辖目标只使用真实 Index，其精确路径集合与 staged diff 分别等于已声明候选和已审核内容；无关、未知或来源不明 staged 内容使流程硬停止，且未被触碰、unstage、覆盖、清空或转移到临时/alternate Index | 改变前后 status、逐文件 Working Tree/staged diff、真实 Index 路径集合、Human 目标和来源 | Git 只读观察、精确 stage 及 AI 差异审核 | 当次真实 Index、候选文件与已审核内容 | 取消创建；保持既有 staged 和无关变化原状并交还，不以 unstage、清空、覆盖或 alternate Index 继续 |
| 验证与 message 声明 | 准备创建及交还时 | 实际来源要求的验证已按当前候选执行；事实候选具有 Helper-only 写入、精确回读与外层 `outcome=ok`、`result.status=complete` 的写后完整性审计依据；受管辖目标的完整 message 与当前真实 Index 已通过 `precheck-git-commit` 取得外层 `outcome=ok`、`result.mechanical_outcome=passed`；AI 语义审核独立完成；message 与完成声明没有超出实际变化、验证和 Git Gate 结果 | 项目/领域规则、实际测试结果、真实 Index 候选、事实写入与审计结果、Helper 预检原始请求/结果、AI 审核和 message | 来源规定的验证入口、Helper 直接调用及 AI 对照审核 | 当次已执行事实写后检查、机械预检、语义审核、验证和声明 | 任一受管辖预检失败、不可验证、不可用或调用错误都停止创建；不得绕过 Helper 受控写入或来源门禁，不得用任一机械结果替代 AI 审核 |
...
| commit 创建与回读 | Git 返回后准备声明成功时 | 受管辖目标的有效可执行 common-dir Hook 已静态确认；一次不带 `--no-verify` 的原生本地 `git commit` 实际触发 `commit-msg` Gate；新 commit 的身份、message、tree、精确内容范围和提交后真实 Index/Working Tree 已回读，并与 AI 声明、审核且获授权的候选一致 | Hook 静态状态、真实 Git 事件结果、`HEAD`、commit 内容、Git Gate 可观察诊断和提交后状态 | 原生本地 `git commit` 及只读回读 | 当次本地新 commit、真实 Hook 事件、环境结果与已观察剩余变化 | Hook 缺失、冲突、不可执行、无法确认或未触发时不创建或不声明成功；报告拒绝、部分结果、实际残留和 09 接入缺口，不在本模板部署 Hook |
...
4. AI 未明确声明候选文件或未逐文件审核对应 diff；真实 Index 混入未声明、未获准、来源不明或属于其它事项的 staged 内容；或者为了继续提交准备触碰、unstage、丢弃、覆盖、回退、隐藏、清空既有变化，或建立/切换临时或 alternate Index；
5. 项目或领域来源要求的验证未完成、失败或不覆盖当前候选，却准备绕过要求或声明完整验证；
6. message、验证或完成声明超出实际候选快照、来源、证据、Git Gate 或可观察结果；
7. commit 创建失败、被拒绝、部分完成，原生 `commit-msg` Hook 未触发或无法确认触发，或者新 `HEAD`、message、tree、内容范围、真实 Index 和剩余 Working Tree 无法回读，却准备声明本地历史锚点已经形成；
8. 提交后没有核对新 commit 和剩余 Working Tree/Index，或者目标相关残留和无关变化准备被静默省略；
9. 准备通过本模板执行或暗示已经执行 push、PR、amend、rebase、merge、tag、发布、Git Hook/CI 安装、修改、禁用或绕过；对受管辖目标，common-dir `commit-msg` Hook 缺失、冲突、不可执行或无法确认，却准备继续创建，或者准备使用 `--no-verify`、直接调用 Hook 或静态检查替代真实 Git 事件；
...
12. 目标为受管辖项目，却准备跳过 `precheck-git-commit`，在该能力不可用、调用错误或机械结果不是外层 `outcome=ok` 且 `result.mechanical_outcome=passed` 时继续，或者真实 Index、候选、完整 message、worktree、管辖、来源、验证或 Hook 状态变化后仍复用旧结果；
