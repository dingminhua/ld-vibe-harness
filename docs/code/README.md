...
本增量已完成请求闭集、精确 H2/H3 选择校验、L4 全文、授权附件与唯一父规范全文、L3 保守展开、固定来源回指、披露一一对应、结果聚合和 Helper 显式实现映射。`MarkdownDocument` 在安全读取并通过前后文件身份一致性检查后，同时保留完整解码原文、原有换行和观察时间；领域读取不重新打开文件，因此内容与 `observed_at` 来自同一次检查快照。

失败分类已按 01 分开：允许载体集合形成后的 `draft/retired`、未知 key 或不唯一标题为 `invalid_request`；重复 key 或来源身份与范围无法确定为 `error`；active 载体因关系、授权或 Stop Conditions 阻断为 `rejected`；存在独立完成项与未完成项时为 `partial`；零完成保持 `error > rejected > unavailable` 的外层聚合顺序。单项 capabilities 只把这些结果映射为已有的可用性枚举，没有扩展共同契约。

完整 Code tests 为 316 passed；Ruff 检查与格式检查、`git diff --check` 全部通过。真实 CLI 已验证请求特定 capabilities、L4 根规范、L4 附件与父规范、L3 H2/H3 精确选择后展开、以及无效选择。独立复核的最终结论为 0 Blocker、0 Major、0 Minor。当前“L3 请求能完成”表示其按契约明确展开到 L4，不表示已有可缩小返回范围的通用 L3 机械判据。
