...
> 相关规范：`specs-v2/00-LD-Vibe-Harness理念与纲要.md`、`specs-v2/02-术语规范.md`、`specs-v2/03.04-工作模型文档规范.md`、`specs-v2/05.01-工作字段内容格式规范.md`、`specs-v2/07-Code实现规范.md`、`specs-v2/08-Web信息同步规范.md`、`specs-v2/09-事实源边界与承载规范.md`、`specs-v2/20-工作模型集合索引.md`、`specs-v2/24-Intent-意图.md`、`specs-v2/27-Task-任务.md`
...
1. 修改 specs-v2 正式规范、evals 吸收结果、refs 摘要或迁移结论；
...
| 内容 | 权威位置 |
|---|---|
| Change 工作模型规范 | `specs-v2/22-Change-变更.md` |
...
Task 规范由 `specs-v2/27-Task-任务.md` 定义。Change 不替代 Task 的验收标准、关闭证据或状态机。

### 4.2 Change 与 Intent

Intent 的创建、完成、关闭、成功标准变更和关联 Task 调整都应留下 Change。Intent 完成证据可以摘要引用相关 Change，但完成判断仍由 `specs-v2/24-Intent-意图.md` 定义。
...
修改 specs-v2、Code、Web、测试、环境适配记录或运行投影时，Change 应在 `type`、`scope`、subject 或 body 中说明影响范围。Change 只记录变更事实，不定义规范规则、Code 行为或 Web 状态。
...
1. 修改 specs-v2 正式规范、事实源边界、工作模型字段契约或状态机；
...
| `body` | 否 | 说明变更原因、内容、风险和 Human Gate 结果 | `新增 specs-v2/22...` |
...
新增 specs-v2/22-Change-变更.md，明确 Change 以 Git
...
| 确定性执行要求 | commit message 格式、type、scope、Refs、subject 长度和中文约束应由 Code 校验或记录缺口 | `tools/commit_validate.py`、`specs-v2/07-Code实现规范.md`、正反样例 | 校验实现 | commit 格式、枚举、Refs 规则或校验实现变化时 |
