...
WorkCase review 是 Reviewer 对计划版本或结果版本提供的只读第二视角。`subagent-read-only` 是唯一独立审核方法；`same-ai-switched-role-read-only` 只是无法创建 Subagent 时的低保证替代，不是独立审核，也不得显示为等价方法。不存在其它协作执行体、平台专用执行体、多层路由、Reviewer 模型选择或持久路由策略。

#### 4.5.1 Controller 能力判断与直接 fallback
...
- 把 same-AI 切换视角称为 Subagent、独立审核或等价审核
...
本定义适用于创建 review、PlanΔ fresh review、result review 与确有必要的关键 item 只读第二视角。方案与结果 review 仍按 §5 的 `workcase-review` 字段和 §6 的版本绑定推进，不因 same-AI fallback 改变 lifecycle phase 或字段所有权。
...
| `workcase-quality-gate-reviewer-mode` | `reviewer_mode` | string | 该关口默认且保证更高的标准 Reviewer policy 标识 | 不表示 Code 能证明实际独立性、任何具体执行者身份或当次 `actual_method` | 当前闭集精确为 `independent-read-only`；same-AI fallback 时仍保持该标准 policy 标识，实际方法只由 review 据实记录 |
...
新建候选与 `human_plan_confirming → executing` 的 Gate1 边还必须通过同一最小质量关口检查：`execution_authorization.quality_gates` 精确声明唯一标准 policy `independent-result-review / independent-read-only`，分别引用当前 `authorized_actions` 中不同的委派与结果复核 action；这些稳定标识不证明当次已经独立复核。每项当前 `creation_reviews` 精确覆盖该 gate_id，并必须记录活动期当前闭集中的 `actual_method`。same-AI review 必须同时具备 Controller 自身 `capability_evidence`、`assurance_gap`、`human_disclosure_summary` 与 `human_disclosed_at`。Code 只检查枚举、成员闭集、引用、版本与字段出现条件，不解释证据正文，也不把 fallback 追认为独立。历史 closed 对象保留旧文本、枚举、投影与附件引用的只读可读性，不回写历史；旧方法不得用于新建或活动 WorkCase。
...
9. Reviewer conclusion 不自动推进或否决。进入 `closure_preparing` 前至少一项实际结果复核已经形成，全部 feedback 已有 Controller resolution，projection 仍完整；结果复核的实际方法与条件性披露必须由该次 review 自身完整记录，不从 Gate1 授权或其它 review 继承；
...
- `item_event` alternative 只允许以下 discriminated union；每种 object 都使用列出的 exact-key 闭集，全部 string 必须非空且不得带调用方提供的时间、字段路径、任意 fragment、`set` / `remove` 或 merge 指令：
  ...
  - `complete-work-item`：字段恰为 `event_key`、`item_id`、`result_summary`、`change_summary`；目标 item 必须唯一且为 `in_progress`，完整 after 只把该 item 改为 `completed`、移除 `current_summary` / `resume_from` / `blocking_summary`、设置 `result_summary` 并追加一条 `change_log`；当该完成使全部 item 进入 terminal（`AllTerminal`）时，event projector 在同一事务内把 `phase` 由 `executing` 推进为 `controller_checking` 并在首次结果投影时设置 `result_version=1`，不留下 `executing + AllTerminal`；该投影复用同一发布函数与 §6 transition 校验，不引入第二套逻辑；
  - `cancel-work-item`：字段恰为 `event_key`、`item_id`、`result_summary`、`change_summary`；目标 item 必须唯一且为 `in_progress` 或 `blocked`，完整 after 只把该 item 改为 `cancelled`、移除 `current_summary` / `resume_from` / `blocking_summary`、设置 `result_summary` 并追加一条 `change_log`；同样适用 `AllTerminal` 同事务 `executing → controller_checking` 投影；不自动改写 `authorization` / `approval`、不进入 Human waiting、不解除 `depends_on`；
- `item_event` 只在 `status=open, phase=executing` 适用，不自动选择 item、不解除依赖、不修改 plan/result/review/approval/proposal/closure 或形成 Human 决定。`complete-work-item` 的同事务 `executing → controller_checking` 投影是本节明确定义的唯一 phase 推进例外，不授权其它事件推进 phase。start / complete 的状态不匹配和 checkpoint no-op 均拒绝；不提供幂等成功、自动 replay、rebase 或 merge，调用方必须重新精确读取；
...
`resolved` 另有字段 `lifecycle_position`、`handoff_narrative_key`、`next_required_control_step`、`progress_group`、`progress_step`、`blocking_overlay`、`stage_goal_status` 与 `pending_item_observation`；`handoff_allowed` / `handoff_reason` 仍按 §9.3.1 派生并作为共同字段存在。`progress_step` 无适用值时为 `null`；其余字段必填。`stage_goal_status` 与 `pending_item_observation` 是非阻断观察字段，不改变 `handoff_allowed` 判定，不构成第二调度器或 phase 推进依据。`stage_goal_status` 闭集为 `established`、`not_attempted`、`unavailable`、`conflict`，默认 `not_attempted`，只在当前对象显式声明时取其它值；该字段只投影非持久化环境阶段 Goal 的可见性缺口，不持久化 Goal 本身。`pending_item_observation` 闭集为 `no_pending_items`、`has_pending_items`、`unavailable`，`closed` 或无 `work_items` 时为 `unavailable`；该字段只让"应推进但未推进"的 item 漂移可见，不阻断合法 no-change 更新。非 blocked 的确定性基表如下：
...
| 准入与创建 | 建议建立、形成正式计划和受控创建前 | Human 工作意图、单一责任、scope、criteria、查重、净价值、完整计划与实际方案复核成立；若 Controller 实际无法创建 Subagent，same-AI 的 Controller 证据、保证差距、披露摘要和披露时间已据实记录；Controller 与 Reviewer 已逐项确认没有 item 或 criterion 吸收生命周期关口或 Human Gate | Human 当前指令、当前来源、相邻事实回读、候选计划与 Reviewer 实际反馈 | AI 语义审核、逐 item / criterion 生命周期关口检查、事实召回、受控创建校验与创建后回读 | 当次候选的已读范围、实际方法、保证边界、语义审核和创建结果；Code 不证明证据语义或未来执行成功 | 不创建；先补齐 same-AI 披露记录或返修误建模 item / criterion，或更新现有对象、留在当前行动、拆分或转 Spark |
...
| 结果与复核 | 形成结果、发起复核、处置反馈或改变 projection 时 | projection 完整、criterion 全覆盖、版本冻结、实际 review 方法/保证边界与 Controller resolution 成立 | item 终值、当前结果与 validation、Reviewer 实际输出、当次 review 的实际方法及条件性披露 | AI 结果审核、规范化 projection 比较、CAS、review/版本/条件字段检查 | 当次结果包结构、已读观察、实际方法和 review 绑定；不证明证据语义或技术结论天然正确 | 不进入关闭准备；补事实、升版、清旧 review，或重新完成当次 Subagent 创建判断与复核 |
