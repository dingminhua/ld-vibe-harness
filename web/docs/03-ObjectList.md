...
- “方案待确认”Card 是 Gate 1 的紧凑入口：完整直读 `goal`、`success_criterion_definitions` 与 `execution_authorization`；三者分别固定使用“目标”“成功标准”“执行授权边界”标题。授权区只以允许动作、禁止项和实际存在的 Human 前置条件数量形成可扫读摘要，不提供展开控件；完整授权动作与边界统一留在同源详情页阅读。摘要不替代或截断来源内容。Card 不显示 `scope`、`work_items`、`creation_reviews` 或 `execution_approval`，这些完整材料留在同源详情页。项目认知中心的决定依据及“复制决定摘要”仍消费完整 Gate 1 投影，不截断、不重新摘要；任一结构缺失或 malformed 时在原位置明确标注，不能丢弃坏成员后形成伪完整基线。
