...
4. 证据留存位置为 ADR YAML 实例的 `context`、`decision`、`consequences` 字段。【暂缓：Change 记录机制待替换，见 L1 规则】
...
8. Tools 辅助程序写入 ADR 时应校验字段完整性、状态合法性、引用有效性、写入授权和变更记录要求。【暂缓：Change 记录机制待替换，见 L1 规则】
...
12. ADR 创建、升级、推翻和废弃流程中的编号生成、ADR YAML 写入、状态流转写入、关联字段回写、推翻或废弃字段回写和 Change 记录生成【暂缓：Change 记录机制待替换，见 L1 规则】属于应评估的可机械化受控写入步骤。
...
3. ADR 写入策略是什么，包括哪些场景允许创建、接受、拒绝、升级、推翻、废弃、关联回写或记录 Change【暂缓：Change 记录机制待替换，见 L1 规则】，哪些写入必须触发 Human Gate，哪些写入由 Tools 承接，哪些写入需 AI 或人按规范完成。
...
6. 确认 ADR 写入入口，包括创建、接受、拒绝、升级、推翻、废弃、关联字段回写和 Change 记录生成【暂缓：Change 记录机制待替换，见 L1 规则】；
...
14. 确认 ADR Tools 是否已覆盖编号生成、ADR 创建、状态流转写入、关联字段回写、推翻或废弃字段回写和 Change 记录生成【暂缓：Change 记录机制待替换，见 L1 规则】等可机械化受控写入步骤；未覆盖时应记录为完整落地初始化阻塞项或暂缓项；
...
17. ADR Tools 是否覆盖编号生成、ADR 创建、状态流转写入、关联字段回写、推翻或废弃字段回写和 Change 记录生成【暂缓：Change 记录机制待替换，见 L1 规则】等可机械化受控写入步骤；未覆盖时是否已标记为阻塞项或暂缓项；
...
7. ADR 写入策略是否符合本文 §7、§9、§12和 §15，是否覆盖 Human Gate、受控写入、人工写入、Change 记录【暂缓：Change 记录机制待替换，见 L1 规则】和 Tools 降级边界；
