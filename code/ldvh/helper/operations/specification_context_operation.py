"""Connect specification-context composition to the Helper runtime."""
...
OPERATION_KEY = "read-specification-context"
_INPUT_CONTRACT = source_reference(
    "rule",
    "specification-model-foundation::9.8 规范上下文组合读取输入字段",
)
...
def _validated_request(request: CommonRequest) -> SpecificationContextRequest:
...
def _read(request: CommonRequest, repository: RepositoryInspection) -> SpecificationContextReadResult:
    domain_request = _validated_request(request)
    try:
        return read_specification_context(
            repository,
            request=domain_request,
            response_profile=request.response_profile,  # type: ignore[arg-type]
        )
    except SpecificationContextSelectionError as error:
...
def _summary(result: SpecificationContextReadResult) -> str:
    if result.suggested_outcome == "ok":
        return "已组合读取全部精确选择的规范上下文"
    if result.suggested_outcome == "partial":
        return "已组合读取部分规范上下文，并保留未完成范围"
...
def _call(
    ...
    result = _read(request, repository)
    verification = tuple(
        {**item, "evidence": [*item["evidence"], *_IMPLEMENTATION_EVIDENCE]}
...
SPECIFICATION_CONTEXT_IMPLEMENTATION = OperationImplementation(
...
__all__ = ["OPERATION_KEY", "SPECIFICATION_CONTEXT_IMPLEMENTATION"]
