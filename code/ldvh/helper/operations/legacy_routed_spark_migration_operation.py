...
from ldvh.facts.contracts import LEGACY_SPARK_IDS, LAYOUTS
...
from ldvh.helper.operation_runtime import AvailabilityEvaluation, OperationExecution, OperationExecutionContext, OperationImplementation
...
def _execute(request: CommonRequest, repository: RepositoryInspection, context: OperationExecutionContext) -> OperationExecution:
    reference, problems = _reference(request)
    requested = reference.to_json() if reference is not None else {"governed_project_id": "unknown", "fact_type_key": "spark", "object_id": "unknown"}
    if problems:
        return _failure(requested, "历史 Spark 迁移请求不符合契约", "；".join(problems), outcome="invalid_request")
    assert reference is not None
    scopes = explicit_scope([item for item in request.work_object_locators if isinstance(item, str)]) if request.work_object_locators else cwd_scope(str(context.cwd))
    ...
    if boundary_values is None:
        return _failure(requested, "当前管辖结果不能形成唯一历史 Spark 迁移边界", "管辖、worktree 或 common-dir 未形成唯一边界", outcome="unavailable")
    ...
    current = read_fact_object(boundary.worktree_root, LAYOUTS["spark"], schema, reference.object_id, expected_common_dir=boundary.git_common_dir)
    ...
    if not isinstance(current.fields.get("disposition_summary"), str) or not current.fields["disposition_summary"].strip():
        return _failure(requested, "历史 Spark 缺少可保留的处置依据", "disposition_summary 必须非空")
    after = {key: value for key, value in current.fields.items() if key not in {"object_id", "fact_type_key", "created_at", "updated_at", "priority"}}
    ...
    change_log = list(after.get("change_log", [])) if isinstance(after.get("change_log"), list) else []
    change_log.append({"at": context.event_at, "summary": "经逐条 Human 授权，历史 routed Spark 迁移为 implemented；旧 routed-to 已转为 related-to。", "signature": {}})
    after["change_log"] = change_log
    delegated = replace(request, arguments={"fact_ref": reference.to_json(), "expected_content_fingerprint": current.content_fingerprint, "fact_object": after})
    return execute_fact_update(delegated, repository, context, allow_legacy_routed_spark_migration=True)


def _availability(request: CommonRequest, repository: RepositoryInspection, context: OperationExecutionContext) -> AvailabilityEvaluation:
    reference, problems = _reference(request)
    if reference is None or problems:
        return AvailabilityEvaluation(availability="unavailable_for_request", unavailable_scope=(() if reference is None else (reference.to_json(),)), gaps=())
