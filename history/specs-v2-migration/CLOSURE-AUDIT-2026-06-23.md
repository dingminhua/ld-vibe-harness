...
| `specs/README.md` | 当前仓库不存在 `specs/README.md`；多个规范保障要求曾把它列为入口可见机制之一 | 已确认不创建 `specs/README.md`，改为把入口机制统一收敛到 01、Rules 入口、`v2-check` 和知识地图输入 |
...
| 等级 | 来源判断 | 归属 | 处理方式 |
|---|---|---|---|
| A | `specs/README.md` 入口资产缺失可能削弱 V1 | 01 / 06 / 运行时入口 | 已确认不创建 specs 内 README；相关保障机制应统一改为 01、Rules 入口、`v2-check` 和知识地图输入 |
| A | Code 依赖与本地可运行性影响 V6 | 04 / 08 / README / Code docs | 已确认 `pyproject.toml` 声明 `PyYAML`；根 README 应补充依赖安装、验证入口和工具不可用降级说明，不改变规范技术栈边界 |
...
当前 `specs/01-规范体系基础规范.md` 以及 20-24 成员规范的保障要求中，曾多处把 `specs/README.md` 作为入口可见机制之一。仓库实际不存在该文件。

已确认的处理口径如下：

1. 不创建 `specs/README.md`；
2. specs 内不得再把 README 作为规范入口事实源；
3. 入口可见机制收敛为 01 当前目录登记、Rules 入口、`python3 code/specs_validate.py v2-check --format text` 和知识地图输入；
4. 项目根目录 `README.md` 可以作为项目入口说明存在，但不替代 active `specs/`、Rules 入口或 Code 诊断。

完成相关保障要求引用修正后，本项可视为收口完成。

### 4.2 Code 运行依赖入口说明

当前 `pyproject.toml` 已声明 `PyYAML`，当前本地 `python3` 可 import `yaml`。因此外部评估中的“系统 python 缺 yaml”不是当前仓库内已复现缺陷。

已确认根 README 应说明以下内容，以帮助 AI 在新环境中完成 V6 强制验证：
...
| 顺序 | 动作 | 输出 |
|---|---|---|
| 1 | 核对并处理 `specs/README.md` 入口口径 | 不创建 specs 内 README；修正保障要求引用 |
| 2 | 核对 README / Code docs 的依赖安装与验证入口说明 | 根 README 更新；后续如 Code 命令增加再同步 `code/docs/` |
