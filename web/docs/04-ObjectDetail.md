...
WorkCase 详情页用于完整理解同一项当前工作责任，不复刻外部 Card。身份头部沿用列表 Card 由 `status / phase` 确定性派生的 `plan_confirmation / progressing / closure_confirmation / closed` 进展分组，使进入详情前后保持同一浏览分类；精确 phase 保留在事实数据和确定性投影中，不在“当前情况”内重复生成“当前阶段”色块。详情页不得根据这些进展分组或推进环节切换、隐藏、重排字段，也不得为不同状态维护四套阅读结构。`human_plan_confirming`、`plan_revising`、`executing`、`controller_checking`、`independent_reviewing`、`closure_preparing`、`human_closure_confirming` 与 `closed` 全部复用同一阅读顺序。条件或可选字段不存在时不渲染相应节点；类型来源必填字段缺失或类型不符时，API 保留字段问题，详情在对应区段显示空态，不升级为 `invalid` 或整个对象读取失败。
...
3. **现在实际处在哪里**：读取精确 `phase` 形成头部进展分组，并在“当前情况”中呈现当前 `summary`，以及实际存在的 `waiting_on`、顶层 `blocking_summary` 和 `resume_from`。当前情况不重复显示当前阶段色块；页面不得从 phase 自动补写等待、阻塞或下一步。
...
详情按两层渐进式披露：上述 Human 语义阅读节点和正式关系默认展开，保持与 Card 相同的图标、标题、正文、语义色和状态词；详情只增加完整信息，不为同一事实建立另一套视觉含义。“目标与边界”节点内的范围正文是阅读密度例外：目标默认展开，范围默认收起，Human 点击范围标题行后展开或再次收起。YAML 数据、未解析结构和字段问题汇总属于来源数据与技术诊断，默认折叠并由 Human 主动打开。必填字段缺失或类型不符仍必须在对应 Human 阅读节点就地显示，不能因为诊断汇总折叠而消失。这里的折叠只控制 Web 阅读密度，不是 AI 信息筛选；AI 仍经事实源与 Helper 取得所需信息，页面折叠状态不影响 API 或来源可用性。

成功标准在 Card 与详情中使用同一个“单一蓝色语义区＋圆点并列条目”语法，不把每条标准改成独立 Card，也不在 Human 阅读层重复展示 `criterion_id`。两处必须共用同一组件、正文色阶、`4px` 圆点、首行中线对齐、`6px` 条目间距、`10px` 圆点至正文间距和 `12px × 10px` 容器内边距；Card 使用紧凑正文层级（`12px / 20px`），详情与目标、范围共用内页正文基准（`13px / 22px`），不得分别维护近似样式。详情可以在对应陈述下增加当前结果状态与结果摘要，但附加结果不能改变标准陈述的主层级；稳定 ID 仍保留在来源数据中供精确对照。
...
“当前情况”除去重复的当前阶段行后，其摘要、恢复入口、等待对象与阻塞说明同样使用标题 `14px / 22px`、正文 `13px / 22px`，并以对应背景的同色系表达。内页语义块标题图标使用 `14px`；WorkCase Card 的 `13px` 语义标题配套使用 `14px` 图标，不得继续沿用抢占标题权重的 `16px` 图标。

“当前计划与工作项”默认呈现工作项身份、状态与工作内容；工作项主内容使用与工作项青色表面协调的青灰正文色，不回退为通用近黑正文。预期结果使用蓝色浅背景段落，方法边界使用灰色背景、边框与文字构成的完整中性色段落，两者正文默认收起并由 Human 点击标题行展开。工作项结果仅在来源字段存在时显示，使用绿色语义段落并默认展开。由于三类段落位于工作项框内，标题使用 Card 层级 `13px / 20px`，展开正文使用 Card 正文层级 `12px / 20px`；预期结果与方法边界标题不放圆点或语义图标，只保留标题文字与展开箭头；工作项结果仍以 `4px` 绿色圆点标识结果语义。方法边界整体降为灰色信息层级，进一步区别于蓝色预期结果和绿色工作项结果；不得只改变文字而保留其它色系，也不得把预期结果与方法边界合并为同色、同一段或默认同时展开。
