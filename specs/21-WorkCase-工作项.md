...
ldvh_spec:
  ...
  relation: "refines"
  positioning: "定义 WorkCase 事实类型的对象边界、Schema、阶段性工作项、生命周期、工作意图确认、审核、对象建立后的执行与关闭双 Human Gate、来源、关系、验证与关闭规则"
...
WorkCase 把一个已经由 Human 确认需要推进并作为项目记录管理、且已经形成明确目标、范围和成功标准的长时工作责任，保留为可跨行动、会话和执行者恢复，可分阶段交付、验证、阻塞与关闭的当前事实。它使后续 AI 不必依赖聊天记忆重建“为什么项目决定承担这项工作、要完成什么、分成哪些阶段结果、当前进展如何、什么阻止继续、经过哪些质量关口、依据什么关闭”。
...
4. 需要跨行动或会话恢复，或具有显著验证、授权、依赖、阻塞或独立关闭事实；
...
2. Human 未确认工作意图却进入正式计划、Subagent 创建审核或对象创建；新建对象不是 human_plan_confirming、没有 creation review、带伪 execution approval，或各状态/阶段缺少条件字段、带禁止字段、空值和未知字段；
...
5. creation review 或 result review 没有实际独立审核者、范围、反馈和主控处置，或者阻断反馈尚未解决；
