...
安装向导的关键表格必须使用稳定图标帮助 Human 快速扫描。路径确认表项目列使用 `📦 LDVH 本体路径`、`🗂️ 目标工作区根目录`、`🧾 配置文件完整路径`；安装前检查项目列使用 `📦 LDVH 本体`、`🧾 工作区配置`、`⚠️ 配置层级`、`📁 事实源目录`、`🔎 配置解析`、`🔌 环境入口`；安装前检查结果列必须使用 `✅ 通过`、`⚠️ 注意`、`⛔ 阻断`、`➖ 不适用`；安装方案预览项目列使用 `🧾 工作区配置`、`🗑️ 项目内旧配置`、`📁 事实源目录`、`🔌 环境插件 / 外部 Hook`、`🧪 验证`、`↩️ 回滚`。图标只用于提高可读性，不得替代状态文字、路径、限制、Human Gate 或验证结论。
