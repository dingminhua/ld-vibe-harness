...
本文不定义正式规范规则。目录、编号、附件、身份块、Human Gate、规范写作执行顺序和知识地图输入等约束，应由 `01-规范体系与治理规范.md` 及其授权附件承接。
...
| `01-规范体系与治理规范.md` | v2 规范体系、规范身份、必要章节、上位依据、构成要素归属与价值判断、权威归口、引用纪律、附件规则和迁移治理边界 | draft，未 active |

当前 01 授权附件如下。附件随父规范核对，不作为独立根规范：

| 路径 | 定位 | 状态 |
|---|---|---|
| `01.Att.01-知识地图关系类型表.md` | 承载 01 授权的知识地图关系类型、方向、来源结构、禁止误用和 Code 消费类别提示 | draft，未 active |
| `01.Att.02-术语表与表达清单.md` | 承载 01 授权的术语条目、推荐表达、不推荐表达、替代写法、中文定语规则和术语与表达辅助核对表 | draft，未 active |
| `01.Att.03-规范保障要求与行动编排接管表.md` | 承载 01 授权的保障要求类型闭集、声明辅助表、行动编排接管关系和建议行动编排实例矩阵 | draft，未 active |
...
以下内容应写入或补强到 `01-规范体系与治理规范.md`，而不是只停留在 README：
