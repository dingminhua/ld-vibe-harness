...
> 创建日期：2026-06-11
> 定位：吸收 ECC、Trae Work CN、Codex App、Claude Code CLI 中与 LDVH 环境适配相关的技术经验，重点讨论环境薄入口、人机沟通、Rules / Skills / Hooks / Agents / Commands 的承接方式
> 性质：内部研究文档，不直接构成正式规范、ADR、Task 或运行适配措施
> 当前结论：环境适配应定义为 LDVH 的主动行为。正式规范先提出落地要求，LDVH 建设或选择能力保障这些要求，再基于目标环境的客观能力清单主动形成适配措施、降级路径和缺口记录；环境不拥有 LDVH 能力，也不反向决定 LDVH 是否建设更优能力。

---

## 1. 本文解决的问题

本文用于补齐 LDVH 环境适配研究中的一个缺口：此前讨论较多集中在 04.03 环境能力清单、Skill + Hook 跨环境不等价、Code baseline 和 42 bootstrap，但对人机沟通机制、Rules、Skills、Hooks、Agents、Commands 等环境适配承接面的整体吸收还不够系统。

本文回答以下问题：

1. LDVH 从 ECC 和三类 AI 开发环境中，应吸收哪些环境适配经验；
2. 哪些能力属于 LDVH 共享内核，哪些只能成为环境薄入口；
3. 人机沟通机制在环境适配中应如何表达；
4. Rules / Instructions、Skills、Hooks、Agents、Commands 如何承接 LDVH 要求；
5. Trae Work CN、Codex App、Claude Code CLI 的能力差异如何映射到 04.04；
6. 后续 04.02、04.03、04.04、42 和 Code 校验应吸收哪些结论。
...
环境适配应被理解为 LDVH 的主动行为，而不是环境对 LDVH 的被动承接。
...
读取目标环境客观能力清单
  ↓
LDVH 主动形成该环境中的运行适配措施、降级路径和缺口记录
```
...
| 环境适配层 | LDVH 基于目标环境的客观能力清单，将能力主动形成环境入口、命令、Skill、Hook、Agent、Web、Code 校验、Human Gate 表达、降级路径或缺口记录 | `docs/specs/04.02`、`docs/specs/04.03`、适配措施 |
...
ECC 对 LDVH 的价值不是其完整安装器、资产库或自动 repair 系统，而是它已经实践过多环境能力适配的工程组织方法。

### 3.1 可吸收方法

| ECC 经验 | LDVH 可吸收方式 | 环境适配意义 |
|---|---|---|
| manifest 三层模型 | 转译为能力声明、适配对象、验证命令和事实源边界字段 | 让 AI 知道某个能力由哪些文件、入口和检查组成 |
| target / adapter | 转译为环境能力清单列和环境薄入口 | 不同环境能力不同，但映射回同一 LDVH 要求 |
| compliance matrix | 转译为适配类型、适配检查、适配降级和待补齐事项 | 防止口头支持和虚假适配 |
...
| rules / skills 分工 | 转译为 specs / rules / skills / tools 分工 | specs 是权威 what，skills/tools 是 how，入口只是薄入口 |
...
人机沟通是环境适配的核心内容之一。环境薄入口不仅要让 AI 看见规范，还要让 AI 知道何时问 Human、问什么、如何暂停、如何记录授权、如何把结果回写到事实源。
...
2. 环境 UI 审批不能自动等价为 LDVH Human Gate；
3. 问题澄清应优先使用当前环境最自然的人机入口，但稳定结论必须回写事实源；
4. STOP 点应出现在入口适配、Skill checklist、Command contract 和 42 报告中；
5. 长期授权、长期降级、持久适配措施、自动触发和事实源写入必须显式触发 Human Gate。
...
在 04.04 中，Rules / Instructions 卡片应记录：
...
当一个环境没有 Hook，但另一个环境有 Skill + Hook 组合时，不得声明等价适配，也不得因为弱环境缺少 Hook 而取消 LDVH 的 Hook / lifecycle 能力建设。
...
在 04.04 中，这种情况应标为“指令适配”或“人工降级”；只有存在明确 thin entry、Command / Code wrapper 和验证证据时，最多标为“已适配”。强环境中的 Hook 路径应保留为优先适配方式，弱环境只在自身适配路径中降级并记录残留风险。
...
5. 04.04 应记录 MCP 是否存在、承接类型、来源、检查方式、降级方式和待补齐事项。

---

## 10. Context / Memory / Session

上下文、记忆和会话恢复也是环境适配的一部分，但必须避免成为隐藏事实源。
...
因此 04.04 中不应把 Web 运行态作为环境能力项，但可以记录：
...
后续 04.03 中，每个适配措施项建议使用统一卡片结构：

| 字段 | 含义 |
|---|---|
| 适配项 | 例如 Rules / Instructions、Skill / Command、Hook / Lifecycle、Agent / Subagent、Human Gate |
| LDVH 要求 | 该能力要满足的 LDVH 抽象要求 |
| 环境能力清单 | Trae Work CN、Codex App、Claude Code CLI 的客观能力、边界和证据状态 |
| Trae Work CN 适配方式 | LDVH 在 Trae 中主动形成的入口、流程、校验、降级或缺口记录 |
| Codex App 适配方式 | LDVH 在 Codex 中主动形成的入口、流程、校验、降级或缺口记录 |
| Claude Code CLI 适配方式 | LDVH 在 Claude Code 中主动形成的入口、流程、校验、降级或缺口记录 |
| 来源 | refs、specs、research 或实测证据 |
| 适配检查 | 如何证明适配未越界、未虚假支持、未把环境能力当成 LDVH 决策 |
| 适配降级 | 能力不足时如何人工降级或显式执行 |
...
按“规范提出落地要求、LDVH 建设或选择能力保障、LDVH 基于环境能力清单主动适配”的主轴，04 系列可收敛为：
...
| 04.03 | 定义环境客观能力清单，以及 LDVH 如何基于清单分别形成适配方式、检查、降级和缺口 |
...
6. 04.04 / 04.05 的迁移、历史化或删除需要与 04.03 新矩阵和 42 消费口径同步处理。
...
2. 再改 04 父文档，确立“规范落地与环境适配基础规范”的总模型；
3. 再改 04.01，收敛规范落地要求与类型；
4. 再改 04.02，整理 LDVH 能力保障和能力建设需求；
5. 再改 04.03，整理环境能力清单与适配矩阵；
6. 最后处理 04.04、04.05、04.04、04.05、42 和 Code 校验。
...
| 内容 | 建议吸收位置 |
|---|---|
| 环境适配三层模型 | 04 父规范、04.01、04.04、04.02、04.03 |
| 环境能力清单不是落地决策 | 04.02、04.03 |
| 环境能力不足时优先判断 LDVH 能力建设 | 04.03、04.04 |
| 人机沟通机制作为环境适配面 | 04.02、04.03、04.04、42 |
| Rules / Instructions 承接边界 | 04.03、04.04 |
| Skill / Command 承接边界 | 04.03、04.04、11.01 |
| Hook 无等价时的降级规则 | 04.03、04.04 |
| Agent 输出必须回主控 | 04.03、44、04.04 |
| MCP 输出不是事实源 | 04.03、04.04、09 |
| Context / Memory / Session 非事实源边界 | 04.02、09、12 |
| Web / Preview 作为 Human-facing 降级说明 | 04.04、08、42 |

### 14.2 应进入 Code 校验的内容

后续 Code 可检查：

1. 04.04 是否存在环境适配卡片必填字段；
2. 承接类型是否使用合法枚举；
...
8. 04.04 / 04.05 是否被 42 继续当作长期独立清单。
...
```text
LDVH 环境适配的重点不是复制环境能力，也不是等待环境被动承接，而是由 LDVH 基于规范落地要求和自身能力体系，读取目标环境的客观能力清单，主动形成不同环境中的入口、触发方式、人机沟通、显式校验、降级路径和缺口记录。
...
所有环境适配都必须回到 04.03 的环境能力清单与适配矩阵表达目标环境客观能力、适配方式、来源、适配检查、适配降级和待补齐事项。

运行适配措施、环境薄入口、环境工具输出和人机沟通界面都不是事实源；稳定结论必须回写 Git 文件事实源。
```
