...
初始状态只能是 active。正常转换只有 `active → retired`；终态不直接重开。根因判断、解决方式、规避方式或 applicability 的实质改变通常建立新 Pitfall；只有仍是同一失败机制的事实更正、症状与触发补强、验证说明更新可以原地修正。
...
| 退出 | 准备 retired 时 | 退出依据成立，适用边界、验证说明和时间一致 | 当前 Pitfall 与 Human 决定 | AI 语义审核和结构校验 | 当次终态声明 | 保持 active；补充退出说明或进入 Human Gate |
