...
需要 Human 判断的步骤优先使用用户验收卡片，而不是把 raw diagnostic 直接交给用户：

| 卡片项 | 写法 |
|---|---|
| 你需要确认什么 | 一句话说明当前要用户看路径、选编号、看插件页面或批准执行 |
| 为什么需要你确认 | 只说明风险或影响范围，不解释内部状态机 |
| 选择后会发生什么 | 写清会进入下一步、会写入、会停止或会转入 31 |
| 看到什么算正常 | 给出可观察标准，例如启用、无错误、验证通过 |
| 失败时把什么发给 AI | 路径、截图、错误文本、命令输出或 scratch 文件状态 |
...
```text
请进入 31 环境 Hook 接入后验收，我授权按测试组逐项检查插件页面、重启后状态、新对话触发、受控负例阻断、受控正例放行和统一安装验证。
```
