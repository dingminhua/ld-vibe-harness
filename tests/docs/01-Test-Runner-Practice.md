...
用户视角表达也属于稳定交互契约。新增或修改 30/31 时，应测试 `你需要确认什么`、`为什么需要你确认`、`选择后会发生什么`、`看到什么算正常`、`失败时把什么发给 AI`、`用户要做什么`、`正常表现`、`失败时给 AI 什么`、状态牌、用户下一步待办、失败信息包、raw diagnostic 不裸露在主界面、no-Hook 分支显示为“手动可用 / 可用但不自动拦截”、支持 Hook 但未 lifecycle 验收显示为“自动接入待验收”、环境自动拦截与提交消息检查分块展示、受控正反例写明 scratch 文件动作。负例应覆盖删除这些用户可理解判断标准、把 `unsupported_target_environment` 或 `environment_hook_integrated=false` 直接作为用户主选项、把两类 Hook 混成一个“Hook 已通过”、以及重新把 no-Hook 写成降级或不安装。
