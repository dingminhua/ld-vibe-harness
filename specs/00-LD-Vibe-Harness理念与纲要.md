...
| 二 | 第一服务对象 |
| 三 | 价值实现标准（§3.1 总体评价、§3.2 五类要素与价值、§3.3 反向判断） |
| 四 | 架构：五类构成要素（§4.1 总览、§4.2 介质、§4.3 Trae Solo 环境机制、§4.4 工具、§4.5 事实模型、§4.6 行动模型） |
| 五 | 五类构成要素的运行闭环（§5.1 事实源闭环、§5.2 AI 行动闭环、§5.3 人类确认闭环、§5.4 工具辅助闭环） |
| 六 | 基础规范索引 |
| 七 | 机制关系声明 |
| 八 | Human Gate 与检查要求 |
| 九 | 待补齐事项 |
...
LD Vibe Harness 的事实源必须回到 Git 可追踪文件。事实源权威性判断、单一事实源原则、承载介质选择和工具输出回写规则，由规划中的 `specs/10-事实源边界与承载规范.md` 定义；本文只保留事实源闭环的顶层价值要求。
