...
def test_rule_binder_projects_working_tree_identity(current_specs_repository: Path) -> None:
    repository = inspect_repository(current_specs_repository)
    reference = generated_source_reference("rule", "specification-model-foundation::6.4 源码身份与失败边界")
    ...
    assert working["details"]["git_worktree_root"] == current_specs_repository.as_posix()
...
def test_generated_references_are_omitted_when_source_identity_is_unavailable() -> None:
    generated = generated_source_reference("rule", "specification-model-foundation::6.4 源码身份与失败边界")
