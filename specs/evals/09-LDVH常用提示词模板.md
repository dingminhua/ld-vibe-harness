...
当需要审查 LDVH 规范体系和项目实践是否真正落实 `specs/00-LD-Vibe-Harness理念与纲要.md` 提出的 V1-V7 核心价值时使用。该模板从设计和实践两个层面进行全面审查，并对"精准获得上下文"（V1 稳定理解）做专项深审。
...
3. **上下文加载机制**：Rules 入口是否引导 AI 按场景加载必要上下文？L0 → L1 → L2 的读取链路是否完整？是否存在跳过读取直接执行的情况？
...
7. **反向判断验证**：对照 `specs/00-LD-Vibe-Harness理念与纲要.md` §3.3 反向判断第1条——AI 是否仍需依赖聊天记忆才能继续项目？如果是，说明精准获得上下文未落实。
