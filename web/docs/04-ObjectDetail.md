...
- ID 使用 `ldvh-meta`，不做大号标题；右上角只提供 `CopyPathButton`，复制对象详情 API 返回的 `target`，不得再放状态标签或其他对象身份信息。
...
上述四类方案作为后续非工作主线对象页面设计的参照，不再回退到普通字段卡片堆叠。提交详情不是工作对象详情，但它已经进入同一阅读语言族：标准身份头部、指标区、圆点正文节点和右侧扩展阅读都必须与这四类详情保持一致。
...
- WorkArea、WorkPlan、Pitfall、ADR、Study 和 Memo 的扩展阅读头部同样不显示状态 chip；右侧面板按详情页身份区顺序展示 `类型 + ID + 标题 + 创建/更新时间 + 复制路径入口`，状态由复用的语义阅读布局表达。
