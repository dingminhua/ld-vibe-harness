...
function item(overrides: Record<string, unknown> = {}) {
  ...
  return {
    ...
    absolute_path: '/workspace/project/facts/sparks/spark-0001.yaml',
...
test('open Spark maps exact legacy fields without dropping V4 truth', () => {
  const projected = projectV4Spark(item())

  assert.equal(projected.ok, true)
  if (!projected.ok) return
  assert.equal(projected.data.id, 'spark-0001')
  assert.equal(projected.data.type, 'spark')
  assert.equal(projected.data.status, 'pending')
  assert.equal(projected.data.created, '2026-07-15T16:00:00+08:00')
  assert.equal(projected.data.updated, '2026-07-15T16:00:00+08:00')
  assert.equal(projected.data.description, 'Preserve V4 truth while adapting the existing DTO.')
  assert.equal(projected.data.path, '/workspace/project/facts/sparks/spark-0001.yaml')
...
test('same-project related-to relations add navigation aliases and preserve originals', () => {
  const relations = [
    {
  ...
  assert.equal(projected.ok, true)
  if (!projected.ok) return
  assert.deepEqual(projected.data.related_workcases, ['workcase-0002'])
  assert.equal(projected.data.related_adrs, undefined)
  assert.deepEqual(projected.data.relations, relations)
})

test('one same-project routed target maps to resolved legacy shape', () => {
  const relation = {
  ...
  assert.equal(projected.ok, true)
  if (!projected.ok) return
  assert.equal(projected.data.status, 'resolved')
  assert.deepEqual(projected.data.resolved_to, { type: 'workcase', ref: 'workcase-0002' })
  assert.equal(projected.data.resolved_at, '2026-07-15T17:00:00+08:00')
...
test('minimal routed and discarded Sparks preserve only present terminal truth', () => {
  ...
  if (discarded.ok) {
    assert.equal(discarded.data.discard_reason, 'No longer relevant.')
...
test('zero, multiple, cross-project, and malformed routed targets fail closed', () => {
  ...
  for (const relations of cases) {
    ...
    assert.equal(projected.code, 'spark_projection_unrepresentable')
    assert.deepEqual(projected.targets, relations.map((relation) => relation.target))
  }
  const malformed = projectV4Spark(item({
    status: 'routed',
    relations: [{ relation_key: 'routed-to' }],
    closed_at: '2026-07-15T17:00:00+08:00',
    disposition_summary: 'Malformed relation cannot be represented.',
  }))
  assert.equal(malformed.ok, false)
  if (!malformed.ok) assert.equal(malformed.code, 'spark_projection_invalid')
})
...
test('discarded status maps disposition summary and list remains partial on any projection loss', () => {
