...
ACTION_TEMPLATE_FOUNDATION_RULE_REQUIREMENTS = [
    {
        "code": "ACTION_TEMPLATE_REFERENCE_COPY_BOUNDARY_MISSING",
        "section": "行动模板结构",
        "message": "06 必须声明行动模板引用与复制的判断边界",
        "terms": [
            "引用与复制的判断标准",
    ...
    {
        "code": "ACTION_TEMPLATE_GRANULARITY_RULE_MISSING",
        "section": "行动模板结构",
        "message": "06 必须声明行动模板七结构粒度要求",
        "terms": [
            "七结构粒度要求",
            "| Context |",
            "| Scenario |",
    ...
    {
        "code": "ACTION_TEMPLATE_CONCRETE_EXAMPLE_BOUNDARY_MISSING",
        "section": "行动模板结构",
        "message": "06 必须声明父层规范不承载具体模板示范",
        "terms": [
            "正式行动模板成员的七结构内容以各自 spec 正文为准",
            "本文不承载具体模板示范",
        ],
    },
]
GIT_COMMIT_ACTION_TEMPLATE_REQUIRED_ROWS = {
    ...
    "执行": ["Git 工作区摘要", "必要 diff", "只 stage 本次范围内文件", "拆分", "03.Att.01", "提交说明", "必要 body 小标题", "匹配验证", "commit validator", "不安装 Hook"],
...
LDVH_INSTALL_ACTION_TEMPLATE_REQUIRED_ROWS = {
    "Context": ["用户目标", "目标环境", "LDVH 本体路径", "目标工作区根目录", "管辖项目候选", "目标工作区配置", "事实源目录状态", "环境入口审计结果", "Git Hook 状态", "source_refs", "Runtime Protocol", "安装向导实践文档"],
    "Scenario": ["安装 LDVH", "接入 LDVH", "初始化 LDVH", "配置管辖项目", "旧插件 / 旧路径", "只回答 01/06/10 边界"],
    "Gate": ["Human Gate", "环境入口", "插件 / 扩展包", "管辖项目 Git Hook", "管辖项目配置", "事实源目录", "LDVH 本体路径", "目标工作区根目录", "配置层级冲突", "integrated", "多项目", "用户告知清单", "安装方案预览", "最终确认"],
    "执行": ["bootstrap discovery", "有限、只读、有证据", "五阶段", "路径确认", "安装前检查", "安装选项", "安装方案预览", "最终确认", "环境入口按 01", "配置和事实源按 10", "验证按 09", "具体命令、输出字段和实现入口"],
    "验证": ["当前回合安装检测", "断点后 lifecycle 验证", "验证入口", "不可验证范围", "09.Att.01", "具体验证命令", "输出字段", "正反样例", "通过标准"],
    "回写": ["过程输出", "Human 确认", "管辖项目配置事实源", "10", "事实源目录创建", "不创建事实实例", "旧插件", "环境适配缺口", "不得把 runtime receipt"],
    "交还": ["安装方式", "路径和配置确认结果", "管辖项目选择", "环境接入判定来源", "用户告知清单", "实际写入", "未写入对象", "验证摘要", "回滚或卸载入口", "下一步 Human Gate", "source_refs", "断点恢复入口语", "可复制只读可见性探针", "真实工作流验证清单", "失败信息包", "阻断原因"],
}
...
LDVH_INSTALL_ACTION_TEMPLATE_BOUNDARY_TERMS = [
    ...
    "历史行动模板",
    "10 归口安装与配置的机器契约",
    "Code facade 归口确定性 `check -> plan -> apply -> verify` 执行壳",
    "CLI 不替代本文",
...
IMPLEMENTATION_DOMAIN_BOUNDARY_REQUIREMENTS = [
    ...
    {
        "code": "SPECS_STATE_OWNERSHIP_BOUNDARY_MISSING",
        "path": SHORT_SPEC_REFS["04"],
        "section": "状态归口原则",
        "message": "04 必须声明 V3 可记录状态只归 specs/attachments 生命周期和事实对象业务状态",
        "terms": [
            "只有两类对象可以拥有可记录、可迁移、可校验的状态",
            "specs / attachments 自身的生命周期状态",
            "事实源对象的业务状态",
            "不得形成独立状态闭集",
            "不得被 specs 当作事实状态消费",
            "不是可记录状态",
...
FOUNDATION_SPEC_CONTRACTS = {
    ...
    "06": {
        ...
        "required_verification_rows": ["结构检查", "来源检查", "粒度检查", "能力边界检查", "验证检查"],
...
def validate_action_template_foundation_rules(root: Path = ROOT) -> list[Diagnostic]:
    path = SHORT_SPEC_REFS["06"]
    raw = (root / path).read_text(encoding="utf-8")
    sections = h2_sections(raw)
    body = sections.get("行动模板结构", {}).get("body", "")
    diagnostics: list[Diagnostic] = []

    for requirement in ACTION_TEMPLATE_FOUNDATION_RULE_REQUIREMENTS:
        missing_terms = [term for term in requirement["terms"] if term not in body]
