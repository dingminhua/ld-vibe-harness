...
1. WorkCase 规范和 `21.Att.01` 已迁入主要状态机、执行编排、方案审核、结果自检、结果复核、关闭确认、修订记录和 orchestration 长字段规则，并已作为 active WorkCase 成员规范生效；
2. 历史 `ldvh_member` 与 active `v2_fact_model_member` 的双读 Code 实现、正反样例和历史追溯策略尚未完成；本文不改变 Code 默认消费入口；
3. WorkCase Web 派生态势、状态标签和关闭/提交口径应在 Web 规范和实现迁移时复核；
4. WorkCase 创建、方案审核、结果复核和关闭确认的具体行动编排不按 v1 直接迁入；应按当前 active 规范保障需求进入行动编排候选计划；
