...
| Pitfall 字段内容格式 | `specs/05.02-字段内容与格式规范.md` |
...
13. `verification` 必须按 `specs/05.02-字段内容与格式规范.md` §3.3 的四段式验证证据结构书写，不得只写“已验证”“通过”或把验证结果混入 `resolution`。
...
Pitfall 详情页是可复用经验阅读页，不按普通字段卡片堆叠。主节点固定为“现象、触发、根因、方案、验证、规避、范围、关联”，节点标题栏整行可点击，默认全部打开，折叠图标规则与 Study 一致。`verification` 节点消费 `specs/05.02-字段内容与格式规范.md` 的四段式验证结构，并按“验证计划、验证命令、验证结果、结论”顺序轻量分段展示，不使用表格左列重复标签。
...
Pitfall 识别、创建、归档和吸收到规范或实现的具体行动流程由后续 30-59 行动编排规范承接。本文只定义 Pitfall 实例的事实规则和状态约束。
...
2. Pitfall 识别、创建、归档和吸收的具体行动编排待 30-59 承接；
3. Pitfall 与行动编排中 Learn 阶段的关系，待 30-59 稳定后进一步校准。
