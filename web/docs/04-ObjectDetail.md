...
内容区：
  WorkCase：当前快照 + 目标与边界 + 成功标准 + 工作项 + 当前复核、处置与 Human 批准 + 验证与关闭 + 关系
...
当前 `WorkCaseReadingLayout` 已直接按 21 单一当前字段族实现以下固定节点顺序：当前情况 → 目标与边界 → 成功标准 → 当前计划与工作项 → 独立方案复核 → 执行批准 → 结果与验证 → 主控自检 → 独立结果复核 → 关闭提案 → 终态处置 → 正式关系 → 外部网址。节点只按对应字段实际存在显示；类型来源必填字段缺失或字段类型不符时，对应节点仍然保留，并在该字段位置就地标明字段问题（与其它事实类型共用同一套 `fieldIssue` / `FieldProblem` 组件），不升级为整对象读取失败，也不只依赖页底汇总。叙述性字段（goal、scope、summary、result_summary、validation_summary、controller_check_summary、disposition_summary、proposed_disposition_summary 等）与其它事实对象保持一致阅读质感：字段名以小字题注置于 Markdown 正文上方，不使用标签列，正文完整渲染不截断；单字段节点（如主控自检）省略与节点标题重复的题注，呈现为纯散文节点。工作项、复核、批准、成功标准条目和处置条目等结构化记录保留标签行，作为字段定位辅助。关闭提案中的拟路由目标按关联区同一规则处理：同项目目标按需解析当前目标标题并可进入右侧扩展阅读，精确读取后提供对象路径复制；项目身份与内容 fingerprint 降权保留为次级定位信息；跨项目或身份不完整的目标只如实显示已知稳定身份，不以 object_id 冒充名称。成功标准定义与三值结果按 `criterion_id` 对照；Reviewer、Controller、Human 执行批准、Controller 关闭提案和终态处置保持独立节点；关系显示正式 `relation_key`。详情与右侧扩展阅读复用同一组件和同一投影，不存在历史字段、列表回退或状态分支。
