...
本文件不声明任何环境已经具备原生 Hook 完整支持，不启用自动写入，不替代 Code、Human Gate、事实源边界或主控 AI 判断。
...
| 承载方式 | 含义 | 限制 |
|---|---|---|
| 原生 Hook | 环境提供生命周期触发能力 | 必须有可查询配置和证据，不能只凭描述声明支持 |
| Skill 模拟 | 由 Skill 在流程中提醒或执行检查 | 只能称为模拟或指令适配，不能声明原生 Hook 完整支持 |
