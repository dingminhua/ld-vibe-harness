...
ldvh_spec:
  ...
  scope: "管辖项目中已经完成、以外部产品、实践、标准、论文、工具或方案为对象，或以项目内部设计问题为对象，且可独立引用的调研报告"
  basis:
...
Study 保存一轮已经完成、可独立引用且具有跨行动稳定阅读价值的**外部产品/实践调研或项目内部设计研究**。其研究对象必须是外部产品、实践、标准、论文、工具、方案，或项目中需要独立研究的内部设计问题；它使后续 AI 能快速恢复研究对象、输入边界、关键发现、建议和后续分流，从而避免重复调研，并把研究启发转化为项目下一步。
