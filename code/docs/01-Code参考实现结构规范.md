...
3. 再提取大型聚合报告，例如 `ldvh_landing`、`web_validate`；
...
1. `runtime_projection`、`governed_projects`、`human_gate`、`doc_structure`、`refs`、`landing` 基础检查和 `landing_report` 已迁入 `code/spec_checks/`，后续迁移应沿用兼容包装、聚焦测试和综合校验的做法；
