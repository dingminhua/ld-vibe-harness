...
ADR 的价值，是把长期决策、方案取舍、事实源归属、协作方式或执行约束沉淀为可追踪事实对象，减少 AI 在后续执行时反复重建决策背景和取舍依据的负担。

ADR 主要服务 V2、V3、V4、V7 和 V9：它帮助 AI 充分理解已确认取舍，正确判断决策边界、影响和风险，稳定沿用已确认约束，按事实对象格式回写决策依据和影响，并在决策需要成为长期规则时推动吸收到对应 specs、行动模板、Code、Web 或保障与衔接层。ADR 是决策事实对象，不是规范正文。
...
4. 当前 V3 ADR 字段契约、影响闭环和实例样例的后续补充判断；该判断不得以本地 V2 成员文件作为默认来源。
...
1. 后续如需补充 ADR 完整字段表、五段式影响闭环和实例样例，必须作为 V3-owned 字段契约、实例校验或明确 WorkCase 推进；
2. ADR 实例目录已建立为 `ldvh-base/adrs/`，后续新增 ADR 必须通过字段 schema、状态闭集和 legacy 字段检查；
