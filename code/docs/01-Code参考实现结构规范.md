...
1. `runtime_projection`、`deployment_entries`、`consistency`、`governed_projects`、`human_gate`、`doc_structure`、`refs`、`landing` 基础检查、`landing_report`、`ldvh_landing` 和 `web_validate` 已迁入 `code/spec_checks/`，后续迁移应沿用兼容包装、聚焦测试和综合校验的做法；
