...
| 诊断 | 报告缺口、冲突、漂移、原因和建议回写方向 | 不直接关闭问题 |
...
Code 需求记录字段表由 `attachments/04.Att.01-Code需求记录字段表.md` 承载。需求记录只能说明来源、输入范围、输出形式、写入行为、验证方式、失败处理和缺口分流方式，不得替代正式规范或 Human Gate。
...
3. 明确失败条件：无法解析、来源缺失、冲突、越权、输入不足或能力缺口时，应输出诊断，不得静默猜测；
...
| 诊断 | 暴露缺口、冲突、漂移、问题原因和建议回写方向 | 直接关闭问题 |
...
过程输出必须区分候选输出、诊断输出、权威写入前检查和已写入事实源。Code 输出无法回指来源、无法区分候选与权威写入、或无法说明问题原因和残留风险时，不得被行动编排、Web 或 Human 当作稳定事实使用。
...
知识地图输入范围表由 `attachments/04.Att.05-知识地图输入范围表.md` 承载。该附件只说明允许输入、排除输入、后置输入和问题原因诊断，不得把知识地图运行时能力提前为当前主线。
...
8. `input_scope`；
9. `schema_version`；
10. `result_status` 或等价结果状态；
11. `issue_causes`；
12. `suggested_action`；
13. `degraded` legacy compatibility 字段；
14. `navigation`；
15. `read_plan`；
16. `next_queries`；
17. `stop_conditions`；
...
`diagnostics` 至少声明 `severity`、`code`、`message`、`source_refs` 和 `suggested_owner`。诊断等级闭集为 `error`、`warning`、`info`；遇到输入范围不明、关系类型不明、来源缺失、v1/v2 权威冲突或 Schema 不完整时，必须输出诊断，填充 `issue_causes` 和 `suggested_action`。过渡期可以同步将 legacy `degraded` 标记为 true，但调用方不得把该布尔值解释为可接受方案。

知识地图投影目标 Schema 由 `attachments/04.Att.06-知识地图投影Schema表.md` 承载。当前 `v2-check` 可以输出 active specs 诊断和只读知识地图预览子集，但必须通过 `issue_causes`、`diagnostics`、review hints 或 legacy `degraded` 暴露未实现的输入范围、原文层、多项目层或 raw 层，不得伪装为完整知识地图运行时。
...
5. 是否存在跨规范、Web、运行时扩展或测试同步影响；当写入目标为 specs 或运行时扩展承载物时，应提示可能受影响的固定 Rules 资产、自描述字段、入口读取顺序、STOP 点、验证入口和问题分流位置；
...
| 入口可见要求 | AI 需要查询、校验、聚合、诊断或生成知识地图时应能定位 Code 入口；涉及具体规范、附件、行动成员、运行时扩展承载物或 Rules 影响判断时，应能定位带 `start_node` 的知识地图任务导航入口 | CLI 帮助、Rules 入口、README、临时核对动作 | AI 执行入口提示 | 命令入口、帮助文本、任务导航入口或读取顺序变化时 |
| 确定性执行要求 | 可机械检查内容应由 Code 输出稳定 exit code、结构化诊断和来源回指 | Code 实现、测试、CI、临时核对动作 | 校验实现 | 解析规则、输出 Schema、诊断等级或输入范围变化时 |
| 知识地图投影要求 | 知识地图必须声明受控输入范围、最小输出 Schema、关系类型闭集、来源回指、问题原因诊断和以 `start_node` 为核心的任务导航语义；汇总健康检查不得替代具体任务定位 | 本文、01.Att.01、07、Code 检查、测试 | 派生投影 | 知识地图输入、节点、边、诊断、来源回指、任务导航语义或双读规则变化时 |
| 受控写入前检查要求 | 任何 Code 写入前检查都必须暴露路径、字段、状态、Gate、事实源和追溯风险，且不得直接授权写入 | Code 检查、07、Human Gate、行动编排 | 受控写入 | 写入前检查能力新增、扩大或失败时 |
| Human 交互要求 | 接受 Code 关键校验长期缺口、受控写入、环境能力声明或高影响自动修复前应评估 Human Gate | Human Gate、影响范围说明、确认记录 | Human Gate | Code 越过只读/诊断边界或影响事实源时 |
| 生命周期触发要求 | 规范、事实模型、行动编排、Web、运行时扩展、事实源或测试变化后，应检查 Code 消费是否同步 | Code 回归、知识地图诊断、测试、临时核对动作 | 生命周期同步 | 任一上游结构或消费契约变化时 |
...
| 知识地图投影 | 输入范围、关系类型、节点边 Schema、来源回指、问题原因字段和 legacy `degraded` 兼容字段可检查 |
...
4. 明确 v1-v2 双读、问题原因诊断和迁移覆盖检查；历史规范图谱不作为当前知识地图验收目标；
