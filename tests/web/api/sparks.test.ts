...
async function testCreateSpark() {
  ...
  assert.deepEqual(payload.summary, {
    ...
    type: 'spark',
    status: 'pending',
    source_refs: [{ path: 'ldvh-base/sparks/spark-0001-api-spark.yaml', role: 'fact_instance' }],
  })
  ...
  assert.deepEqual(persisted.related_workcases, [])
  assert.deepEqual(persisted.related_adrs, [])
  assert.deepEqual(persisted.related_docs, [])
  assert.equal('category' in persisted, false)
  assert.equal('status_history' in persisted, false)
  assert.equal('规范10' in persisted, false)
}
