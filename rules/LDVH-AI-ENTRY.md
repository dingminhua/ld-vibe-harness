...
它不是最终事实源。正式规则以 `specs/` 为准，管辖项目清单以工作区根目录 `LDVH-GOVERNED-PROJECTS.yaml` 为准，04 系列总纲以 `specs/04-规范落地与环境适配基础规范.md` 为准，环境入口适配、部署、个人环境特别要求写入和检查方法以 `specs/04.03-环境入口适配与部署规范.md` 为准；需要长期保留的稳定事实必须按事实源归属进入对应 Git 文件。管辖项目内容事实以对应项目的 `docs/`、`ldvh-base/` 或其他权威 Git 文件为准。
...
| 要求 | 内容 | 适用范围 | 影响动作 | 优先级 | 退出条件 |
|---|---|---|---|---|---|
| Python 命令入口 | 执行 Python 相关命令时默认使用 `python3` | 当前个人环境和当前工作区执行 | Code 检查、测试、校验和脚本命令 | Human 当前指令、项目 README、依赖管理说明、虚拟环境要求使用 `python`、`uv`、`poetry`、`pipenv` 或其它入口时优先 | Human 明确替换、项目约定改变或该要求迁移到更准确环境入口时 |
| 写入与权限边界 | 修改工作区级入口、环境入口、长期配置、自动触发配置或跨工作区写入前必须先触发 Human Gate | 当前个人环境、当前工作区和 Human 授权场景 | 部署、写入、适配检查、自动触发和跨工作区操作 | Human Gate 和项目安全边界优先，不得用本入口绕过 | Human 明确取消或对应规范变更并完成入口同步时 |

个人环境特别要求不得升级为 LDVH 通用跨环境规则；未写入对应环境入口且未被 Human 当前确认的个人要求不得假定启用。
...
8. 要新增、修改或删除环境入口中的个人环境特别要求；
9. 入口内容与 specs 正式规范、管辖项目配置、工作对象事实或 Code 校验结果冲突；
10. 需要接受长期降级、关闭关键缺口、绕过 Human Gate 或改变事实源边界。
