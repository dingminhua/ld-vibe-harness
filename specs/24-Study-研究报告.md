...
Study 保存一轮已经完成、可独立引用且具有跨行动稳定阅读价值的**外部内容调研**。其研究对象必须是外部产品、实践、标准、论文、工具或方案；它使后续 AI 能快速恢复外部研究对象、输入边界、关键发现、建议和后续分流，从而避免重复调研，并把研究启发转化为项目下一步。

Study 主要服务 V1 快速定位、V2 充分理解、V3 边界识别、V5 据实判断、V7 清晰沟通和 V8 持续积累。外部调研的执行与进度由 WorkCase 和当次计划承担；Study 不承载搜索过程、任务编排、内部设计探索或行动授权。项目内 Spark、WorkCase、ADR、Pitfall、specs 和普通文档只可说明调研动机、项目边界或后续吸收，不能单独构成 Study 的研究主体或外部资料。新增成本包括查重、来源精确化、Schema、迁移和消费；通过只准入已经完成的一轮外部调研、Markdown 单一正文、两状态、四个专属阅读元数据字段、统一来源资料和禁止多套 related 字段，维护成本低于重复重建资料、误用旧结论和无法定位依据的损耗。普通文档已经能够自然承载且不需要稳定身份、状态和后续复读价值时，不创建 Study。
...
3. 研究问题、快速摘要、来源、输入边界、关键发现、建议与后续分流要求；
...
2. 存在一个可独立引用的外部研究对象和核心研究问题；子问题共享同一外部对象、输入、方法、适用边界和发现单元；
3. 实际输入、研究方法、观察时点、适用边界、排除项和未覆盖范围清楚；
4. `urls` 至少包含一条实际读取的外部 HTTP(S) 资料；每项资料的用途或限制在共用 `urls[].summary` 中说明，关键发现的来源边界在正文中如实表达；
5. 报告能够以可阅读的“外部观察—项目启发—适用边界—后续选择”单元形成关键发现、建议和后续分流，不以验证或关系充当研究结论的证明；
...
Study 对象使用 UTF-8 Markdown，一文件一对象，当前权威位置固定为管辖项目仓库中的 `ldvh-base/studies/<object_id>.md`。`object_id` 必须匹配 `study-[0-9]{4,}`；文件名必须与 `object_id` 完全一致，分配后的身份不得因标题、路径、状态或内容改变。`title` 应以“外部研究对象 + 当前项目判断主题”简短识别研究，不复制 `research_question`、`abstract`、建议摘要或结论；它是当前文件名之外的可读命名入口。YAML frontmatter 只承载本节绑定字段，之后的 Markdown 正文承载详细报告。未知或不适用的条件字段必须省略，不使用 `null`、空字符串、空数组、占位时间、默认状态或默认关系。
...
|---|---|---|
| `研究问题` | 清楚区分当前项目为何需要这轮研究，与本对象实际回答的外部问题；可用 H3、段落或表格组织 | 只重述标题、搜索关键词或 Human 原话 |
| `输入与边界` | 说明实际读取的外部资料如何分工，以及观察时点、未覆盖、冲突或不适用范围；可用 H3、段落或表格组织 | 只复制 URL 列表，或用内部代码、规则、日志代替外部资料 |
| `关键发现` | 一个或多个可独立讨论的发现单元；每个单元据实区分外部观察和对项目可能带来的启发。只有存在实质误解、风险、冲突或不适用边界时，才说明其限制；active Study 至少有一个发现或建议明确提出由研究带来的具体项目方向、取舍或下一研究问题 | 一段压缩结论、资料摘抄、把产品能力直接写成项目规则，或把每条发现机械写成“这不等于什么”的免责声明 |
| `建议` | 一个或多个具体建议或取舍，说明适用条件、风险或不成立边界；可以按立即实践、待验证行动或 Human 取舍组织，也可以没有固定子标题 | 为填满模板虚构每一类建议，或把建议伪装成已决定行动 |
| `后续分流` | 清楚说明每项建议/未决问题的后续承载方向，或为何无需对象化；可用表格、列表或短段组织 | 只写“后续再看”、无条件地创建 Spark，或暗示下游对象已经建立 |
...
下列读序定义的是同一份 Study 源内容的消费语义，不新增阅读投影字段、DTO、副本或第二报告，也不把 V3 的 `user_intent` 原话重新引入 V4：
...
这一定义恢复 V3 的“意图—摘要—建议—正文”阅读节奏，而非恢复其旧字段、关系或审计式资料结构；外部资料仍按 `urls` 和正文“输入与边界”按需进入阅读。具体页面布局、入口控件、复制行为和 Web 传输投影由 08 与实现说明负责，不由本文定义。

Study 必须由外部内容调研形成。每项来源在 `urls[].summary` 中说明用途或限制，正文说明研究输入、边界和由此得到的启发；只复制 URL、标题或旧结论而丢弃用途、边界和发现不成立。项目内对象或文档可记录问题来源或 LDVH 上下文，但不能代替外部研究对象或外部资料。若一项稳定信息无法在现有引用成员和正文中无歧义承接，必须停止创建并按 05 重新进行字段准入。
...
Study 必须有至少一条 `urls`。`urls` 是全体事实对象共用的字段：每项使用 `{ ref, title, summary }`，`ref` 是绝对 HTTP(S) URL，`title` 是资料标题，`summary` 说明该资料对当前对象支持什么、未支持什么或具有什么限制。Study 对该共用字段收紧为必填；项目内路径、代码、日志、会话与 Git revision 不得写入事实对象。

研究问题由 `research_question` 表达；外部资料的用途、输入边界、关键发现、推断和限制由 `urls[].summary`、正文与 `abstract` 据实说明。不得以 URL 本身、关系或验证动作代替这些说明。Study 不定义 relation_key；下游对象是否建立关系，依相邻对象的类型规则决定。

Study 没有统一 TTL；当前消费依赖易变资料时，AI 必须重新检查必要外部资料并更新自然语言结论或收紧适用范围。

### 主动召回与消费时机

Study 在当前问题需要外部研究启发、准备调研实质相同或相邻的外部对象、比较外部方案或当前输入精确引用研究报告时产生召回机会。F2 Study 候选卡直接投影 `object_id`、`title`、`status`、`research_intent`、`research_question`、`abstract`、`recommendation_summary` 和 `updated_at`，并只提示引用中是否存在版本或观察时点边界，不在候选层解释研究结论当前性。F2 投影可以同时含研究问题；它不是面向 Human 的首级详情布局。只在关键发现、限制或建议会影响当前判断时展开完整正文，不得因主题相似就向 AI 无差别注入整份报告。
...
任何创建或更新都必须先召回相邻对象和当前字段登记，确认 Human 当前指令已经授权，且适用于该行动的全部来源规则许可条件已经成立，随后写入唯一当前文件，回读 Working Tree，并验证 frontmatter、正文、状态和来源。created_at 与 object_id 不因内容修正改变；正文、research_intent、abstract、recommendation_summary、来源用途、版本或观察时点实质变化时 updated_at 必须同步更新。
...
| 外部资料与研究边界 | 外部资料以可复核 URL、标题和支持范围/限制摘要保留；输入、观察和时效限制由正文表达 | `urls`、正文与重读来源 | 资料治理 | 创建、更新或消费研究时 |
...
| 外部资料、研究边界与时效 | 创建、更新或消费可变化研究时 | `urls` 至少一项，且每项为 HTTP(S) URL、标题和支持范围或限制摘要；正文表达输入、观察、发现和未覆盖范围；重读范围清楚 | 本文、03、对象正文与外部 URL | URL shape 检查、正文与 URL 回读、AI 观察审查 | 被核对的外部资料、发现和时效边界 | 暂停受影响发现消费；补充资料或说明、更新或退出 |
...
Code 的共同机械边界按 05 §§10–11 执行；对 Study，只可额外检查本文明确给出的 frontmatter、`urls` 的 HTTP(S) shape、标题、摘要与去重、正文 H2 和状态约束。研究价值、外部资料充分性与真实性、观察时效、推断、限制、建议、自然语言同义性、阅读单元的实质内容和外部事实当前性仍由 AI 依据当前来源审核。

最低验证样例必须覆盖：有效 HTTP(S) URL；缺 URL、标题或摘要；缺研究意图或建议摘要；本机路径、代码/日志路径、会话描述、Git revision、非 HTTP(S) URL 与重复 URL；正文五个 H2 的缺失、重复、乱序和空内容；两个状态及终态字段。当前 Study parser/validator 已由 `read-fact-objects` 的只读实现从本文与统一登记派生；实现和测试只证明实际覆盖的机械范围，不得把读取通过冒充研究价值、外部资料真实性、资料充分性或当前可消费性已经成立。
...
2. 缺少可复核的外部 URL、标题或支持范围/限制摘要；关键发现、输入边界或限制无法在对象自身说明与外部资料之间如实表达，或当前性要求尚未重读来源；
