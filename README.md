...
当前 00–08、Spark、WorkCase、ADR、Pitfall、Study 唯一定义来源及四份授权附件声明为 `active`，并通过当前已实现的机械结构检查；这不替代 01 对其它变更要求的语义审核和独立复核。05 及其字段登记附件已经建立统一字段与结构登记、唯一字段定义、类型使用绑定和单一提升来源边界；Spark、WorkCase、ADR、Pitfall 与 Study 已按严格串行顺序完成类型定义、机械验证、两路独立复核及五类型全局归并。字段与结构准入比较、独立复核和 V3 历史统计已退出 20–24 的主阅读路径，改由非规则源的 Git 审计证据承载；Code 只检查证据引用、覆盖和结构，字段与类型语义仍由 05、`05.Att.01` 与 20–24 唯一定义。五类型的完整字段组合可以从统一登记与类型绑定实时派生，但派生视图不是第二 Schema 权威。阶段 4 已关闭；阶段 5 的事实对象只读增量已经完成并加入来源定义的 `read-fact-objects`，但受控回写尚未授权。V4 Code 已建立规范模型确定性基础、Helper CLI 基础契约和四项当前公开操作；Web 已建立首批自有 API 特征测试和浏览器表现基线。自然语言规则的语义和适用性由 AI 依据当前来源判断；Code 不得解释规则，只执行来源已定义的结构化契约。当前仍没有事实对象写入或迁移、行动模板执行、环境接入或 V4 Web 适配完成声明；完整路线、当前边界和验证证据见 [工作推进总纲](docs/v4-architecture/V4-工作推进总纲.md)。
