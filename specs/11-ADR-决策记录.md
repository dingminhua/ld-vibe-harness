...
> 创建日期：2026-05-30
> 定位：定义 ADR 决策记录生产对象模型，包括对象定位、准入条件、事实源边界、状态机、对象关系、Human Gate、字段契约、适配原则、模型实践子文档规划和实例检查
...
`specs/06-LDVH生产对象基础规范.md` 定义生产对象通用规则、文件命名、模型实践子文档命名和生产对象模型标准组成。本文依据 06 §4.2 定义 ADR 对象模型。
...
模型实践子文档含义和命名规则见 06 §4.5。ADR 模型实践子文档规划如下：
