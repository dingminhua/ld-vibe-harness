...
def test_urls_are_validated(current_specs_repository: Path) -> None:
    ...
    fields = {
        **_common("study", "study-0001", "active"),
        "urls": [{"ref": "https://example.invalid/research", "title": "Research", "summary": "Supports the bounded finding."}],
        "applicability": "This project",
        "validation_summary": "Read the material; scope is limited to its stated contract.",
...
def test_study_requires_urls(current_specs_repository: Path) -> None:
    ...
    fields = {
        **_common("study", "study-0001", "active"),
        "applicability": "This project",
        "validation_summary": "No external material was recorded.",
        "research_question": "What is the external contract?",
        "abstract": "A bounded answer.",
    }
    issues = validate_fact_object("study", fields, schema)
...
def test_relations_accept_only_key_and_target(current_specs_repository: Path) -> None:
    ...
    fields = {
        **_common("spark", "spark-0001", "open"),
        "summary": "A current question.",
        "priority": "P2",
        "relations": [{"relation_key": "related-to", "target": {"governed_project_id": "p", "fact_type_key": "adr", "object_id": "adr-0001"}}],
    }
    assert validate_fact_object("spark", fields, schema) == ()
    fields["relations"] = [{**fields["relations"][0], "source_refs": []}]
    assert any(issue.field_path == "relations[0].source_refs" for issue in validate_fact_object("spark", fields, schema))
...
def test_workcase_natural_language_result_is_the_evidence_carrier(current_specs_repository: Path) -> None:
    ...
    fields = {
        **_common("workcase", "workcase-0001", "open"),
        ...
        "plan_version": 1,
        "work_items": [{"item_id": "item-01", "goal": "Implement", "expected_result": "A validated result", "status": "pending", "approach_summary": "Make the bounded change."}],
        "creation_reviews": [{"reviewer": "reviewer", "reviewed_at": "2026-07-14T09:30:00+08:00", "subject_version": 1, "scope": "Plan", "conclusion": "pass", "feedback": ["Proceed"], "controller_resolution": "Accepted."}],
        "priority": "P2",
