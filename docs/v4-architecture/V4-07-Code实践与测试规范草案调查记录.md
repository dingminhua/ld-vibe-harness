...
6. 把 Code 规划和测试拆成两份需要重复读取、同步维护的横向规范。

本次只新建 07 草案及本文，不修改 active 00–03、附件、其它候选规范、Code、tests、`web/`、`icons/` 或归档。

## 2. 当前规则源与历史输入

### 2.1 直接规范依据

07 的直接 `basis` 只保留 `ldvh-root`。00 已经定义：
...
“Code”已经是当前双语术语；“实践与测试 / Practices and Testing”在标题中使用自然描述，不建立新的 LDVH 核心术语或机器表示，因此本轮不修改双语术语表。
...
|---|---|---|---|
| [ISO/IEC/IEEE 42010:2022 — Architecture description](https://www.iso.org/standard/74393.html)，国际标准，Edition 2，2022-11，状态 Published | 公开摘要规定架构描述的结构与表达要求，同时明确不规定创建和管理架构描述的方法、表示法、工具、格式或介质 | 仅作间接类比：支持可检查结构说明与不固定表达工具可以并存；它不直接要求 LDVH 在实现前建立 Code 规划 | 未声称 07 符合该标准，也未复制其 viewpoint、model kind 或一致性模型 |
| [SWEBOK Guide V4.0a](https://www.computer.org/education/bodies-of-knowledge/software-engineering)，IEEE Computer Society 共识性知识指南，页面标示 2025-09 更新为 V4.0a | 官方页面说明其汇总 generally accepted、consensus-driven knowledge；其 Software Design 内容包含 decomposition/modularization、coupling/cohesion、separation of interface and implementation 等设计原则 | 支持检查模块化、耦合、接口与实现分离是否是有可靠出处的工程关注点 | SWEBOK 不是 LDVH 规则源；未把所有知识领域、术语或推荐做法整体引入 07 |
...
| [ISO/IEC/IEEE 29119-2:2021 — Test processes](https://www.iso.org/standard/79428.html)，国际标准，Edition 2，2021-10，状态 Published | 公开摘要说明其测试过程可用于治理、管理和实施软件测试，并适用于各种软件生命周期模型 | 仅支持测试过程可以适配不同生命周期；“测试必须与每次实现变化同步”是 LDVH 基于自身风险作出的内部规则选择，不是该摘要的直接要求 | 未声明采用完整 29119 流程、文档模板、角色或一致性要求 |
...
| [NIST SP 800-218 SSDF v1.1](https://csrc.nist.gov/pubs/sp/800/218/final)，美国 NIST Final Publication，2022-02 | 官方摘要将安全实践描述为可整合进不同 SDLC 的高层 outcome-based practices，并强调降低漏洞、影响和复发根因 | 支持在适用时把安全、信息泄露和复发根因纳入风险与重构考虑 | 该来源只直接覆盖安全软件开发；观察日另有 v1.2 draft，本记录只采用已发布 final v1.1，不把 draft 内容写入当前结论，也未把 SSDF 全套实践扩张为所有 LDVH Code 的通用流程 |
...
| [Google Engineering Practices — What to look for in a code review](https://google.github.io/eng-practices/review/reviewer/looking-for.html)，组织一手工程指南 | 页面要求按变化选择 unit/integration/end-to-end tests，并检查测试在 Code 被破坏时是否真正失败；同时提醒 tests 也是需要维护的 Code | 支持测试缺陷敏感性、按风险选择层次以及 tests 自身可维护性 | 未把 Google review 规则、语言风格或组织角色引入 LDVH |
...
草案要求在首次实现或实质改变模块、依赖、接口、Schema、诊断和测试策略前，先形成相应 Code 规划。规划至少连接：
...
7. 把 Code 规划、测试报告或外部资料登记为规则源或事实模型；
...
1. YAML 固定为 `spec_id: "07"`、`status: "draft"`、`parent_spec: "ldvh-root"`、`relation: "refines"`、`basis: ["ldvh-root"]`、无附件；
2. 中文标题与 Human 决定一致，英文标题自然对应；
3. `spec_key` 按稳定责任而非标题镜像选择；
4. 固定头部与固定尾部齐全，H2 连续；
5. Code 规划、模块、依赖、接口、Schema、诊断、重构、风险驱动测试、测试检查范围与证据边界均已覆盖；
...
本文只完成主执行草案与自检。07 转为 active 前仍须按 01 完成独立复核、异议处理和当次规则源验证。
