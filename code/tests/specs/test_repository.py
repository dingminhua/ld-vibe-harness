...
def test_current_v4_sources_form_the_expected_real_combination(current_specs_repository: Path) -> None:
    ...
    assert len(checked_documents) == 36
    assert sum(document.kind != "attachment" for document in checked_documents) == 20

    assert sum(document.kind == "attachment" for document in checked_documents) == 16
    assert len(inspection.projections) == 108
    ...
    assert declaration_sources == {
        ...
        "check-workcase-handoff": "specs/21-WorkCase-工作项.md",
        ...
        "git-hooks-status": "specs/09-环境接入规范.md",
        ...
        "prepare-fact-object-update": "specs/05-事实模型基础规范.md",
        "prepare-local-edit-candidates": "specs/04-Helper CLI 服务规范.md",
        ...
        "recover-invalid-workcase": "specs/21-WorkCase-工作项.md",
    ...
    assert len(fields.registrations) == 192
...
def test_foundation_relationship_failure_stops_before_other_identity_validation(
    ...
    assert validated_paths == [
        ...
        "specs/attachments/01.Att.02-规范身份字段表.md",
        "specs/attachments/01.Att.03-规范关系与层级表.md",
    ]
    assert inspection.active_documents_passing_implemented_checks == ()
    assert inspection.projections == ()
    assert "specification-model-foundation" in inspection.incomplete_scope
