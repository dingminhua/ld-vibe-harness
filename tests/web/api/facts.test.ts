...
async function main() {
  ...
  assert.deepEqual(workcases.summary.source_refs, [{ path: 'ldvh-base/workcases', role: 'fact_instance_directory' }])
  ...
  assert.ok(Array.isArray(firstWorkCase.source_refs))
  assert.match(String((firstWorkCase.source_refs as Array<Record<string, unknown>>)[0].path), /^ldvh-base\/workcases\/workcase-\d{4}-.+\.yaml$/)
  ...
  try {
    ...
    const fixtureItems = fixturePitfalls.data.items as Array<Record<string, unknown>>
    const firstPitfall = fixtureItems.find((item) => item.id === 'pitfall-0001')
    assert.ok(firstPitfall)
    assert.equal(firstPitfall.type, 'pitfall')
    assert.equal('repeatability' in firstPitfall, false)
    assert.deepEqual(firstPitfall.source_refs, [{ path: 'ldvh-base/pitfalls/pitfall-0001-api-contract.yaml', role: 'fact_instance' }])
  ...
  assert.deepEqual(detail.summary.source_refs, detail.data.source_refs)
