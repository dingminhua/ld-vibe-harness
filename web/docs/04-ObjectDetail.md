...
1. 计划态势：页面主区域第一块，按活跃、待关闭、已闭合分组展示关联计划。
2. 活跃/待关闭计划默认展开，每行作为进入 TaskPlan 的入口；已闭合计划默认折叠。
3. 计划行展示计划标题、计划 ID、更新时间、状态、成功标准/完成证据记录状态、复制路径和进入箭头。
4. 属性：按目标、来源、范围、约束展示 `description/source/scope/constraints`，不把 `scope` 放进顶部元信息 chip；标题使用与计划态势一致的图标+标题样式。
5. 关联材料：只有 `related_docs/related_adrs/related_memos/related_pitfalls` 非空时显示。
