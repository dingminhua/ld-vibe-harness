...
> 创建日期：2026-05-30
> 定位：LD Vibe Harness 对自建 MCP 的项目级评估
> 调研边界：不直接构成强制规则
...
> 来源：`docs/sources/02-Trae-MCP用法调研.md`、`docs/studies/06-LDVH的MCP使用评估.md`
...
LDVH 可以考虑自建 MCP，但自建 MCP 不应成为新的工具层，也不应替代 code/ Tools 辅助层。它更准确的定位是：
...
4. 底层解析、校验、聚合、上下文包和写入能力优先复用 code/；
...
| 底层能力可复用 | 底层解析、校验、聚合或写入逻辑优先来自 code/ Tools 辅助层 |
...
8. 自建 MCP 会重新实现一套与 code/ 重复的业务逻辑；
9. 自建 MCP 需要维护独立权威数据存储；
10. MCP 输出无法追溯到 Git 文件事实源。

---

## 4. 自建 MCP 与 05 工具层的关系
...
1. 自建 MCP 的底层逻辑应复用 code/ 中的解析、校验、聚合和受控写入模块；
...
| 编程语言 | Python | 与 code/ Tools 辅助层一致，便于复用解析、校验、聚合模块 |
...
| 阶段 | 内容 | 前置条件 |
|---|---|---|
| 阶段一 | Fact Reader MCP + Status Aggregator MCP | 10-39 至少 Task 和 Change 对象规范稳定；code/ 解析和聚合模块可用 |
...
自建 MCP 应复用 code/ 中的核心模块，而不是重新实现：
...
| 维护成本上升 | 需要适配 Trae MCP 协议、对象规范和 code/ 模块变更 | MCP 入口层保持薄封装；业务逻辑复用 code/；入口层只测试协议适配 |
| 与 Web 信息同步层功能重叠 | Fact Reader、Status Aggregator、Context Pack 与 Web 信息同步层可能读取同类数据 | 底层复用同一 code/ 模块；MCP 面向 Agent 结构化调用，Web 面向人可视化交互 |
...
1. 本文结论如何影响 LDVH 自建 MCP 工具设计待工具规范稳定后确定；
2. 本文结论如何影响 MCP 工具与事实源边界待事实源规范稳定后确定。
