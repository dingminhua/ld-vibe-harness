...
- 展示 Rules、Skill、Agent、Hook 四类必备部署入口的当前仓库资产、位置、当前资产状态、使用场景和降级方式。
- 数据来源优先指向 `rules/LDVH-DEPLOYMENT-ENTRIES.md`、`rules/LDVH-AI-ENTRY.md` 和 Code 的部署入口资产清单检查结果。
...
- 若清单缺失、入口类型不齐、资产路径不存在、Rules 未引用清单或 Code 检查失败，应展示错误态或降级提示，并引导 Human 回到 Validate、Code 输出或 Git 文件事实源查看详情。
...
| 点击最近变更条目 | 跳转到变更日志 |
| 点击落地引导按钮 | 跳转到校验页 |
| 查看部署入口资产清单摘要 | 只读查看四类入口资产和检查状态；如需处理问题，跳转或提示到 Validate / Code 输出 / Git 文件事实源 |
...
7. 部署入口资产清单摘要不得设计成开关、安装向导、可选配置表或环境完整支持声明；只能作为 Human-facing 只读态势展示。
