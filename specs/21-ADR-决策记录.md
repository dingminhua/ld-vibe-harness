...
| 1 | 本文解决的问题 |
| 2 | 与 13 的关系 |
| 3 | 对象定位与准入条件（§3.1 定义、§3.2 与临时判断、§3.3 准入条件、§3.4 与 specs/Rules 边界） |
| 4 | 事实源边界 |
| 5 | 状态机（§5.1 标准状态、§5.2 合法状态流转） |
| 6 | 与其他对象的关系（§6.1 Memo→ADR、§6.2 Task/Evidence→ADR、§6.3 ADR→Task、§6.4 ADR→specs/Rules、§6.5 不满足准入） |
| 7 | Human Gate（§7.1 必须触发、§7.2 应评估、§7.3 推翻与废弃） |
| 8 | 字段契约（§8.1 基础字段、§8.2 ADR 扩展字段、§8.3 字段约束） |
| 9 | 事实源回写要求 |
| 10 | 证据留存要求 |
| 11 | AI 协作适配 |
| 12 | Tools 辅助适配 |
...
8. Tools 辅助程序写入 ADR 时应校验字段完整性、状态合法性、引用有效性、写入授权和 Change 记录要求。
...
12. ADR 创建、升级、推翻和废弃流程中的编号生成、ADR YAML 写入、状态流转写入、关联字段回写、推翻或废弃字段回写和 Change 记录生成属于应评估的可机械化受控写入步骤。
...
3. ADR 写入策略是什么，包括哪些场景允许创建、接受、拒绝、升级、推翻、废弃、关联回写或记录 Change，哪些写入必须触发 Human Gate，哪些写入由 Tools 承接，哪些写入需 AI 或人按规范完成。
...
6. 确认 ADR 写入入口，包括创建、接受、拒绝、升级、推翻、废弃、关联字段回写和 Change 记录生成；
...
14. 确认 ADR Tools 是否已覆盖编号生成、ADR 创建、状态流转写入、关联字段回写、推翻或废弃字段回写和 Change 记录生成等可机械化受控写入步骤；未覆盖时应记录为完整落地初始化阻塞项或暂缓项；
...
17. ADR Tools 是否覆盖编号生成、ADR 创建、状态流转写入、关联字段回写、推翻或废弃字段回写和 Change 记录生成等可机械化受控写入步骤；未覆盖时是否已标记为阻塞项或暂缓项；
...
7. ADR 写入策略是否符合本文 §7、§9、§12和 §15，是否覆盖 Human Gate、受控写入、人工写入、Change 记录和 Tools 降级边界；
