...
6. **当前复核、授权边界与批准是什么**：活动期读取 `plan_version`、当前 `creation_reviews`、`execution_authorization`、`execution_approval`，以及已经形成时的 `result_version`、`controller_check_summary` 和当前 `result_reviews`。详情必须分别呈现 Reviewer 复核、Controller 处置、执行授权边界与 Human 执行批准；授权边界完整展示 authorized actions、行动上限、禁止项、允许调整、验证回滚、越界处置和可选 Human 前置条件。批准必须显示绑定版本、`baseline_fingerprint` 与非空 `source_refs`。
...
“目标与边界”中的目标和范围使用适合连续阅读的紧凑层级：标题为 `14px / 22px`，正文为 `13px / 22px`。目标使用与紫色背景协调的紫色标题和正文色阶，范围使用与青色背景协调的青色标题和正文色阶；不得退回通用黑灰正文，也不得让标题与正文同字号。
