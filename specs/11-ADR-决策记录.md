...
> 相关规范：`specs/00-LD-Vibe-Harness理念与纲要.md`、`specs/01-specs文档结构与术语规范.md`、`specs/02-LDVH目录说明.md`、`specs/03-事实源边界与承载规范.md`、`specs/04-LDVH-AI协作规范.md`、`specs/05-LDVH工具基础规范.md`、`specs/10-生产对象集合索引.md`
...
`specs/06-LDVH生产对象基础规范.md` 定义生产对象通用规则、文件命名、附件型实践子文档命名和生产对象模型标准组成。本文依据 06 §4.2 定义 ADR 对象模型。
...
| ADR 模型实践子文档 | `specs/11.01-Rules.md` 至 `specs/11.05-Web.md` |
...
附件型实践子文档含义和命名规则见 06 §4.5。ADR 附件型实践子文档按需创建，不是 ADR 模型完整性的默认要求。当前规划如下：
...
1. ADR 对象模型规范写作是否符合 06 生产对象模型标准组成（06 §4.2），缺项是否已说明原因；
2. ADR 实例字段是否符合本文 §八 定义的字段契约；
3. ADR 状态流转是否符合本文 §五 定义的状态机；
4. ADR 对象关系引用是否符合本文 §六 定义的关系规则；
5. ADR 事实源边界是否符合本文 §四 和 03 事实源边界规范；
6. ADR Human Gate 是否符合本文 §七 定义的触发条件；
7. ADR AI 协作适配是否符合本文 §九；
8. ADR Tools 辅助适配是否符合本文 §十；
9. ADR Web 展示适配是否符合本文 §十一；
10. ADR 附件型实践子文档命名是否符合 06 §4.5，是否按需创建且经 Human Gate 确认。
...
3. ADR YAML schema 待按需创建的 `11.04-Tools.md` 或 `11.06-Contract.md` 中细化；
