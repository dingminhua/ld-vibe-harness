...
> 相关规范：`specs/01-LDVH目录说明.md`、`specs/03-Specs文档规范.md`、`specs/13-LDVH事实模型基础规范.md`、`specs/14-LDVH行动模型基础规范.md`、`specs/20-事实模型集合索引.md`、`specs/50-行动模型集合索引.md`
...
LDVH 当前最适合做的系统性结构调整，不是新增更多对象或一次性重排所有规范，而是围绕“五类构成要素”和 Core Loop 建立更清晰的层级边界：正式规范稳定定义规则，evals 承载讨论和方向，事实模型承载项目事实，行动模型承载 AI 行动控制，Tools 和 Web 只消费和辅助事实源，不成为新的权威状态。
...
3. 行动模型是否能补齐 Plan、Execute、Verify、Record、Learn 的最小入口，而不是一次性扩张大量 Skill；
...
LDVH 当前顶层结构由五类构成要素共同支撑：介质、开发环境机制、工具、事实模型和行动模型。
...
| 行动模型 | specs/50-79、Rules、Skill、Agent 编排 | 控制 AI 如何读取、判断、执行、停下和回写 |
...
| 10-19 | 核心基础规范 | 承载事实源、协作、工具、事实模型、行动模型等基础规则 |
...
当前行动模型区间已有 `51-multi-role-thinking-多角色思考.md`，并为 52-60 预留了项目初始化、对话转需求、需求转任务、Task 执行、阻塞处理、Review、审计、对象创建和状态更新等行动。
...
结构调整时应避免把行动模型、Skill 和事实模型混为一谈。行动模型定义“AI 如何行动”，事实模型定义“项目事实如何存在”。Skill 是行动模型在开发环境机制中的一种落地入口，但不是行动模型本身。
