...
- `ldvh-base/`（工作区级目录，位于工作区根目录）承载结构化生产对象实例和变更记录
...
3. 新增或修改项目文档、规则或规范后，必须在 `ldvh-base/changes/`（工作区级目录）创建 Change YAML 记录（依据 `specs/10-事实源边界与承载规范.md` §七）

## L2 场景规则

- `ldvh-l2-specs-rules.md`：编辑 specs/ 目录下文档时生效
- `ldvh-l2-production-rules.md`：编辑 ldvh-base/（工作区级目录）下 YAML 实例时生效
