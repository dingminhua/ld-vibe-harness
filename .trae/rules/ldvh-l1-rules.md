...
LDVH 定位和总纲级价值原则见 00，目录见 01，术语见 02，specs 骨架和引用纪律见 03，specs 索引和章节定位见 03.01。定位 specs 文档和章节时，先尝试使用 specs 索引程序或其输出定位候选文档和章节行号，再按路径和行号读取原文片段；索引缺失、过期或不可用时，回退到搜索 Markdown 标题和行范围读取；evals/refs 仅在用户明确指定或任务明确要求时读取。
...
LDVH | 00总纲 | 不自动push | 价值标准服务 | 索引优先定位 | 索引不可用才搜索回退 | 读原文判断 | 稳定事实回Git | HumanGate先暂停 | 改文档写Change | specs进L2 | ldvh-base进事实模型L0 | Rules审计读11.01§7
