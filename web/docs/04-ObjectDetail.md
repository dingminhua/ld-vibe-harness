...
- ADR 的 `retired` 中文状态显示为”已退出”；Study 的 `retired` 中文状态显示为”已退出”；Pitfall 状态显示 `draft / active / discarded`，分别使用“待确认 / 活跃 / 已废弃”。Pitfall 的 draft、discarded 仍使用与 active 相同的完整正文阅读形状；discarded 的处置说明 Human 未接受 draft，或曾经 active 的经验已不再适用及其去向，不生成额外时间字段。详情页不提供 promote、discard、批量审核或自动过期控件。
