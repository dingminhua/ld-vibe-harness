...
> 文件状态：temporary migration decision。本文记录阶段 9 的剩余 V2 内容归口、用户参与边界和 V3 soft mainline switch 完成标准；它不授权 Hook 安装、Web 通用写入、Human Gate 决策或 hard switch。正式规则仍以 `specs/` 正文为准。
...
5. Hook 不在 soft mainline 中启用。阶段 9 只完成最小 Git 提交链路的 commit gate CLI / wrapper；真实 Git Hook、session_start、pre_tool_use、completion_claim 等通用入口作为 hard switch 后置。
...
| 9B 最小提交入口 | 让 V3 具备提交链路校验能力 | Git 提交行动、commit message 契约、read_plan 消费证据、验证声明边界、commit gate CLI / wrapper | 不启用真实 Git Hook，不把 gate 输出写成 Human 授权 |
...
5. 声明 V3 hard switch、启用阻断型环境入口，或接受仍未迁移能力的残留风险；
...
9A 已完成迁移层依赖审计，结论见 `_migration/9A-migration-layer-dependency-audit.md`。9B 已完成 V3 自有 commit gate、CLI 和 Hook wrapper，但未启用真实 Git Hook。9C 已完成事实对象完整迁移，结论见 `_migration/9C-fact-object-full-migration.md`。9D 已完成 Web 数据契约迁移，结论见 `_migration/9D-web-data-contract-migration.md`。9E 已完成行动模板候选后置，结论见 `_migration/9E-action-template-candidate-deferral.md`。9F 已完成 soft mainline 收口，结论见 `_migration/9F-mainline-soft-switch-closure.md`。后续若要 hard switch，必须另起 Hook / Rules / runtime adapter 环境接入工作并进入 Human Gate。
