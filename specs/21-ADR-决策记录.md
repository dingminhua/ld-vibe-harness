...
> 创建日期：2026-05-30
> 定位：定义 ADR 决策记录工作模型，包括对象定位、准入条件、事实源边界、状态机、对象关系、Human Gate、字段契约、事实源回写、证据留存、适配原则、附件型实践子文档、落地初始化、落地审计和合规检查
...
`specs/07-工作模型基础规范.md` 定义工作模型通用规则、文件命名、附件型实践子文档命名和工作模型标准组成。本文依据 07 §4.2 定义 ADR 对象模型。
...
2. ADR 对象模型是否明确归属于 LDVH 工作模型，且未混淆四类构成要素边界；
...
1. ADR 对象模型规范写作是否符合 07 工作模型标准组成（07 §4.2），缺项是否已说明原因；
...
12. ADR Tools 契约式校验与执行适配、只读查询能力、受控写入能力缺口识别及其测试验收是否符合本文 §12和 `specs/06-Python与Web工具规范.md §4`；
