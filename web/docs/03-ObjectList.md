...
- 保留通用卡片头部：ID、复制对象路径、状态、标题；外层卡片可点击进入 WorkArea 详情，标题右侧加箭头作为对象入口提示；计划分组框本身不响应点击。
...
- 计划行可展示一条 compact 执行态势条，复用 WorkPlan 执行项状态顺序和颜色：`已完成 / 已跳过 / 已阻塞 / 执行中 / 待执行`；态势条占满计划行宽度，态势段只用 hover / focus tooltip 显示数量，不在 WorkArea 卡片里展开执行项。
