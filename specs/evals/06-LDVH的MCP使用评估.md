...
| 事实源回写 | 回写目标判断和写入执行 | 不需要 MCP | 回写由 03 事实源边界和 05.01 受控写入原则决定，Tools 辅助层已覆盖 |
...
| 对象聚合和视图 | 不需要 MCP | — | 由 Tools 辅助层聚合和 Web 展示层覆盖 |
...
| 替代方案 | 使用 Trae 内置文件系统工具和 05.01 Tools 辅助层 |
...
1. MCP 工具能力受 05 工具基础规范约束，不得绕过Tools 辅助层的校验和受控写入边界（依据 `specs/05` §七）；
...
2. 05.01 Tools 辅助层稳定后，评估 Context7 输出是否需要纳入上下文包生成流程；
