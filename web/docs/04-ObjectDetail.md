...
2. 活跃/待关闭计划默认展开，每行作为进入 TaskPlan 的入口；已闭合计划默认折叠。活跃/待关闭组标题只作为静态组 header，不表现为按钮；只有可折叠的已闭合组展示折叠按钮状态。
3. 计划行展示计划标题、计划 ID、更新时间、状态、compact 任务态势条、复制路径和辅助阅读入口图标；态势条占满计划行宽度；计划标题使用 `ldvh-body`，不使用 `ldvh-card-title`，避免越过组标题和区块标题层级；成功标准、完成证据等计划关闭材料留在 TaskPlan 详情中表达。复制路径图标默认和 hover 都保持中性 ghost；辅助阅读图标默认中性，在计划行 hover 或当前右侧阅读已打开时才使用该计划组状态色。
4. WorkArea 详情页点击计划行只打开右侧辅助阅读区，不切换主路由到 TaskPlan 详情；主路由跳转只属于对象列表卡片。
5. 属性：按目标、范围、约束、来源展示 `description/scope/constraints/source`，不把 `scope` 放进顶部元信息 chip；页面大区块标题使用小圆点，降低装饰负担。属性属于核心定义事实，不默认折叠；多行内容渲染为语句列表，`包含` / `不包含` 等短前缀渲染为语义标签。
6. 关联材料：只有 `related_docs/related_adrs/related_memos/related_pitfalls` 非空时显示。
7. 关联材料行是 plain 引用行，不带状态色背景，也不再套小卡片边框；右侧复制路径和扩展阅读图标保持中性 ghost 表现，左侧对象类型图标、ID 或短标签承担对象类型识别。

## 5. TaskPlan 语义阅读布局

TaskPlan 不使用普通字段卡片堆叠，而作为“一次目标的执行态势”展示：

1. 属性：按目标、来源、所属工作域展示 `description/source/workarea`，标题表现与 WorkArea 的属性区块一致；工作域入口只打开右侧辅助阅读，不切换主路由。
2. 关闭判断：展示 `success_criteria`、关闭请求、完成证据和 closed 计划关闭时间的记录状态，并展示 `completion_evidence`；成功标准是关闭判断依据，不作为独立平级区块。
3. 任务拆解：展示整体 Task 态势条，并按与列表一致的队列顺序展示 Task 行；队列顺序与态势条空间方向对应，态势条最右侧状态在上，最左侧状态在下。
4. Task 行在 TaskPlan 详情中使用轻量行，保留状态图标、标题、ID、复制路径和辅助阅读入口，但不使用大面积状态色背景；有 SubTask 时只展示 compact 子任务态势条，不在 TaskPlan 详情展开子任务行。
