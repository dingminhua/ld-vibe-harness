...
source: ai
source_detail: 用户要求将本轮关于 specs 工作流程、WorkPlan、ExecutionItem、Memo 分流和事实源边界的长对话整理为 Study。
conclusion: |
...
- `TaskPlan` 改名为 `WorkPlan`，避免与 Codex、Claude、Trae 等环境中的运行时 plan 混淆；
- 独立 `Task` 移出 active 工作模型；
- `SubTask` 不再作为 active 工作模型；
- 原 Task 的执行编排职责迁入 `WorkPlan.orchestration.execution_items`；
- ExecutionItem 只作为 WorkPlan 内部恢复和编排节点；
- Review、Audit、Decision Review 是流程环节，不是工作模型；
- Human 主要审 WorkPlan 的目标、范围、成功标准、关闭判断和证据，不直接管理 AI 的内部执行项。
...
- 关键语义转折；
- 待分流方向；
- 与 WorkPlan、ADR、Study、Pitfall 或 docs 的关联。
...
1. `specs/21-WorkPlan-工作计划.md` 中的 `orchestration.execution_items` 字段是否过多、过少或命名不稳；
2. `mode: single | sequential | parallel | mixed` 是否应该是 WorkPlan 总体编排字段，还是只由 execution item 关系推导；
...
1. 精读并逐段核对 `specs/21-WorkPlan-工作计划.md`；
2. 重点检查 WorkPlan 准入、状态机、`orchestration`、ExecutionItem、review、验证证据和关闭证据；
3. 回看 `specs/05-工作模型基础规范.md`，确认 ExecutionItem 不进入 active 工作模型集合；
4. 回看 `specs/05.01-工作字段内容格式规范.md`，确认公共字段和证据字段边界；
5. 回看 `specs/06-工作流程基础规范.md`，确认计划、执行、自检、复检、Human Gate 和 Learn 的流程表达；
