...
| 能力 | API | 写入目标 | 允许字段 | 约束 |
|---|---|---|---|---|
| Memo 快速创建 | `POST /api/memos` | `ldvh-base/memos/memo-{NNNN}-{slug}.yaml` | `title`、`description`、`source`、`category`、`importance` | 只能创建 `status: draft` 的新 Memo；必须校验必填字段、分类和重要程度；必须写入 `status_history`；不得在创建后通过 Web 编辑 Memo 字段或状态 |
...
1. 写入前校验 `title`、`description`、`source`、`category`、`importance` 和 `status_history` 最小字段；
