...
LDVH_INSTALL_ACTION_TEMPLATE_REQUIRED_ROWS = {
    ...
    "交还": ["安装方式", "配置位置选择", "管辖项目 ID", "Git common-dir", "ldvh-base/", "事实源子目录状态", "环境入口状态", "`integrated` / `manual_ready` / `available` / `deferred` / `removed_top_level` / `absent`", "承接形态说明", "手动可用安装交还", "不会自动阻断", "不得交还进入 31", "用户告知清单", "验证摘要", "回滚或卸载入口", "下一步 Human Gate", "source_refs"],
}
...
LDVH_INSTALL_WIZARD_TERMS = [
    ...
    "不得新增安装运行时状态闭集",
    "不得把交互展示项写成环境入口状态",
    "具体图标、表格、状态列和交还摘要模板归 `code/docs/03-LDVH-Install-Wizard-Practice.md` 或运行时输出",
    "用户视角摘要",
    "简洁确认块",
    ...
    "deferred",
    "absent",
    "不得把 repo instruction、thin reference 或外部 adapter 候选写成环境接入状态",
    "不会自动阻断写入或完成声明",
    "当前 AI 运行环境名称",
    "不得沿用示例环境名称",
...
ENV_HOOK_ACCEPTANCE_ACTION_TEMPLATE_REQUIRED_ROWS = {
    ...
    "执行": ["逐项推进验收", "一次只判断一项", "闭集确认", "重启 App", "新窗口或新会话", "受控 scratch target", "不得安装", "不得升级", "不得修改用户环境", "失败即停止"],
...
ENV_HOOK_ACCEPTANCE_FLOW_TERMS = [
    ...
    "每一步只问一个判断",
    "闭集确认",
    "具体状态表、图标、编号和按钮文案归实现域",
    "具体卡片、表格、图标、编号和 scratch 文件名归 `code/docs/02-Environment-Plugin-Practice.md` 或运行时输出",
