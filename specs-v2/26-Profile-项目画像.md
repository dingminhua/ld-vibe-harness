...
> 相关规范：`specs-v2/00-LD-Vibe-Harness理念与纲要.md`、`specs-v2/01-目录说明.md`、`specs-v2/02-术语规范.md`、`specs-v2/03.04-工作模型文档规范.md`、`specs-v2/04.02-环境适配与运行投影规范.md`、`specs-v2/04.03-环境适配映射与运行投影记录模板.md`、`specs-v2/05.01-工作字段内容格式规范.md`、`specs-v2/07-Code实现规范.md`、`specs-v2/08-Web信息同步规范.md`、`specs-v2/09-事实源边界与承载规范.md`、`specs-v2/20-工作模型集合索引.md`、`specs-v2/22-Change-变更.md`、`specs-v2/23-Pitfall-踩坑.md`、`specs-v2/24-Intent-意图.md`、`specs-v2/25-Memo-备忘.md`、`specs-v2/27-Task-任务.md`
...
Profile 不是所有项目配置、环境能力、运行投影或平台设置的默认归宿。当前开发环境能力核验、环境适配映射、运行投影和降级方式，应由根目录 `LDVH-ENVIRONMENT-INITIALIZATION.md` 按 `specs-v2/04.03-环境适配映射与运行投影记录模板.md` 承载。Profile 可以引用该记录的位置和接入状态，但不得复制环境适配正文或替代该记录。
...
| 内容 | 权威位置 |
|---|---|
| Profile 工作模型规范 | `specs-v2/26-Profile-项目画像.md` |
| Profile 实例 | `ldvh-base/profiles/` |
| Profile 字段内容格式 | `specs-v2/05.01-工作字段内容格式规范.md` |
...
Profile 可以通过 `related_intents` 关联项目级目标、初始化目标或治理目标。Intent 的准入、状态和字段契约由 `specs-v2/24-Intent-意图.md` 定义。

Profile 不替代 Intent 的目标、成功标准、约束或完成判断。

### 4.2 Profile 与 Task

Profile 可以通过 `related_tasks` 关联项目接入、路径调整、初始化、审计或迁移 Task。Task 的准入、状态和字段契约由 `specs-v2/27-Task-任务.md` 定义。
...
Profile 可以通过 `related_pitfalls` 关联适用于该项目或来源于该项目的踩坑经验。Pitfall 的准入、状态和字段契约由 `specs-v2/23-Pitfall-踩坑.md` 定义。
...
Profile 的创建、状态变化、路径字段修改、环境初始化记录路径修改、归档和重命名都应留下 Change。Change 的 commit message 契约由 `specs-v2/22-Change-变更.md` 定义。

### 4.8 Profile 与 docs

Profile 可以通过 `related_docs` 关联 README、项目说明、初始化说明、需求文档或其他项目正文入口。docs 的文档规则由 `specs-v2/03.02-管辖项目文档规范.md` 和 `specs-v2/01-目录说明.md` 定义。
...
字段内容格式按 `specs-v2/05.01-工作字段内容格式规范.md` 执行。字段缺失、类型错误、状态非法、路径字段缺失、引用不存在、条件必填缺失或文件命名不匹配时，Code 应报告诊断，不得静默通过。
...
updated: 2026-06-09
description: |
  LDVH 自身项目，承载 specs-v2 规范体系、Code、Web 和 dogfood 事实实例。
...
| 确定性执行要求 | Profile 字段、状态、路径、文件命名、引用和环境记录路径应由 Code 校验或记录缺口 | `specs-v2/07-Code实现规范.md`、Profile 校验 Code、正反样例 | 校验实现 | 字段契约、状态机、路径规则、引用关系或环境记录边界变化时 |
...
1. Profile 校验 Code 待字段契约稳定后补齐正反样例；
2. Profile Web 名册、项目切换和环境初始化记录入口待 Web 实现规划时补齐；
3. Profile 创建、激活、暂停、归档和项目切换的具体工作流程待 40-59 迁入后承接；
4. `project_kind` 是否需要扩展为更细的项目类型，待多项目实践后评估；
5. 多项目集中名册与各管辖项目自有 Profile 的关系，待产品初始化和产品审计流程迁入后进一步校准。
