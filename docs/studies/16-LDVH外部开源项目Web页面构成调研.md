...
|---|---|---|---|
| mcp-shrimp-task-manager | React + Vite 管理后台 | `code/task-viewer` | 事实对象列表、详情、编辑、多层 Tab、历史视图 |
...
`mcp-shrimp-task-manager/code/task-viewer` 是一个 React + Vite 的任务查看器，依赖 React、React DOM、Vite、TanStack React Table 和 react-i18next。入口结构简洁：`index.html` 提供 root 挂载点，`src/main.jsx` 挂载 React 应用，`src/App.jsx` 承担应用级状态、路由式视图状态和数据加载逻辑。
...
1. 本文结论如何影响 LDVH Web 页面构成设计待 Web 信息同步规范稳定后确定；
2. 本文结论如何影响 LDVH Web 组件选型待工具规范稳定后确定。
