...
规范治理复核主动检查对象边界、状态、Study 与普通文件承接、evolution、来源结构、关系目标与循环、净价值、Human Gate 和 Stop Conditions。Code/字段复核检查 49 个 V3 Spark、12 个代表样本、字段分布、统一登记闭包、非全局结构成员准入和失败隔离。该历史数量以 commit `3d028ec2` 快照为准：49 个 Spark 中 46 个为 `pending`；它们只是设计与反例证据，不取得 V4 实例当前性。
