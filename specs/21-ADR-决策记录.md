...
> 创建日期：2026-05-30
> 定位：定义 ADR 决策记录工作模型，包括对象定位、准入条件、事实源边界、状态机、对象关系、Human Gate、字段契约、事实源回写、证据留存、适配原则、机制适配边界、落地初始化、落地审计和合规检查
...
机制适配边界含义、命名规则和状态骨架由各模型主规范自行声明。ADR 对象模型已为现有机制承接入口形成显式结论；该结论表示历史机制拆分文件状态已声明，不等同于 ADR 对象实例事实源、Tools 测试或项目落地初始化已经完成。
...
21.01-21.06 为历史机制文件，后续应逐步评估、合并或迁移；不得再作为 ADR 完整性的默认要求。后续新增、删除、重命名或改变机制适配边界状态，应通过 Human Gate 确认后处理。若机制承接状态、实体实现、测试验收或项目实际落地状态不一致，应先重新执行落地前决策和价值与要素审查，再进入初始化或整改。
...
4. 是否需要更新 ADR 的 Rules、Skill、Agent、Tools、Web 或 Contract 机制承接状态；状态应与实际文件和落地情况一致。
...
当 ADR 主模型规范已经存在，但项目实际落地状态不明、过期、部分完成或与机制承接状态声明不一致时，应重新执行落地前决策。重新执行时应明确当前属于首次落地、补齐落地、重新初始化、审计整改或暂缓落地中的哪一种场景。
...
11. 确认 21.01-21.06 现有机制承接入口均有显式结论，且对应机制文件存在；
12. 确认已创建、暂缓或不创建的 Trae Skill、Agent、Tools 脚本和 Web 页面均在对应机制文件中说明；
...
ADR 落地初始化可分为首次落地初始化、补齐落地初始化、重新初始化和审计整改初始化。无论属于哪一种场景，都必须先完成落地前决策和价值与要素审查；不得仅因主规范、机制承接文件或 Tools 脚本存在，就声称 ADR 对象模型已经在项目中完成落地。
...
12. 21.01-21.06 现有机制承接入口是否均有显式状态结论，且不存在 planned 状态；
13. 机制适配边界是否存在、非空、不替代主模型；
14. active 机制承接涉及实体实现时是否说明实体路径、当前可用能力、使用边界和验证方式；
...
18. active-prep 和 not-needed 机制承接是否说明当前状态、原因、前提和检查项；
19. 识别初始化缺口、过期实践和需要整改的机制适配边界。

落地审计结论应明确 ADR 当前处于未初始化、部分初始化、机制承接已声明但事实源未建立、Tools 能力与测试验收不一致、已初始化但存在整改项或已完成初始化中的哪一种状态。审计发现不应直接改写为完成状态；需要补齐时，应回到落地前决策、价值与要素审查和 Human Gate。
...
5. ADR 事实源边界是否符合本文 §4 和 04 事实源边界规范；
...
14. ADR 机制适配边界命名和状态是否与实际文件一致，是否存在对应机制文件且经 Human Gate 确认。
