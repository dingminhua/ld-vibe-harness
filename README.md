...
LDVH 面向不同 AI 开发环境提供同一条用户流程：

> 获取 → 安装 → 配置 → 接入 → 验证

当前仓库尚未声明已经发布到公共 registry；用户可以从源码构建本地 wheel/sdist，再安装到独立 Python 环境。安装后先配置显式 workspace 和管辖项目，运行只读 `ldvh-doctor` 检查 LDVH 自有入口，再把[环境无关接入说明](docs/启用与AI环境接入.md)交给目标 AI 开发环境中的 AI，由它依据该环境当前权威资料研究自动扩展或事件机制、形成方案并完成真实验证。

LDVH 不预先维护所有厂商的配置步骤或兼容矩阵。AI 开发环境是否支持完整接入按功能判断：只有权威资料或实际环境肯定证明不存在能够自动调用所需 LDVH 入口并承接必需输入与反馈的机制时，才是 `unsupported`；未调查、资料未找到、权限不足或不可观察均为 `unverified`。当前发行物的环境无关入口、参数和反馈边界见 [LDVH 接入面](docs/LDVH接入面.md)。
