...
5. Tools 辅助程序可在 Human Gate 已确认、操作意图已由 AI 或人明确给出、且满足 Contract 校验时，执行可机械化、可校验的受控写入。
6. Tools 辅助程序写入 ADR 时应校验字段完整性、状态合法性、引用有效性、写入授权和 Change 记录要求。
7. Tools 辅助程序不得自行判断 ADR 准入、生成授权、替代流程性判断或自动执行 ADR 状态流转中需要 Human Gate 确认的操作。
8. 当前 ADR Tools 受控写入能力是否实现，由 21.04-Tools.md 声明；未声明实现前，不得假定 Tools 已具备写入能力。
...
附件型实践子文档含义和命名规则见 `specs/04-LDVH模型子文档规范.md`。ADR 对象模型已按六槽位完整实践形成以下子文档结论：
...
1. 是否需要在 Rules 中增加 ADR 准入条件入口摘要；若涉及所有接入项目或事实实例编辑通用约束，应评估 L0；若仅涉及单项目或特定场景，应评估 L1 或 L2。
2. 是否需要更新 ADR 的 Rules、Skill、Agent、Tools、Web 或 Contract 子文档状态；状态应符合 `specs/04-LDVH模型子文档规范.md` 的六槽位完整实践要求。
3. 是否需要创建、更新或说明 ADR 的 Trae Skill、Agent、Tools 脚本、Web 页面或其他实体实现。
4. 是否需要 Tools 辅助程序覆盖 ADR 字段完整性校验和状态合法性校验。
...
8. 确认 21.01-21.06 六个子文档槽位均有显式结论，且对应子文档存在；
9. 确认已创建、暂缓或不创建的 Trae Skill、Agent、Tools 脚本和 Web 页面均在对应子文档中说明；
10. 确认 Tools 辅助程序已覆盖 ADR 字段完整性校验和状态合法性校验；
11. 确认 ADR Tools 已创建或更新对应自动化测试，并已执行测试验收、记录测试命令和结果；
12. 记录暂缓项和初始化产物。
...
10. 21.01-21.06 六个子文档槽位是否均有显式状态结论，且不存在 planned 状态；
11. 附件型实践子文档是否存在、非空、不替代主模型；
12. active 子文档涉及实体实现时是否说明实体路径、当前可用能力、使用边界和验证方式；
13. ADR Tools 是否存在对应自动化测试，是否已执行测试验收并记录测试命令和结果；
14. active-prep 和 not-created 子文档是否说明当前状态、原因、前提和检查项；
15. 识别初始化缺口、过期实践和需要整改的附件型实践子文档。
