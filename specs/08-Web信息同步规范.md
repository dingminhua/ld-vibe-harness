...
> 文件状态：active；本文吸收 V2 Web 父层规则。Web API 数据契约、同源独立读取、来源回指、缓存边界和基础回归测试已迁入 `tests/web`；本文不把具体页面设计、缓存实现细节、DTO 长表或视觉回归矩阵写成规则正文。

## 1. 价值判断

本文存在的价值，是让 Human 能看见 AI 行动所依据的状态、风险、证据、验证结果、回写位置和待确认事项，从而在 Human Gate、验收、风险接受或方向校正中给出可被 AI 消费的明确回复。

Web 主要服务 V5 门禁确认、V6 强制验证、V7 格式回写和 V8 范式交互，并支撑 V1、V2 和 V3：它把事实源状态、风险、证据、验证结果和回写边界以适合场景的范式呈现给 Human，帮助 AI 获得明确、可解析、可回指的确认结果。Web 的直接服务对象是 Human，但它仍服务 00 的 AI 第一目标；Web 页面状态不是事实源，也不替代 AI 判断或 Human Gate。
...
1. V2 `05.Att.*` 中 DTO/API、来源回指、缓存边界和 Spark quick create 轻写入回归已由 Web 实现域和 `tests/web` 承接；未形成正式授权的长表不在当前工作树保留，只能作为审计追溯、争议复核或历史取证材料，不作为当前 Web 规则或验收依据；
