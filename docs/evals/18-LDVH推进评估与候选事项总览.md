...
| Code 方向正确 | 结构校验、对象校验、commit 校验已部分覆盖 | 确定性反馈 |
...
| 规范落地要求缺全局聚合视图 | AI 判断承接状态仍需手工跨文件聚合 | 可见状态、可用工具 |
...
以下为按 `docs/specs/41-landing-orchestration-规范落地统筹.md` 执行审计后的主要缺口（审计范围：37 篇正式规范，157 条规范落地要求）。

| 编号 | 缺口 | 状态 | 影响 | 建议承接 |
|---|---|---|---|---|
| G-01 | 缺少规范落地要求全局聚合报告命令 | open | AI 需手工跨文件聚合 157 条要求，容易遗漏 | Code：扩展 `tools/specs_validate.py`，输出 landing report |
...
| 反合理化红旗与失败暂停 | 06、10、12、26、47、53 | 不得用"应该通过""看起来没问题"等替代验证证据；连续失败应回到根因调查、Human Gate、Memo、Pitfall、ADR 或子 Task |
| closure_evidence 结构化证据层 | 05.01、10、26、47、08 | 优先增强 Task 关闭证据和 Web 展示，不恢复独立 Evidence 工作模型 |
...
| 渐进式上下文加载与 Context Pack | 06、07、10、40、54 | 先索引定位，再按需读取；Context Pack 是派生输入，不成为事实源 |
...
2. **扩展规范落地要求聚合报告**：扩展 `tools/specs_validate.py`，生成 landing report，按类型分组，标记 closed/open/degraded/needs_human_gate/not_required，输出可读报告或 JSON。
...
5. 后续 Code 聚合报告实现后，应重新生成 §4 的统计结果，避免长期依赖手工聚合；
6. Dogfood 42/43 后，应回看 §4 缺口清单并标注哪些已被正式承接；
7. 本文中的稳定结论被正式规范或工作流程吸收后，应标注已吸收状态。
