"""Source-complete action-semantic trigger calibration paired experiment helpers.

The v6 experiment compares two calibration arms under the same activation
projection: calibration-baseline (the v5 wide-trigger recall-first
policy) and calibration-enhanced (the v6 action-semantic
trigger policy: signal + action-anchor, no veto).  Both arms share the
identical task/gold set and the identical activation-projected card
presentation; the only difference is the recall-layer trigger boundary.
"""
...
# v6 activation trace field set (experiment-internal, not written to production)
...
SCHEMA_VERSION = "ldvh-knowledge-precheck-v6/1"
EVIDENCE_SCHEMA_VERSION = "ldvh-knowledge-precheck-v6-evidence/1"
CONDITIONS = ("calibration-baseline", "calibration-enhanced")
NATIVE_SUBAGENT_RUNTIME = "codex-native-subagent"
ACTIVATION_TRACE_SCHEMA_VERSION = "ldvh-knowledge-precheck-v6-activation-trace/1"
...
class KnowledgePrecheckV6Error(ValueError):
    """Raised when the frozen v6 protocol or evidence fails closed."""


def _one_line(value: object, field: str) -> str:
    if not isinstance(value, str) or not value.strip() or "\n" in value or "\r" in value:
        raise KnowledgePrecheckV6Error(f"{field} must be one non-empty line")
    return value


def _hex64(value: object, field: str) -> str:
    if not isinstance(value, str) or _HEX64.fullmatch(value) is None:
        raise KnowledgePrecheckV6Error(f"{field} must be lowercase SHA-256")
    return value


def _closed_mapping(value: object, fields: frozenset[str], label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping) or set(value) != fields:
        raise KnowledgePrecheckV6Error(f"{label} fields are not closed")
    return value


def _unique_strings(value: object, field: str) -> list[str]:
    if not isinstance(value, list) or any(not isinstance(item, str) or not item for item in value):
        raise KnowledgePrecheckV6Error(f"{field} must be an array of non-empty strings")
    if len(value) != len(set(value)):
        raise KnowledgePrecheckV6Error(f"{field} must be unique")
    return value
...
def _validate_gateway_request(operation: str, request: Mapping[str, Any]) -> None:
    try:
        ...
        raise KnowledgePrecheckV6Error(str(error)) from error


class ReadOnlyKnowledgeGateway:
    """Fail-closed v6 gateway with dual-card-set F2 validation and projection.
    ...
    - ``calibration-baseline`` returns ``family_cards_baseline`` (original
      knowledge-body attribute cards).
    - ``calibration-enhanced`` returns ``family_cards_activation`` (activation-
    ...
    def __init__(
        ...
        *,
        ...
        if expected_object_set_fingerprint is None and expected_f2_cards_baseline is None:
            raise KnowledgePrecheckV6Error("gateway needs a frozen F2 source binding")
        ...
        if self._expected_f2_cards_baseline and set(self._expected_f2_cards_baseline) != set(TASK_FAMILIES):
            raise KnowledgePrecheckV6Error("frozen baseline F2 cards must cover every knowledge family")
        ...
        if self._expected_f2_cards_activation and set(self._expected_f2_cards_activation) != set(TASK_FAMILIES):
            raise KnowledgePrecheckV6Error("frozen activation F2 cards must cover every knowledge family")
    ...
    def call(
        ...
        *,
        ...
        if any(exchange["exchange_id"] == exchange_id for exchange in self._exchanges):
            raise KnowledgePrecheckV6Error("exchange_id must be unique")
        ...
        if not isinstance(response_bytes, bytes):
            raise KnowledgePrecheckV6Error("dispatch must return bytes")
        try:
            response = json.loads(response_bytes)
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            raise KnowledgePrecheckV6Error("Helper response is not UTF-8 JSON") from error
        if not isinstance(response, Mapping):
            raise KnowledgePrecheckV6Error("Helper response must be an object")
        if response.get("operation_key") != operation or response.get("outcome") != "ok":
            raise KnowledgePrecheckV6Error("Helper response operation/outcome mismatch")
        if response.get("changes") != []:
            raise KnowledgePrecheckV6Error("knowledge gateway observed a state-changing response")
        ...
        if not isinstance(result, Mapping):
            raise KnowledgePrecheckV6Error("Helper response has no result object")
        if operation == "find-fact-object-candidates":
            ...
            if not isinstance(raw_coverage, Mapping) or raw_coverage.get("status") != "complete":
                raise KnowledgePrecheckV6Error("F2 coverage is not complete")
            if not isinstance(cards, list):
                raise KnowledgePrecheckV6Error("F2 cards must be an array")
            ...
            if self._expected_f2_cards_baseline:
                ...
                for card in cards:
                    if not isinstance(card, Mapping):
                        raise KnowledgePrecheckV6Error("F2 card must be an object")
                    fields = card.get("fields")
                    if not isinstance(fields, Mapping) or not isinstance(fields.get("object_uid"), str):
                        raise KnowledgePrecheckV6Error("F2 card has no frozen object identity")
                ...
                if expected_baseline is not None:
                    ...
                    if observed_fields != expected_baseline_fields:
                        raise KnowledgePrecheckV6Error(f"F2 {family} knowledge cards drifted")
                # Then project at runtime according to the condition:
                # calibration-enhanced replaces the card fields with the frozen
                # activation-projected fields (with a secondary drift check on
                # the projection set); calibration-baseline keeps baseline fields.
                if card_layer == "calibration-enhanced":
                    projected = self._expected_f2_cards_activation.get(family)
                    if projected is None:
                        raise KnowledgePrecheckV6Error(f"F2 {family} has no frozen activation projection")
                    projected_response = []
                ...
                raise KnowledgePrecheckV6Error("F2 object set drifted")
            ...
            for card in cards:
                if not isinstance(card, Mapping):
                    raise KnowledgePrecheckV6Error("F2 card is not an object")
                ...
                if not isinstance(card_match_reasons, list):
                    raise KnowledgePrecheckV6Error("F2 card has no match reasons")
                match_reasons.extend(deepcopy(card_match_reasons))
        else:
            items = result.get("items")
            if not isinstance(items, list) or not items:
                raise KnowledgePrecheckV6Error("F3 read returned no items")
            for item in items:
                if not isinstance(item, Mapping) or item.get("check_status") != "mechanically_valid":
                    raise KnowledgePrecheckV6Error("F3 item is not mechanically valid")
                fact_object = item.get("fact_object")
                if not isinstance(fact_object, Mapping):
                    raise KnowledgePrecheckV6Error("F3 item has no fact object")
                ...
                if not isinstance(identity, Mapping) or not isinstance(resolved_ref, Mapping):
                    raise KnowledgePrecheckV6Error("F3 item has no normalized identity")
                uid = _one_line(resolved_ref.get("object_uid"), "F3 object_uid")
                if identity.get("object_uid") not in (None, uid):
                    raise KnowledgePrecheckV6Error("F3 fact identity does not match resolved ref")
                fact_type_key = _one_line(identity.get("fact_type_key"), "F3 fact_type_key")
                if fact_type_key not in TASK_FAMILIES:
                    raise KnowledgePrecheckV6Error("F3 object type is not a knowledge family")
                ...
                if self._expected_f3_fingerprints and expected is None:
                    raise KnowledgePrecheckV6Error(f"F3 object is not frozen: {uid}")
                if expected is not None and fingerprint != expected:
                    raise KnowledgePrecheckV6Error(f"F3 source drifted: {uid}")
                f3_objects.append(
                    {
                        "object_uid": uid,
            ...
            if len(returned_uids) != len(set(returned_uids)) or set(returned_uids) != set(requested_uids):
                raise KnowledgePrecheckV6Error("F3 response does not exactly match requested refs")
...
def validate_protocol(protocol: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if not isinstance(source, Mapping):
        ...
        except KnowledgePrecheckV6Error:
            problems.append("source-snapshot")
    ...
    if not isinstance(orders, list) or len(orders) != RETAINED_PAIR_TARGET:
        ...
        {("calibration-baseline", "calibration-enhanced"): 9, ("calibration-enhanced", "calibration-baseline"): 9}
    ):
...
def build_model_input_packet(
    ...
    *,
    ...
    if problems:
        raise KnowledgePrecheckV6Error(f"protocol is invalid: {','.join(problems)}")
    if condition not in CONDITIONS:
        raise KnowledgePrecheckV6Error("condition is not frozen")
    tasks = {task["pair_id"]: task for task in protocol["tasks"]}
    if pair_id not in tasks:
        raise KnowledgePrecheckV6Error("pair_id is not frozen")
    _one_line(attempt_id, "attempt_id")
...
def validate_model_input_packet(packet: Mapping[str, Any], protocol: Mapping[str, Any]) -> None:
    ...
    if packet.get("schema_version") != SCHEMA_VERSION:
        raise KnowledgePrecheckV6Error("model input schema is invalid")
    ...
    if packet != rebuilt:
        raise KnowledgePrecheckV6Error("model input is not the deterministic frozen projection")
...
def build_model_input_packet_unchecked(
    ...
    *,
    ...
    if pair_id not in tasks or condition not in CONDITIONS:
        raise KnowledgePrecheckV6Error("model input binding is not frozen")
    _one_line(attempt_id, "attempt_id")
    _hex64(fresh_context_id_hash, "fresh_context_id_hash")
    task = tasks[pair_id]
    policy = protocol["policies"][condition]
...
def condition_from_packet(packet: Mapping[str, Any], protocol: Mapping[str, Any]) -> str:
    ...
    if not isinstance(card_layer, str) or card_layer not in CONDITIONS:
        raise KnowledgePrecheckV6Error("model input card_layer does not identify one frozen arm")
    return card_layer
...
def validate_trigger_response(response: Mapping[str, Any], family: str) -> None:
    ...
    if type(triggered) is not bool:
        raise KnowledgePrecheckV6Error("triggered must be boolean")
    ...
    if (triggered and trigger_family != family) or (not triggered and trigger_family is not None):
        raise KnowledgePrecheckV6Error("trigger family is inconsistent")
    ...
    if triggered and (not positives or vetoes):
        raise KnowledgePrecheckV6Error("a positive trigger needs reasons and no veto")
    if not triggered and positives and not vetoes:
        raise KnowledgePrecheckV6Error("a vetoed positive signal must name a veto")
...
def build_trigger_trace(
    ...
    """v6-specific trigger trace that resolves condition from card_layer."""
...
def validate_trigger_trace(
    ...
    """v6-specific trigger trace validation that resolves condition from card_layer."""
    if set(trace) != frozenset(
        ...
        raise KnowledgePrecheckV6Error("trigger trace fields are not closed")
    for field in ("attempt_id", "pair_id", "family"):
        if trace.get(field) != packet.get(field):
            raise KnowledgePrecheckV6Error(f"trigger trace {field} binding is invalid")
    if protocol is not None and trace.get("condition") != condition_from_packet(packet, protocol):
        raise KnowledgePrecheckV6Error("trigger trace condition binding is invalid")
    if trace.get("schema_version") != EVIDENCE_SCHEMA_VERSION:
        raise KnowledgePrecheckV6Error("trigger trace schema is invalid")
...
def validate_model_output(output: Mapping[str, Any], protocol: Mapping[str, Any]) -> None:
    """v6-specific model output validation using v6 CONDITIONS."""
    ...
    if output.get("pair_id") not in tasks or output.get("condition") not in CONDITIONS:
        raise KnowledgePrecheckV6Error("model output binding is not frozen")
    ...
    if output.get("model_name") is not None or output.get("agent_runtime_name") != NATIVE_SUBAGENT_RUNTIME:
        raise KnowledgePrecheckV6Error("member identity must remain the frozen native-subagent identity")
    ...
    if output.get("usage") != "unavailable" or output.get("latency") != "unavailable":
        raise KnowledgePrecheckV6Error("unobservable usage/latency must remain unavailable")
...
def is_valid_structured_response(
    ...
    *,
    ...
    """v6-specific check using v6 CONDITIONS."""
    candidate = {
        "attempt_id": packet["attempt_id"],
        "pair_id": packet["pair_id"],
        "condition": condition_from_packet(packet, protocol),
...
def validate_f3_decision_response(
    ...
    *,
    ...
    """v6-specific F3 decision validation.

    The frozen v6 snapshot maps unclassified knowledge refs to ``"unknown"``
    ...
    if len(refs) > 2:
        raise KnowledgePrecheckV6Error("member F3 decision exceeds the two-ref ceiling")
    for uid in refs:
        known = snapshot_families.get(uid)
        if known not in (None, "unknown") and known != family:
            raise KnowledgePrecheckV6Error("member F3 decision crossed the frozen task family")
    return refs
...
def validate_technical_failure_response(
    ...
    *,
    ...
    """v6-specific technical failure validation using v6 CONDITIONS."""
    if exclusion_code not in TECHNICAL_EXCLUSION_CODES:
        raise KnowledgePrecheckV6Error("replacement uses a nontechnical exclusion")
    if exclusion_code != "missing_structured_output":
        return
    if not is_valid_structured_response(response, packet=packet, protocol=protocol):
        return
    raise KnowledgePrecheckV6Error("technical failure response is a valid structured output")
    ...
    *,
    ...
    for exchange in exchanges:
        ...
        try:
            ...
            raise KnowledgePrecheckV6Error("F2 raw response cannot be replayed") from error
        if not isinstance(cards, list):
            raise KnowledgePrecheckV6Error("F2 raw response has no cards")
        f2_cards.extend(cards)
        for card in cards:
            if not isinstance(card, Mapping):
                raise KnowledgePrecheckV6Error("F2 raw response card is invalid")
    ...
    for exchange in exchanges:
        ...
        for obj in exchange.get("f3_objects", []):
            if not isinstance(obj, Mapping):
                raise KnowledgePrecheckV6Error("F3 replay object is invalid")
...
def validate_activation_trace(trace: Mapping[str, Any]) -> None:
    ...
    if trace.get("schema_version") != ACTIVATION_TRACE_SCHEMA_VERSION:
        raise KnowledgePrecheckV6Error("activation trace schema version is invalid")
    ...
    if trace.get("condition") not in CONDITIONS:
        raise KnowledgePrecheckV6Error("activation trace condition is not frozen")
    hits = trace.get("activation_hits")
    if not isinstance(hits, list):
        raise KnowledgePrecheckV6Error("activation trace hits must be a list")
    for hit in hits:
        ...
        if not isinstance(hit.get("hit_stage"), str) or hit["hit_stage"] not in ("trigger", "f2", "f3"):
            raise KnowledgePrecheckV6Error("activation hit_stage must be trigger|f2|f3")
        if hit.get("triggered") is not True:
            raise KnowledgePrecheckV6Error("activation hit triggered must be true")
    delta = trace.get("action_delta")
    if not isinstance(delta, Mapping):
        raise KnowledgePrecheckV6Error("activation trace action_delta must be an object")
    _closed_mapping(delta, _ACTION_DELTA_FIELDS, "action delta")
    if type(delta.get("action_changed_vs_gold")) is not bool:
        raise KnowledgePrecheckV6Error("action_delta.action_changed_vs_gold must be boolean")
...
def validate_score(score: Mapping[str, Any]) -> None:
    ...
    if score.get("scorer_model_name") is not None or score.get("scorer_runtime_name") != NATIVE_SUBAGENT_RUNTIME:
        raise KnowledgePrecheckV6Error("scorer identity must remain the frozen native-subagent identity")
    action_bridge = score.get("action_bridge_score")
    if not isinstance(action_bridge, int) or action_bridge not in (0, 1, 2, 3):
        raise KnowledgePrecheckV6Error("action_bridge_score must be an integer 0-3")
...
def validate_scorer_response(response: Mapping[str, Any]) -> None:
    ...
    if response.get("condition_blind_attested") is not True:
        raise KnowledgePrecheckV6Error("scorer must attest condition blindness")
    for field in (
        "knowledge_adjusted_first_legal_action_correct",
        "action_changed",
        "strong_reuse",
        ...
        if type(response.get(field)) is not bool:
            raise KnowledgePrecheckV6Error(f"{field} must be boolean")
    ...
    if response.get("strong_reuse") and (
        ...
        raise KnowledgePrecheckV6Error("strong reuse scorer response is internally inconsistent")
...
def _source_snapshot_observation_hashes(root: Path, snapshot: Mapping[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(raw, Mapping):
        raise KnowledgePrecheckV6Error("source observation hashes are missing")
    return dict(raw)
...
def _validate_source_observations(
    ...
    # v6 source observations: one F2 response per family plus the frozen
    # F3 fingerprints response (request files and per-family F3 responses
    # are not part of the v6 artifact set).
    ...
    if set(by_name) != expected_names:
        raise KnowledgePrecheckV6Error("source observation names are not the frozen v6 F2/fingerprints set")
    ...
    if (
        ...
        raise KnowledgePrecheckV6Error("source snapshot family projection is incomplete")
    ...
    for family in TASK_FAMILIES:
        ...
        if (
            ...
            raise KnowledgePrecheckV6Error(f"{family} source F2 response is invalid")
        ...
        if (
            ...
            raise KnowledgePrecheckV6Error(f"{family} source F2 coverage is invalid")
        card_uids: list[str] = []
        baseline_cards: list[Mapping[str, Any]] = []
        for card in cards:
            if not isinstance(card, Mapping) or not isinstance(card.get("fields"), Mapping):
                raise KnowledgePrecheckV6Error(f"{family} source F2 card is invalid")
            fields = card["fields"]
            ...
            if (
                ...
                raise KnowledgePrecheckV6Error(f"{family} source F2 card identity is invalid")
        ...
        if baseline_cards != family_cards_baseline[family] or len(card_uids) != len(set(card_uids)):
            raise KnowledgePrecheckV6Error(f"{family} source F2 baseline cards differ from the snapshot")
        ...
        if not isinstance(activation_cards, list) or len(activation_cards) != len(card_uids):
            raise KnowledgePrecheckV6Error(f"{family} source F2 activation cards differ in count from the snapshot")
        activation_uids: list[str] = []
        for card in activation_cards:
            if not isinstance(card, Mapping):
                raise KnowledgePrecheckV6Error(f"{family} activation card is invalid")
            activation_uid, object_id = _card_object_identity(card)
            if object_id is None:
                raise KnowledgePrecheckV6Error(f"{family} activation card has no object_id")
            activation_uids.append(activation_uid)
        if sorted(activation_uids) != sorted(card_uids):
            raise KnowledgePrecheckV6Error(f"{family} activation projection does not bind the frozen baseline set")
    ...
    if (
        ...
        raise KnowledgePrecheckV6Error("source F3 fingerprints response is invalid")
    ...
    if not isinstance(items, list) or len(items) != len(fingerprints):
        raise KnowledgePrecheckV6Error("source F3 fingerprints coverage is incomplete")
    for item in items:
        if not isinstance(item, Mapping):
            raise KnowledgePrecheckV6Error("source F3 fingerprint item is invalid")
        ...
        if uid not in fingerprints:
            raise KnowledgePrecheckV6Error(f"source F3 fingerprint references unknown UID: {uid}")
...
def _read_json(root: Path, relative: str) -> Any:
    try:
        ...
        raise KnowledgePrecheckV6Error(f"cannot read evidence file: {relative}") from error
...
def _timestamp(value: object, field: str) -> datetime:
    ...
    try:
        ...
        raise KnowledgePrecheckV6Error(f"{field} must be RFC3339") from error


def _manifest_files(manifest: Mapping[str, Any]) -> dict[str, str]:
    raw = manifest.get("files")
    if not isinstance(raw, Mapping) or not raw:
        raise KnowledgePrecheckV6Error("manifest files must be a non-empty object")
    files: dict[str, str] = {}
    for relative, digest in raw.items():
        path = _one_line(relative, "manifest path")
        if Path(path).is_absolute() or ".." in Path(path).parts:
            raise KnowledgePrecheckV6Error("manifest path escapes the bundle")
...
def compile_evidence_bundle(bundle_root: Path) -> dict[str, bytes]:
    ...
    if not isinstance(manifest, Mapping) or set(manifest) != _MANIFEST_FIELDS:
        raise KnowledgePrecheckV6Error("evidence manifest fields are invalid")
    if manifest.get("schema_version") != EVIDENCE_SCHEMA_VERSION:
        raise KnowledgePrecheckV6Error("evidence manifest schema is invalid")
    files = _manifest_files(manifest)
    actual = {
        str(path.relative_to(root)) for path in root.rglob("*") if path.is_file() and path.name != "manifest.json"
    }
    if actual != set(files):
        raise KnowledgePrecheckV6Error(
    ...
    for relative, digest in files.items():
        if bytes_sha256((root / relative).read_bytes()) != digest:
            raise KnowledgePrecheckV6Error(f"bundle hash mismatch: {relative}")

    protocol = _read_json(root, str(manifest["protocol_file"]))
    if not isinstance(protocol, Mapping) or validate_protocol(protocol):
        raise KnowledgePrecheckV6Error("frozen protocol is invalid")
    ...
    if not isinstance(snapshot, Mapping) or set(snapshot) != _SOURCE_SNAPSHOT_FIELDS:
        raise KnowledgePrecheckV6Error("source snapshot fields are not closed")
    if (
        snapshot.get("schema_version") != "ldvh-knowledge-precheck-v6-source-snapshot/1"
        or snapshot.get("governed_project_id") != "ldvh"
    ):
        raise KnowledgePrecheckV6Error("source snapshot identity is invalid")
    ...
    if canonical_sha256(snapshot) != protocol["source_snapshot"]["content_sha256"]:
        raise KnowledgePrecheckV6Error("source snapshot hash mismatch")
    if snapshot.get("object_set_fingerprint") != protocol["source_snapshot"]["object_set_fingerprint"]:
        raise KnowledgePrecheckV6Error("source snapshot object set fingerprint mismatch")
    ...
    if not isinstance(observation_files, list) or observation_files != sorted(observation_files):
        raise KnowledgePrecheckV6Error("source observation file list is invalid")
    ...
    if expected_observations:
        ...
        if set(observed_names) != set(expected_observations):
            raise KnowledgePrecheckV6Error("source observation coverage is incomplete")
        for path, name in zip(observation_files, observed_names, strict=True):
            if bytes_sha256((root / path).read_bytes()) != expected_observations[name]:
                raise KnowledgePrecheckV6Error(f"source observation drifted: {name}")
    ...
    if not isinstance(fingerprints, Mapping) or not isinstance(families, Mapping) or set(fingerprints) != set(families):
        raise KnowledgePrecheckV6Error("knowledge reference snapshot is incomplete")
    ...
    for task in tasks.values():
        ...
        if any(ref not in fingerprints for ref in all_refs):
            raise KnowledgePrecheckV6Error("task gold references an unfrozen object")
        if any(families[ref] not in (None, "unknown") and families[ref] != task["family"] for ref in all_refs):
            raise KnowledgePrecheckV6Error("task gold reference family is inconsistent")
        ...
        if set(source_refs) != set(all_refs) or any(source_refs[ref] != fingerprints[ref] for ref in all_refs):
            raise KnowledgePrecheckV6Error("task source refs do not exactly bind the frozen gold boundary")
    denylist = set(protocol["artifact_denylist"])
    family_cards_baseline = snapshot.get("family_cards_baseline")
    family_cards_activation = snapshot.get("family_cards_activation")

    return _compile_evidence_bundle_v6(
        root,
        manifest,
        protocol,
        snapshot,
...
def _compile_evidence_bundle_v6(
    root: Path,
    ...
    """Deterministically rebuild v6 records, results, and report from sealed evidence."""
    ...
    if not isinstance(ledger, Mapping) or set(ledger) != {
        ...
        raise KnowledgePrecheckV6Error("attempt ledger shape is invalid")
    ...
    if type(pair_attempts) is not int or not RETAINED_PAIR_TARGET <= pair_attempts <= MAX_PAIR_ATTEMPTS:
        raise KnowledgePrecheckV6Error("pair attempt ceiling is invalid")
    if not isinstance(replacements, list) or len(replacements) > MAX_REPLACEMENTS:
        raise KnowledgePrecheckV6Error("replacement ceiling is invalid")
    if pair_attempts != RETAINED_PAIR_TARGET + len(replacements):
        raise KnowledgePrecheckV6Error("pair attempt/replacement accounting is inconsistent")
    ...
    for replacement in replacements:
        if not isinstance(replacement, Mapping) or set(replacement) != {
            ...
            raise KnowledgePrecheckV6Error("replacement entry shape is invalid")
        ...
        if (
            ...
            raise KnowledgePrecheckV6Error("replacement attempt binding is invalid")
        attempt_group = tuple(attempt_ids)
        if attempt_group in replacement_attempt_groups:
            raise KnowledgePrecheckV6Error("replacement attempt binding is duplicated")
    ...
    if (
        ...
        raise KnowledgePrecheckV6Error("member launch ceiling is invalid")
    if (
        ...
        raise KnowledgePrecheckV6Error("scorer context ceiling is invalid")
    ...
    for index, launch in enumerate(launch_rows, start=1):
        if not isinstance(launch, Mapping) or set(launch) != {
            ...
            raise KnowledgePrecheckV6Error("member launch row shape is invalid")
        if launch["sequence"] != index:
            raise KnowledgePrecheckV6Error("member launch sequence is not contiguous")
        attempt_id = _one_line(launch["attempt_id"], "launch attempt_id")
        if attempt_id in launch_by_attempt:
            raise KnowledgePrecheckV6Error("member launch attempt id is duplicated")
        ...
        if elapsed < 0 or elapsed > RUN_TIMEOUT_SECONDS:
            raise KnowledgePrecheckV6Error("member run exceeded its frozen timeout")
        ...
        if pair_id not in tasks or launch["condition"] not in CONDITIONS:
            raise KnowledgePrecheckV6Error("member launch pair or condition is not frozen")
    ...
    for index, pair_id in enumerate(tasks):
        ...
        if [str(row["condition"]) for row in rows] != expected_order * waves:
            raise KnowledgePrecheckV6Error("member runs violate frozen condition order")
        chunks = [rows[offset : offset + len(CONDITIONS)] for offset in range(0, len(rows), len(CONDITIONS))]
        if len(chunks) != waves:
            raise KnowledgePrecheckV6Error("member pair-attempt accounting is inconsistent")
        for chunk in chunks[:-1]:
            ...
            if replacement is None or replacement["pair_id"] != pair_id:
                raise KnowledgePrecheckV6Error("excluded whole-pair launch binding is incomplete")
    ...
    if set(pair_launches) != set(tasks) or observed_replacement_groups != set(replacement_attempt_groups):
        raise KnowledgePrecheckV6Error("replacement/launch coverage is inconsistent")
    if excluded_attempt_ids & retained_attempt_ids or len(retained_attempt_ids) != expected_members:
        raise KnowledgePrecheckV6Error("retained and excluded member launch sets are invalid")
    ...
    for packet in model_inputs:
        if not isinstance(packet, Mapping):
            raise KnowledgePrecheckV6Error("model input must be an object")
        validate_model_input_packet(packet, protocol)
        attempt_id = str(packet["attempt_id"])
        if attempt_id in input_by_id:
            raise KnowledgePrecheckV6Error("duplicate model input attempt id")
        ...
        if (
            ...
            raise KnowledgePrecheckV6Error("model input/member launch binding is invalid")
        input_by_id[attempt_id] = packet
    if set(input_by_id) != set(launch_by_attempt):
        raise KnowledgePrecheckV6Error("model input/member launch coverage is incomplete")
    ...
    for evidence in trigger_responses:
        if not isinstance(evidence, Mapping):
            raise KnowledgePrecheckV6Error("member trigger raw response must be an object")
        ...
        if packet is None or attempt_id in derived_traces:
            raise KnowledgePrecheckV6Error("member trigger raw response binding is invalid")
    ...
    if not retained_attempt_ids <= set(derived_traces) <= set(input_by_id):
        raise KnowledgePrecheckV6Error("member trigger raw response coverage is incomplete")

    traces = [_read_json(root, path) for path in manifest["trigger_trace_files"]]
    trace_by_id: dict[str, Mapping[str, Any]] = {}
    for trace in traces:
        if not isinstance(trace, Mapping):
            raise KnowledgePrecheckV6Error("trigger trace must be an object")
        attempt_id = str(trace.get("attempt_id"))
        packet = input_by_id.get(attempt_id)
        if packet is None or attempt_id in trace_by_id:
            raise KnowledgePrecheckV6Error("trigger trace attempt binding is invalid")
        validate_trigger_trace(trace, packet, protocol)
        if trace != derived_traces[attempt_id]:
            raise KnowledgePrecheckV6Error("trigger trace differs from the raw member response")
        trace_by_id[attempt_id] = trace
    if set(trace_by_id) != set(derived_traces):
        raise KnowledgePrecheckV6Error("trigger trace coverage is incomplete")
    if set(input_by_id) - set(trace_by_id) - excluded_attempt_ids:
        raise KnowledgePrecheckV6Error("retained member trigger evidence is incomplete")
    ...
    for evidence in f3_decision_responses:
        if not isinstance(evidence, Mapping):
            raise KnowledgePrecheckV6Error("member F3 decision raw response must be an object")
        ...
        if packet is None or attempt_id in f3_decision_by_id or trace is None or not trace["triggered"]:
            raise KnowledgePrecheckV6Error("member F3 decision raw response binding is invalid")
    ...
    for exchange in exchanges:
        if not isinstance(exchange, Mapping) or set(exchange) != _EXCHANGE_FIELDS:
            raise KnowledgePrecheckV6Error("helper exchange fields are invalid")
        if exchange.get("schema_version") != EVIDENCE_SCHEMA_VERSION:
            raise KnowledgePrecheckV6Error("helper exchange schema is invalid")
        if exchange.get("operation") not in ALLOWED_KNOWLEDGE_OPERATIONS or exchange.get("state_changing_calls") != 0:
            raise KnowledgePrecheckV6Error("helper exchange is not read-only allowlisted evidence")
        ...
        if bytes_sha256(request_bytes) != exchange.get("request_sha256") or bytes_sha256(
            ...
            raise KnowledgePrecheckV6Error("helper raw bytes do not match their hashes")
        try:
            raw_request = json.loads(request_bytes)
            json.loads(response_bytes)
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            raise KnowledgePrecheckV6Error("helper raw bytes are not JSON") from error
        denied = _denylisted_keys(raw_request, denylist) | _denylisted_keys(json.loads(response_bytes), denylist)
        if denied:
            raise KnowledgePrecheckV6Error(f"helper exchange contains denylisted keys: {','.join(sorted(denied))}")
        ...
        if packet is None or exchange_id in exchange_by_id:
            raise KnowledgePrecheckV6Error("helper exchange binding is invalid")
        ...
        if exchange["operation"] == "find-fact-object-candidates" and raw_request["arguments"]["fact_type_keys"] != [
            ...
            raise KnowledgePrecheckV6Error("F2 request family differs from the member input")
        if exchange["operation"] == "read-fact-objects":
            ...
            if any(
                ...
                raise KnowledgePrecheckV6Error("F3 request crossed the frozen task family")
            if requested_uids != f3_decision_by_id.get(attempt_id):
                raise KnowledgePrecheckV6Error("F3 request differs from the raw member decision")
        card_layer = str(packet.get("card_layer", "calibration-baseline"))
        ...
        if replay != exchange:
            raise KnowledgePrecheckV6Error("helper derived fields do not replay from raw bytes")
    ...
    if set(f3_decision_by_id) != f2_attempt_ids:
        raise KnowledgePrecheckV6Error("raw post-F2 member decision coverage is incomplete")
    if {attempt_id for attempt_id, refs in f3_decision_by_id.items() if refs} != f3_attempt_ids:
        raise KnowledgePrecheckV6Error("raw member F3 decisions do not match actual F3 calls")
    ...
    for evidence in technical_responses:
        if not isinstance(evidence, Mapping):
            raise KnowledgePrecheckV6Error("member technical failure raw response must be an object")
        ...
        if attempt_id not in excluded_attempt_ids or attempt_id in technical_response_ids:
            raise KnowledgePrecheckV6Error("member technical failure raw response binding is invalid")
    ...
    if technical_response_ids != excluded_attempt_ids:
        raise KnowledgePrecheckV6Error("member technical failure raw response coverage is incomplete")
    ...
    for evidence in final_responses:
        if not isinstance(evidence, Mapping):
            raise KnowledgePrecheckV6Error("member final raw response must be an object")
        ...
        if attempt_id not in retained_attempt_ids or attempt_id in final_response_by_id:
            raise KnowledgePrecheckV6Error("member final raw response binding is invalid")
        final_response_by_id[attempt_id] = response
    if set(final_response_by_id) != retained_attempt_ids:
        raise KnowledgePrecheckV6Error("member final raw response coverage is incomplete")

    outputs = [_read_json(root, path) for path in manifest["model_output_files"]]
    output_by_id: dict[str, Mapping[str, Any]] = {}
    for output in outputs:
        if not isinstance(output, Mapping):
            raise KnowledgePrecheckV6Error("model output must be an object")
        validate_model_output(output, protocol)
        if _denylisted_keys(output, denylist):
            raise KnowledgePrecheckV6Error("model output contains a denylisted field")
        ...
        if packet is None or attempt_id not in retained_attempt_ids or attempt_id in output_by_id:
            raise KnowledgePrecheckV6Error("model output binding is invalid")
        if (
            ...
            raise KnowledgePrecheckV6Error("model output/input/raw-response binding is inconsistent")
        bound = []
        for exchange_id in output["helper_exchange_ids"]:
            ...
            if exchange is None or exchange["attempt_id"] != attempt_id:
                raise KnowledgePrecheckV6Error("model output references an unbound exchange")
    ...
    for packet in blind_packets:
        if not isinstance(packet, Mapping):
            raise KnowledgePrecheckV6Error("blind packet must be an object")
        attempt_id = str(packet.get("attempt_id"))
        output = output_by_id.get(attempt_id)
        if output is None or attempt_id in blind_by_id:
            raise KnowledgePrecheckV6Error("blind packet binding is invalid")
        ...
        if packet != expected:
            raise KnowledgePrecheckV6Error("blind packet differs from its condition-blind projection")
        blind_by_id[attempt_id] = packet
    if set(blind_by_id) != set(output_by_id):
        raise KnowledgePrecheckV6Error("blind packet coverage is incomplete")
    ...
    for evidence in scorer_responses:
        if not isinstance(evidence, Mapping):
            raise KnowledgePrecheckV6Error("scorer raw response must be an object")
    ...
    if not isinstance(adjudication, Mapping) or adjudication != {"overrides": []}:
        raise KnowledgePrecheckV6Error("post-hoc score overrides are disabled")
    ...
    for metric in metric_names:
        delta = [
            float(by_key[(pair_id, "calibration-enhanced")][metric])
            - float(by_key[(pair_id, "calibration-baseline")][metric])
    ...
    # Activation rate metrics (v6-specific), counted over the applicable pairs
    ...
    # V6 adoption threshold gates (aligned with protocol adoption_thresholds)
    trigger_correct_net_gain = trigger_sums["calibration-enhanced"] - trigger_sums["calibration-baseline"]
    gates = {
        "trigger_correct_not_decreased": bool(
            trigger_sums["calibration-enhanced"] >= trigger_sums.get("calibration-baseline", 0)
        ),
        "unnecessary_f2_not_increased": bool(
            unnecessary_f2_sums["calibration-enhanced"] <= unnecessary_f2_sums.get("calibration-baseline", 0)
        ),
        "correct_activation_rate_not_decreased": bool(
            correct_activation_sums["calibration-enhanced"] >= correct_activation_sums.get("calibration-baseline", 0)
        ),
        "behavior_change_rate_not_decreased": bool(
            behavior_change_sums["calibration-enhanced"] >= behavior_change_sums.get("calibration-baseline", 0)
        ),
        "strong_reuse_applicable_not_decreased": bool(
            strong_sums["calibration-enhanced"] >= strong_sums.get("calibration-baseline", 0)
        ),
        "missed_detection_rate_not_increased": bool(
            missed_detection_sums["calibration-enhanced"] <= missed_detection_sums.get("calibration-baseline", 0)
        ),
        "correct_non_use_not_decreased": bool(
            non_use_correct_sums["calibration-enhanced"] >= non_use_correct_sums.get("calibration-baseline", 0)
        ),
        "trigger_precision_recovery_absolute": bool(trigger_sums["calibration-enhanced"] >= 15),
        "trigger_precision_recovery_net_gain": bool(trigger_correct_net_gain >= 1),
        "unnecessary_f2_absolute": bool(unnecessary_f2_sums["calibration-enhanced"] <= 2),
    ...
    results = {
        ...
        "adoption_decision": (
            "supports-action-semantic-trigger" if support else "do-not-support-action-semantic-trigger"
        ),
    ...
    lines = [
        "# Action-semantic trigger calibration paired experiment (v6)",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- Correct activation rate (calibration-enhanced): {correct_activation_sums['calibration-enhanced']}/9",
            f"- Missed detection rate (calibration-enhanced): {missed_detection_sums['calibration-enhanced']}",
            f"- Behavior change rate (calibration-enhanced): {behavior_change_sums['calibration-enhanced']}/9",
            "",
            "## Interpretation boundary",
            "",
            (
                "This source-complete paired run tests whether the v6 action-semantic "
                "trigger (signal + action-anchor positive condition, no veto) recovers "
                "trigger precision without regressing the v5 activation quality.  The "
                "calibration-baseline arm uses the v5 recall-first wide-trigger policy; "
                "the calibration-enhanced arm uses the v6 action-semantic trigger "
                "policy, both over the identical activation-projected cards and "
                "utility-layer tracing.  The experiment does not establish broad "
                "causal generalization beyond the frozen 18 tasks and observable "
                "runtime."
            ),
            "",
        ]
