...
def test_readme_exposes_the_four_source_integration_jobs() -> None:
    readme = (REPOSITORY_ROOT / "README.md").read_text(encoding="utf-8")

    assert "环境接入（取得源码、确认必需入口、部署薄 Skill） → 管辖配置 → Git Hook 部署 → 验证" in readme
    assert "specs/attachments/09.Att.01-环境接入面.md" in readme
    assert "specs/33-环境接入行动模板.md" in readme
    assert "specs/09-环境接入规范.md" in readme
    assert "./ldvh capabilities" in readme
    assert "./ldvh doctor" in readme
    assert "不打包、不安装本体" in readme
    ...
    assert (
        "请阅读 README，完成 LDVH 的环境接入、管辖配置、Git Hook 部署与验证，并如实报告已验证与未验证的范围。" in readme
    )
    assert "## 给 AI：接入新的开发环境" not in readme


def test_thin_skill_uses_the_canonical_source_template_and_both_repository_routes() -> None:
    environment_spec = (REPOSITORY_ROOT / "specs/09-环境接入规范.md").read_text(encoding="utf-8")
    integration_surface = (REPOSITORY_ROOT / "specs/attachments/09.Att.01-环境接入面.md").read_text(encoding="utf-8")
    skill = (REPOSITORY_ROOT / "skill/SKILL.md").read_text(encoding="utf-8")
    readme = (REPOSITORY_ROOT / "README.md").read_text(encoding="utf-8")

    assert "canonical 模板 `skill/SKILL.md`" in environment_spec
    assert "任何环境的部署件必须与当前仓库模板逐字节一致" in environment_spec
    assert "模板不含任何逐环境值，部署即复制文件" in environment_spec

    assert "| `skill-template` | `skill/SKILL.md`（canonical 文件）" in integration_surface
    assert "只读复制且部署件必须逐字节一致" in integration_surface
...
def test_capability_boundaries_are_verbatim_without_expanding_skill_duties() -> None:
    ...
    for source in (root_spec, environment_spec, skill):
        ...
        assert "主 worktree" in source
        assert "linked worktree" in source
        assert "独立 clone" in source
...
def test_integration_surface_attachment_matches_current_source_dispatcher() -> None:
    ...
    assert "不存在环境 Hook、插件或 adapter 层" in surfaces


def test_ai_enablement_guide_keeps_capability_and_evidence_boundaries() -> None:
    template = (REPOSITORY_ROOT / "specs/33-环境接入行动模板.md").read_text(encoding="utf-8")
    environment_spec = (REPOSITORY_ROOT / "specs/09-环境接入规范.md").read_text(encoding="utf-8")
    ...
    assert "技能位置、加载时机、可见会话范围、权限与可观察入口" in environment_spec
...
def test_b4_covers_pre_gate_creation_reviewer_and_post_gate_consumption() -> None:
    checklist = (REPOSITORY_ROOT / "specs/attachments/09.Att.02-接入验收行为清单.md").read_text(encoding="utf-8")
