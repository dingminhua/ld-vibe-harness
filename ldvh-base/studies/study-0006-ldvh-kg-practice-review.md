...
与 LDVH 的匹配点是：先定义最小关系 schema，再把检查与触发闭环接到工作流程与 Human Gate。

## 建议

### 阶段 1（P1）：最小图谱抽取与校验闭环

- 构建统一对象抽取器：输入 `ldvh-base` 对象，输出 `entities.json`、`edges.json`。
- 增加关系一致性校验：缺失目标、重复边、非法边类型、时间戳缺失告警。
- 输出 `graph-metadata.json` 承载版本号、构建时间、告警摘要、schema 版本。
...
- 将本 Study 的阶段化建议分流为 WorkPlan，优先推进 `P1` 的抽取器与校验器。
