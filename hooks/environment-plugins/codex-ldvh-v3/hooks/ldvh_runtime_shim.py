...
def split_unquoted_and_chains(command: str) -> list[str]:
    return action_classifier_module().split_unquoted_and_chains(command)
...
def git_subcommand(parts: list[str]) -> str:
    return action_classifier_module().git_subcommand(parts)


def is_allowed_read_only_python(parts: list[str]) -> bool:
    return action_classifier_module().is_allowed_read_only_python(parts)
...
def explicit_operation(payload: dict[str, Any]) -> str:
    return action_classifier_module().explicit_operation(payload)
