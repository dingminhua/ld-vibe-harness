...
继续迁移任一 v2 规范前，应先读取 `MIGRATION-MAP.md`。行动编排成员不按 v1 现有流程直接迁移；v2 03 应先定义行动编排如何由规范保障需求生成、登记、接管和回写，具体 30-59 候选行动编排待 v2 各规范保障需求稳定后再规划。
...
当前辅助控制文件如下。辅助控制文件不属于正式规范目录，不作为 active 规范事实源：
...
除上表和辅助控制文件外，`specs-v2/` 中其它文件、目录、编号草案或占位内容，都不得被解释为已经处理、已经确认、已经迁移或已经进入 v2 规范目录。后续每完成一篇规范、附件或成员草案的单篇核对，再把它加入本文当前已处理范围。
...
9. 不得绕过 `MIGRATION-MAP.md` 直接宣称 v1 机制已经由 v2 承接；行动编排成员不得按 v1 流程默认逐篇迁移，必须由 v2 规范保障需求生成候选计划。
