...
3. 计划行展示计划标题、计划 ID、更新时间、状态、compact 任务态势条、复制路径和辅助阅读入口图标；态势条占满计划行宽度；计划标题使用 `ldvh-body`，不使用 `ldvh-card-title`，避免越过组标题和区块标题层级；成功标准、完成证据等计划关闭材料留在 TaskPlan 详情中表达。辅助阅读图标不是占位，未展开时表示打开右侧阅读，当前计划已展开时切换为关闭方向。由于计划行位于状态色背景中，复制路径和辅助阅读图标可继承该计划组状态色，但二者必须同色。
4. WorkArea 详情页点击计划行只打开右侧辅助阅读区，不切换主路由到 TaskPlan 详情；主路由跳转只属于对象列表卡片。
5. 属性：按目标、范围、约束、来源展示 `description/scope/constraints/source`，不把 `scope` 放进顶部元信息 chip；标题使用与计划态势一致的图标+标题样式。属性属于核心定义事实，不默认折叠；多行内容渲染为语句列表，`包含` / `不包含` 等短前缀渲染为语义标签。
6. 关联材料：只有 `related_docs/related_adrs/related_memos/related_pitfalls` 非空时显示。
7. 关联材料行是 plain 引用行，不带状态色背景；右侧复制路径和扩展阅读图标保持中性 ghost 表现，左侧对象类型图标、ID 或短标签承担对象类型识别。
