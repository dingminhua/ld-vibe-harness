...
本 Skill 是 Skill 能力资产，只承载可复用 SOP，不新增稳定规则，不替代 specs、Code、Human Gate、Agent 审查或 Git 文件事实源。
...
1. 修改 `specs/` 正式规范后；
2. 修改 `rules/`、`skills/`、`agents/`、`hooks/` 等能力资产且影响 04 系列边界时；
3. 准备提交规范或能力资产变更前；
...
1. 本次变更目标；
2. 修改文件清单；
3. 是否涉及 Rules、Skill、Agent、Hook 能力资产；
4. 已运行或计划运行的验证命令；
...
3. 对能力资产变更，检查是否保持薄入口，不复制完整规范正文，不把 Command、CLI、MCP、CI 或文档写成与 Rules、Skill、Agent、Hook 同级的文本能力资产类型；
