...
ldvh_spec:
  ...
  scope: "AI 在一个身份和目标范围可确定的 Git worktree 中准备、创建和验证本地新 commit；不包含 push、PR、amend、rebase、merge、tag、Hook 或 CI 变更"
  basis:
...
本文主要服务 V1 快速定位、V2 充分理解、V3 边界识别、V4 稳定推进、V5 据实判断、V6 工作接续和 V7 清晰沟通。V8 持续积累来自把重复行动结构维护在同一当前来源，而不是复制到 Skill、Agent、Hook、项目说明或 Code。01 和 03 已经定义 Index、`HEAD`、commit 与证明边界，06 已经定义模板共同规则；本文的独有价值只来自把 Human 目标、用户既有 Index、项目规则、范围匹配验证、既有 Hook 实际结果和提交后目标残留组织成同一个 Git 行动边界。该剩余结构不能成立时，本文应取消而不是扩写。
...
2. 提交时机、提交拆分、message 格式、type/scope、签名、branch、Hook、CI 或测试门禁等领域或项目强制规则；
3. Human 是否已经授权当次提交，或者修改文件、验证通过、模板命中是否自动产生提交授权；
4. Git 命令、参数、暂存算法、锁、错误码、API、Code 模块、测试实现或环境工具映射；
5. push、PR、amend、rebase、merge、cherry-pick、tag、release、Hook/CI 安装或远端 ref 变更；
...
1. Human 当前指令已经明确授权创建本地新 Git commit，且实际适用的项目与领域规则允许在相应条件下执行；项目或领域规则自身不得被推定为 Human 的当次写入授权；
2. 目标工作对象能够绑定到唯一实际 Git worktree；
3. 本次准备提交的目标变化范围能够依据 Human 目标、当前来源和实际差异确定；
4. 实际 Git 状态、必要差异和当次可能适用的项目规则与验证入口可以读取；
5. 不需要通过本模板实施 push、PR、amend、rebase、merge、tag、Hook/CI 安装或其它排除行为。
...
1. Human 当前明确给出的本地新 commit 目标和授权范围；项目规则、模板命中、修改完成或测试通过不能替代该授权；
...
2. Human 只承担当前来源保留的提交授权、目标取舍、破坏性历史操作、无法安全隔离的混合变化取舍或风险接受；清楚授权已经存在时不重复请求；
3. Git、Code、tests 和环境工具只提供实际支持的状态读取、差异、暂存、验证、commit 创建和回读能力；静态存在不证明当次可用；目标仓库中已经生效的 Hook 可能由 Git 自动执行，属于实际环境条件；
4. 本模板不安装、修改、禁用、绕过或定义 Hook，也不要求特定 Skill、Agent、validator、Git 子命令或平台接口。未来薄接入只能调用同一当前模板和相应能力，不得复制正文或项目规则；
5. 模板命中、测试通过、文件已经修改或存在待提交变化都不授予 commit、push 或其它行动权限。
...
2. 受管辖目标按 `source-of-truth-traceability` §9.2 判断一个可共同说明、验证和回退的主要目的；普通拆分由 AI 依据该来源判断，需要 Human 决定时只按 `source-of-truth-traceability` §12(6) 进入 Gate；项目来源要求更细拆分、目标范围无法共同说明、验证不能共同覆盖或候选无法安全隔离时，按实际来源拆分或缩小范围；模板不自行建立拆分政策；
...
4. 根据候选快照、Human 目标、受管辖目标适用的 `source-of-truth-traceability` §§9.3–9.4、项目附加 message 规则和实际验证形成提交说明。明确非受管辖且没有项目格式规则时，提交说明仍必须据实描述实际变化，不得声称未发生的验证、Gate、完成、push 或发布；
5. 按 `source-of-truth-traceability` §9.5 在创建 commit 前重新核对实际候选快照、目标路径、message、管辖结果、03 来源身份和验证范围；任何不一致都使先前结论失效并回到本阶段处理，不得以最初计划替代当前快照。

#### C. 创建、回读与验证

1. 只有授权、目标快照、来源要求和必需验证均达到实际允许执行的范围后，才调用当次可用的 Git/Code 能力创建一个本地新 commit；
2. 失败、拒绝、无变化、Hook 修改或拒绝、部分结果、结果不可观察或 `source-of-truth-traceability` §9.6 所述不合规时，保留实际 Git 状态和诊断，不得重试破坏性操作、扩大路径、禁用既有 Hook 或绕过门禁；
3. commit 返回成功后，重新读取实际 `HEAD`、新 commit 的身份和内容范围，以及剩余 Working Tree 与 Index；
...
4. 提交说明所依据的项目规则、实际检查过的来源与作用范围，以及未检查、不可读取或身份不清的缺口；只在该有限范围内报告未发现额外格式规则；
5. commit 后剩余的 staged、unstaged、untracked 和无关变化；
6. 被拒绝、未执行、部分完成或需要 Human 决定的内容；
7. push、PR、发布及其它排除行动仍未由本模板执行；
8. 可以继续普通求解、另行授权、修正、重试或保持暂停的入口。
...
|---|---|---|---|---|---|---|
| 独立规范责任与模板准入 | 准备把本文从 `draft` 改为 `active` 时 | §13.5 记录已经由独立复核者主动比较 02、03、06、07、项目规则、Code和普通求解；重复价值、独立失败和净价值仍成立 | 准入调查、独立复核意见、主执行者处置和代表性场景 | 对照 01、06 和调查记录审核 | 本规范责任和 `git-commit` 模板准入 | 保持 `draft`；缩减、合并到现有来源或取消候选 |
...
| 规则与模板边界 | 修改步骤、Gate、验证或交还时 | 本文只组织来源读取、行动关系和失败分流，没有定义 message、branch、拆分、测试、Hook 或实现政策 | 本文、02、03、06、07、项目规则和差异审核 | 来源逐项对照和正反场景走查 | 当次修改及直接相邻责任 | 移除复制内容，回到正确来源定义；保持 `draft` 或暂停受影响模板 |
| 适用与授权 | 准备使用模板或创建 commit 时 | Human 当前指令已明确授权本地新 commit，实际来源未禁止且相应限制已经满足，repo/worktree 和目标范围唯一，排除行为未被隐含纳入 | Human 当前指令、管辖/工作对象结果、Git 状态和适用来源 | AI 适用判断与来源回读 | 当次 repo、目标范围和本地 commit | 不执行状态变更；补充输入、缩小范围或普通求解 |
...
| 验证与 message 声明 | 准备创建及交还时 | 实际来源要求的验证已按当前候选执行；message 与完成声明没有超出实际变化、验证和 Gate | 项目/领域规则、实际测试结果、候选快照和 message | 来源规定的验证入口及 AI 对照审核 | 当次已执行验证和声明 | 修正、缩小声明、标记未验证或暂停；不得绕过来源门禁 |
| commit 创建与回读 | Git 返回后准备声明成功时 | 新 commit 实际存在，身份、实际 message 与内容范围已回读，和已授权目标一致；既有 Hook 的拒绝、修改或其它可观察结果及剩余 Working Tree/Index 已纳入判断 | 实际 Git 结果、`HEAD`、commit 内容、Hook 可观察诊断和提交后状态 | 当次 Git/Code 入口及只读回读 | 当次本地新 commit、环境结果与已观察剩余变化 | 不声明成功或完整完成；报告拒绝、修改、部分结果、实际残留和可安全继续入口 |
| 能力声明 | 声称模板发现、Git/Code、Skill、Hook 或环境已经支持本文时 | 具有范围匹配的当前来源、实现、调用、测试或环境触发依据 | 实际实现、调用、tests、Hook/环境探针和输出 | 对应真实入口 | 当次已实现、已接入、已验证范围 | 标记未实现、未接入或未验证；继续普通求解或交还缺口 |

本文草案、声明表或未来 `active` 本身都不证明 Git 提交能力可用，也不证明任何一次 commit 已经获得授权、执行或验证。

## 7. Human Gate

本文不因普通只读检查、模板召回、规则定位和已有明确授权下的安全提交步骤新增重复 Human Gate。出现以下情况时，必须进入实际来源保留给 Human 的决定：
...
Human 已在当前指令中清楚给出相应决定时，不重复请求同一决定。Human 决定不能使错误 repo、超范围快照、虚假 message、未执行 Git 命令或未回读 commit 自动成立，也不能把本地 commit 扩张为 push、PR 或工作完成。

## 8. Stop Conditions

出现以下任一情况时，必须暂停受影响的 commit 创建或相应成功声明：

1. 没有当前明确提交授权，或者只因文件已修改、模板被发现、测试通过或存在 staged 变化就准备提交；
...
6. message、验证或完成声明超出实际候选快照、来源、证据、Gate 或可观察结果；
...
暂停只覆盖受影响的状态变更、路径、快照或声明。期间可以继续只读核对 Human 目标、来源规则、Git 状态、差异、验证和安全隔离方案，或者明确不依赖本模板继续普通求解。只有授权、目标身份、候选范围、来源要求、验证、commit 实际结果和回读达到对应成立条件，或声明缩小到现有依据实际支持的范围后，才能恢复。
