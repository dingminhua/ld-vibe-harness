...
它的方法，是以约束体系划定边界，以事实源沉淀记忆，以工具链连接能力，以工作流驱动执行，以任务治理守住交付，以可视化工具桥接信息——让人的自然语言意图通过规范规则、事实源、工具链和工作流，转化为可规划、可执行、可验证、可沉淀的 AI 工程闭环。
...
工具可以读取、解析、校验、聚合、提供 Web 展示和受控写入 Git 文件事实源；工具缓存、数据库派生视图、运行时索引和页面状态不得成为任务状态、证据、决策、规范规则或审计结果的唯一来源。
...
1. 10-19 核心基础规范待逐步创建；
2. 20-49 生产对象规范和 50-79 行动模型规范待逐步创建，具体待补齐事项见 `specs/03-Specs文档规范.md` §八和 `specs/01-LDVH目录说明.md` §七。
