...
| 候选 | 原因 | 建议承载 |
|---|---|---|
| spark-0022 子 Agent 复查编排流程缺口 | 用户已经多次要求用子 Agent 审核规范和迁移质量，当前仍缺少固定行动编排 | 后续行动编排或运行时扩展相关 WorkCase |
| spark-0017 知识地图建设方向 | active specs 范围、preflight 和知识地图入口已有初版 Code，但正式知识地图机制尚未完全展开 | Code / 知识地图 / 受控写入 WorkCase |
| spark-0007 Spark 分流与收敛提醒机制 | 当前本索引只解决读取误读，尚未形成自动提醒或分流检查 | Code 诊断或 Web 提醒 WorkCase |
