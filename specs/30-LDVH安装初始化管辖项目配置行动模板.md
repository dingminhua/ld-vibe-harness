...
| 环境检查 | 是否使用环境状态、入口审计或等价验证区分安装检测通过、integrated / manual_ready / deferred / removed_top_level；是否提示插件页面状态、重启 App、授权 / trust、新窗口或新会话测试和正常判断标准；是否避免把用户侧冒烟检查当作安装完成阻断项 | 不得声明环境 integrated；安装检测未通过时不得声明安装完成 |
