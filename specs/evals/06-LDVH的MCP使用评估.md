...
LD Vibe Harness 的五类构成要素是介质、Trae Solo 环境机制、Harness 工具、Harness 生产对象和 Harness 行动模型（依据 `specs/00` §四）。MCP 是 Trae Solo 环境中 Agent 可调用的工具协议，属于 Trae 平台提供的能力，不是 LDVH 自定义的构成要素。

### 2.2 MCP 不是 04 系列正式治理对象

04 系列只治理 Rules、Skill、Agent 三类可落地 AI 协作机制（依据 `specs/04` §三）。MCP 不在其中。MCP 在 LDVH 中的角色是 Agent 的工具能力来源，其使用边界由 04.03 Agent 机制规范和 05 工具基础规范共同约束。

### 2.3 MCP 受 05 工具边界约束

MCP 工具输出与 Harness 工具输出一样，必须遵守 `specs/05` 和 `specs/03` 的事实源边界：
...
| Agent 调度 | Agent 调度条件判断 | 不需要 MCP | Agent 调度由 04.03 治理条件决定，不需要外部工具 |
| 事实源回写 | 回写目标判断和写入执行 | 不需要 MCP | 回写由 03 事实源边界和 05.01 受控写入原则决定，程序辅助层已覆盖 |

### 3.2 AI 协作机制（specs/04 系列）中的 MCP 需求

| 机制 | 可能需要 MCP 支撑的场景 | 候选 MCP | 评估 |
|---|---|---|---|
| Rules | 规则读取和加载 | 不需要 MCP | Rules 由 Trae 原生机制加载，不需要外部工具 |
| Skill | Skill 执行中的外部信息获取 | Context7、Fetch | 部分适合；Skill 可建议用户使用 Agent 查询外部信息，但 Skill 自身不调度 Agent（依据 04.02 §七） |
| Agent | Agent 执行中需要外部工具能力 | 按角色分配 | 适合；这是 MCP 的核心使用场景，按 04.03 权限最小化原则配置 |

### 3.3 工具层（specs/05 系列）中的 MCP 需求

| 工具层 | 可能需要 MCP 支撑的场景 | 候选 MCP | 评估 |
|---|---|---|---|
| 程序辅助层 | 解析、校验、聚合 | 不需要 MCP | 程序辅助层处理确定性逻辑，不需要外部工具能力 |
| 程序辅助层 | 上下文包生成中的外部文档补充 | Context7 | 部分适合；上下文包主要从 Git 文件事实源生成，仅在需要外部框架文档时补充 |
| Web 展示层 | UI 实现和验证 | Playwright | 适合；Web Tools 页面开发后的自动化验证 |
| Web 展示层 | 人类确认工作台 | 不需要 MCP | 确认工作台是 Web 展示层自身功能，不需要外部工具 |

### 3.4 生产对象（specs/06）中的 MCP 需求

| 场景 | 可能需要 MCP 支撑 | 候选 MCP | 评估 |
|---|---|---|---|
| 对象实例创建和状态流转 | 不需要 MCP | — | 由 05.01 受控写入和对象规范决定 |
| 对象校验 | 不需要 MCP | — | 由程序辅助层确定性校验覆盖 |
| 对象聚合和视图 | 不需要 MCP | — | 由程序辅助层聚合和 Web 展示层覆盖 |
...
| 维度 | 评估 |
|---|---|
| 对 LDVH 的价值 | 高。直接支撑 07 行动模型中的 Context 组织、Scenario 识别和复杂 Gate 判定；支撑 specs 修改时的跨规范影响分析；支撑生产对象设计时的复杂逻辑拆解 |
...
| 维度 | 评估 |
|---|---|
| 对 LDVH 的价值 | 中。支撑 05.02 Web 展示层的 UI 自动化验证；支撑 Web Tools 页面开发后的交互测试和截图回归 |
...
| 引入条件 | 团队已使用 Atlassian 或飞书体系，且需要 AI 辅助跨平台协作 |
...
| 替代方案 | 使用 ldvh-base/ 承载结构化生产对象实例，使用 docs/ 承载管辖项目资料，使用 specs/refs/ 承载外部资料引用 |
...
| 替代方案 | 使用 Trae 内置文件系统工具和 05.01 程序辅助层 |
...
| 对管辖项目 | 如管辖项目需要，应由管辖项目按自身需求配置，不进入 LDVH 核心 MCP 配置；必须遵守 04.03 权限最小化原则和 Human Gate |
...
1. MCP 工具能力受 05 工具基础规范约束，不得绕过程序辅助层的校验和受控写入边界（依据 `specs/05` §七）；
2. MCP 工具不得直接调用 AI、Skill 或 Agent（依据 `specs/05` §七）；
3. MCP 工具不得替代 specs/、ldvh-base/ 或 docs/ 的权威事实（依据 `specs/05` §四）。

### 5.3 Agent 权限约束

1. MCP 应按 Agent 职责分配，不得把所有 MCP 都加给所有 Agent（依据 `specs/04.03` §三和 refs/02 §七）；
2. Agent 的 MCP 权限范围应最小必要（依据 `specs/04.03` §九）；
...
## 六、MCP 与 Rules / Skill / Agent 的组合建议
...
## 八、MCP 引入的 Human Gate 建议

以下情况应评估 Human Gate：

1. 新增、删除或修改项目级 `.trae/mcp.json` 配置；
2. 将 MCP 工具输出提升为稳定事实但未回写 Git 文件事实源；
3. 将 Memory 类 MCP 引入项目；
4. 将高风险 MCP（云命令、数据库写操作、支付退款）开放给通用 Agent；
5. 在 MCP 配置中包含明文密钥或 Token；
6. 改变 MCP 在 LDVH 体系中的定位（从工具能力来源变为事实源或协作机制）。
...
| 02-LDVH对gstack的借鉴评估 | 无直接关联 |
| 03-LDVH对Agent-Harness的借鉴评估 | 03 涉及 Agent 调度，MCP 是 Agent 的工具能力来源，本文 4.1 和 4.3 的 MCP 配置建议应与 Agent-Harness 概念对齐 |
| 04-LDVH对七层管理模型的借鉴评估 | 无直接关联 |
| 05-Git版本管理临时参考 | 05 涉及 Git 操作，MCP 不应替代 Git 命令行工具；Git MCP（如 GitHub MCP）可条件引入，但需遵守 05 中"人管推送和版本"的原则 |
...
2. 05.01 程序辅助层稳定后，评估 Context7 输出是否需要纳入上下文包生成流程；
3. 05.02 Web 展示层稳定后，补齐 Playwright 在 Web Tools 验证中的使用规范；
4. 如未来创建 LDVH 专用 Agent，应按本文第六节的组合建议配置 MCP；
5. 如未来建立 MCP 配置治理流程，应将本文 §八 的 Human Gate 建议纳入正式规范。
