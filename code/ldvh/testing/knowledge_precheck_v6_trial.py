"""Runner-owned command surface for the v6 action-semantic trigger calibration paired trial."""
...
def helper_call(
    *,
    ...
    if operation == "read-fact-objects":
        ...
        if (
            ...
            or any(
                ...
                or (
                    families.get(ref.get("object_uid")) not in (None, "unknown")
                    and families.get(ref.get("object_uid")) != packet["family"]
                )
                for ref in refs
            )
        ):
    ...
    gateway = ReadOnlyKnowledgeGateway(
        ...
        expected_f2_cards_baseline=snapshot["family_cards_baseline"],
        expected_f2_cards_activation=snapshot["family_cards_activation"],
    ...
    # The model consumes the served card set (activation projection for the
    # enhanced arm, original baseline cards otherwise) while the raw Helper
    # response remains byte-identical in the evidence record.
    helper_response = json.loads(exchange["raw_response_utf8"])
    if operation == "find-fact-object-candidates":
        helper_response = deepcopy(helper_response)
        helper_response["result"] = deepcopy(helper_response.get("result", {}))
        helper_response["result"]["cards"] = deepcopy(exchange["served_cards"])
    return {"exchange_id": exchange_id, "helper_response": helper_response}
...
def record_scorer_technical_failure(
    *,
    ...
    if not isinstance(technical_failures, list):
        raise TrialRunnerError("scorer technical failure ledger is invalid")
    if any(isinstance(failure, Mapping) and failure.get("attempt_id") == attempt_id for failure in technical_failures):
        raise TrialRunnerError("scorer technical failure attempt is duplicated")
    existing_scores = [_read_object(path, "score") for path in sorted((root / "scores").glob("*.json"))]
    used_hashes = {str(score.get("fresh_scorer_context_id_hash")) for score in existing_scores} | {
...
def seal_bundle(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise TrialRunnerError("attempt ledger is not a complete preregistered v6 run")
