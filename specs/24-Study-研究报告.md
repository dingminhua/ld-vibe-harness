...
ldvh_spec:
  ...
  scope: "管辖项目中已经完成、以外部产品、实践、标准、论文、工具或方案为对象，且可独立引用的外部内容调研报告"
  basis:
...
Study 保存一轮已经完成、可独立引用且具有跨行动稳定阅读价值的**外部内容调研**。其研究对象必须是外部产品、实践、标准、论文、工具或方案；它使后续 AI 能快速恢复外部研究对象、输入与方法边界、主要发现、结论限制、建议和后续分流，并据来源版本与观察时点判断能否继续引用，避免重复调研或把过时结论冒充当前事实。

Study 主要服务 V1 快速定位、V2 充分理解、V3 边界识别、V5 据实判断、V7 清晰沟通和 V8 持续积累。外部调研的执行与进度由 WorkCase 和当次计划承担；Study 不承载搜索过程、任务编排、内部设计探索或行动授权。项目内 Spark、WorkCase、ADR、Pitfall、specs 和普通文档只可说明调研委托、LDVH 适用边界或后续吸收，不能单独构成 Study 的研究主体或外部证据。新增成本包括查重、来源精确化、时效复核、Schema、迁移和消费；通过只准入已经完成的一轮外部调研、Markdown 单一正文、三状态、两个专属元数据字段、统一来源证据和禁止多套 related 字段，维护成本低于重复重建资料、误用旧结论和无法定位依据的损耗。普通文档已经能够自然承载且不需要稳定身份、状态、证据、时效边界和整体替代历史时，不创建 Study。
...
1. 正式规则、长期决定、执行目标、研究计划、实际失败经验、纯内部分析或行动授权；
...
一份外部内容调研报告只有同时满足以下条件，才可以形成 Study：

1. 一轮研究已经完成，不是进行中的搜索、调查任务或链接收集；
2. 存在一个可独立引用的外部研究对象和核心研究问题；子问题共享同一外部对象、输入、方法、适用边界和结论单元；
3. 实际输入、研究方法、观察时点、适用范围、排除项和未覆盖范围清楚；
4. `urls` 至少包含一条实际读取的外部 HTTP(S) 资料，且关键发现与结论能够精确回到该外部证据；来源直接陈述、当次观察和 AI 推断可以区分；
...
以下内容不得形成 Study：临时搜索结果、裸链接清单、聊天摘要、未完成调查、运行日志或一次性笔记；研究任务、执行计划、todo、验收或进度；已经成立的长期决定；实际发生并解决的失败经验；正式规则；纯内部 WorkCase、编排、实现或设计分析；内部审计报告；单纯迁移索引、吸收台账、派生搜索视图或没有独立外部研究结论的资料汇编。

未完成或仍需澄清的问题保持 Spark 或进入 WorkCase；已经作出的选择进入 ADR；已经发生、查明、解决并验证的失败机制进入 Pitfall；纯内部审计或分析按其稳定身份进入普通文档或相邻对象。Study 回答“对哪个外部对象，在什么输入、方法、版本和观察时点下发现了什么、限制是什么”，不回答“当前必须遵守什么”“LDVH 已经决定做什么”或“内部实现应如何开展”。
...
|---|---|---|
| `study` | 已经完成、以明确外部对象为主体、可独立引用且具有跨行动稳定阅读价值的一轮外部内容调研报告 | `study-fact-type::5. Study 类型定义` |
...
| `urls` | required | `study-fact-type::7. 外部网址、自然语言证据、时效与替代关系` |
| `relations` | conditional | `study-fact-type::7. 外部网址、自然语言证据、时效与替代关系` |
| `disposition-summary` | conditional | `study-fact-type::6. 对象语义与生命周期` |
| `closed-at` | conditional | `study-fact-type::6. 对象语义与生命周期` |
| `adr-applicability` | required | `study-fact-type::7. 外部网址、自然语言证据、时效与替代关系` |
| `workcase-validation-summary` | required | `study-fact-type::7. 外部网址、自然语言证据、时效与替代关系` |
...
|---|---|---|---|---|---|
| `study-research-question` | `research_question` | string | 本报告实际回答的一个以外部对象为主体、可独立引用的研究问题或边界一致的子问题闭集 | 不表示 Human 原话、纯内部 WorkCase 执行目标、ADR 决定问题、搜索关键词或待办 | 必填非空；正文“研究问题”只可展开，不得改变本字段；必须能识别外部对象；问题实质变化通常形成新 Study |
...
Study 必须由外部内容调研形成。来源 locator、正文对外部来源用途与限制的说明必须按 对象自有语义字段、自然语言验证说明 与正文骨架重新建立准确映射；只复制 URL、标题、摘要或其它旧字段而丢弃用途、观察边界和限制不成立。项目内对象或文档可记录问题来源、LDVH 适用范围或验证上下文，但不能代替外部研究对象或外部证据。若一项稳定信息无法在现有引用成员和正文中无歧义承接，必须停止创建并按 05 重新进行字段准入。

完整 Schema 由统一登记的 `fact-object` 直接字段、本节绑定、跨类型共享定义、类型专属字段定义和本节正文骨架组合。Study 不得出现 current summary、priority、evolution、WorkCase/ADR/Pitfall 专属字段、user_intent、conclusion、input_refs、related_*、archive_reason、正文外第二报告、自由 metadata 或其它未登记 frontmatter。

## 6. 对象语义与生命周期

Study 只记录完成的一轮外部内容调研。搜索、阅读、实验或比较仍在进行时不得创建 active Study；调研工作保持在 WorkCase 或当次上下文，报告完成后再形成 Study。active 只表示报告在明示 applicability、来源版本、观察时点、validation_summary 和正文限制内仍可作为当前研究入口，不表示所有外部事实仍最新、建议已采纳、结论成为规则或决定已经成立。
...
| `superseded` | 一个后来成立的新 Study 对同一研究问题与适用范围形成整体替代 | disposition_summary、closed_at、自然语言验证说明 必填；旧对象必须成为一个在建边时为 active 的新 Study 的有效 supersedes 目标 |
| `retired` | 报告不再安全或没有当前引用价值，且没有新 Study 整体替代 | disposition_summary、closed_at、自然语言验证说明 必填；必须说明具体失效、冲突或退出依据 |
...
## 7. 外部网址、自然语言证据、时效与替代关系

Study 必须有至少一条 `urls`。每项使用 `{ ref, title, summary }`：`ref` 是绝对 HTTP(S) URL，`title` 是资料标题，`summary` 说明该资料支持什么或不支持什么。项目内路径、代码、日志、会话与 Git revision 不得写入事实对象。

研究问题由 `research_question` 表达；外部资料的用途、观察边界、关键发现、推断、限制和验证覆盖由正文、`abstract`、`validation_summary` 与其它类型自有自然语言字段据实说明。不得以 URL 本身代替这些说明。下游对象通过 `relations` 表达对象关系，不反向复制来源。
...
Study 在当前问题需要外部研究依据、准备调研实质相同或相邻的外部对象、比较外部方案或作出依赖外部研究结论的决定，以及当前输入精确引用研究报告时产生召回机会。F2 Study 候选卡直接投影 `object_id`、`title`、`status`、`research_question`、`abstract`、`applicability`、`validation_summary` 和 `updated_at`，并只提示引用中是否存在 version/observed_at 边界，不在候选层解释结论当前性。只在主要发现、证据映射、限制或建议会影响当前判断时展开完整正文，不得因主题相似就向 AI 无差别注入整份报告。
...
|---|---|---|---|---|
| 外部对象、完成与单一问题准入 | 只把以明确外部对象为主体、完成且可独立引用的一轮外部内容调研形成 Study | 本文准入、来源回读、AI 审核 | 对象治理 | 创建或拆分 Study 时 |
...
| 来源、证据与时效 | 创建、更新或消费可变化结论时 | kind 属于闭集；自然语言验证说明 至少有一个 `web-page`、`api-observation` 或具有明确外部出处的 `human-provided-artifact`；locator 符合 profile；version/observed_at 出现条件和 `observed_at <= updated_at` 成立；重要发现可映射到证据；重验证范围清楚 | 本文、03、对象引用与外部当前来源 | 来源矩阵检查、来源回读、版本核对、实际观察与证据审查 | 被核对的来源、版本、时点和结论 | 暂停受影响结论消费；补证、收紧 applicability、更新、替代或退出 |
...
Human 决定的复用按 00 §10 执行；Human 当前指令已经授权据实记录相应 Study，且适用于该行动的全部来源规则许可条件已经成立时，不因对象类型本身重复进入 Human Gate。Human 确认不能替代来源、Schema、时效复核或证据充分性，也不能把建议变成决定或规则。
...
1. 外部调研仍在进行，研究问题未能识别明确外部对象，或方法、输入边界、主要发现、限制和分流不能形成完成报告；
2. 来源或证据不可恢复、自然语言验证说明 缺少 `web-page`、`api-observation` 或具有明确外部出处的 `human-provided-artifact`、重要结论无法映射到证据、可变化来源缺少必要 version/observed_at，或当前性要求尚未重验证；
...
暂停范围与允许继续的行动按 00 §11 执行；对 Study，只有完成性、来源、时效、字段、正文、状态、关系和权威边界成立，实际变更完成回读后，才能恢复相应范围。
