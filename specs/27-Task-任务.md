...
`specs/07-LDVH事实模型基础规范.md` 定义事实模型通用规则、文件命名、附件型实践子文档命名和事实模型标准组成。本文依据 07 §4.2 定义 Task 对象模型。
...
Evidence 独立事实模型已取消。Task 直接通过 `closure_evidence` 字段记录关闭证据，通过引用结果物（如文件路径、构建产物、测试报告等）承接验证和关闭证据功能。不再使用 `related_evidence` 字段和独立的 Evidence 对象。
...
Task 基础字段遵循 `specs/07-LDVH事实模型基础规范.md` §7.3 的字段契约原则。
