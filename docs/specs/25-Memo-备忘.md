...
Pitfall 的准入、状态和字段契约由 `docs/specs/23-Pitfall-踩坑.md` 定义。管辖项目配置的字段和边界由 `docs/specs/03.06-管辖项目配置规范.md` 定义。环境能力核验、环境适配和适配措施正文不得写入管辖项目配置；需要长期保留的稳定事实应进入环境适配待补齐事项、Task、Memo、ADR、正式规范或按 04 系列规范处理。当前具体检查结果只保留当前过程结论，不作为持久状态事实源。
