...
| 规则与模板边界 | 修改步骤、Git Gate、验证或交还时 | 本文只组织来源读取、行动关系和失败分流，没有定义 message、branch、拆分、测试、Hook 或实现政策 | 本文、02、03、06、07、项目规则和差异审核 | 来源逐项对照和正反场景走查 | 当次修改及直接相邻责任 | 移除复制内容，回到正确来源定义；保持 `draft` 或暂停受影响模板 |
