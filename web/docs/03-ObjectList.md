...
列表顶层同时返回 Web machine 当次实际观察范围：`coverage_status`、`observed_at`、`object_read_problems` 和 `coverage_problems`。问题项只提供稳定 `object_ref`（对象问题）或 `workcase_collection` 范围身份（集合问题）、读取状态与问题摘要；列表响应及其顶层 `issues` 不输出或承诺 `canonical_path`、`absolute_path`、载体与内容指纹。`invalid / not_found` 表示对象读取已经完成，可以与 `coverage_status: complete` 同时出现；`unavailable`、结构问题或范围未完成才使 coverage 为 `partial / unavailable`。页面必须保留已形成的可消费 Card，独立展示候选读取问题与未完成范围；无论完整读取是否发现问题，都显示 machine 观察时间和“重新读取”入口，重新读取必须发起新请求，不能复用旧 payload。机器边界尚未形成而返回 503 时，页面必须保留安全的实际失败原因，明确观察时间不可用并提供同一重新读取入口，不得回退本地读取或显示伪零值。数量在 `partial` 时用 `N+`，在 `unavailable` 时不显示伪零值。
