...
Pitfall 的价值，是把已解决且验证的反复问题、误判、规避策略或可复用经验记录为事实对象，减少 AI 在后续执行中重复踩坑和重新推理的负担。

Pitfall 不是 bug backlog、未验证问题列表、TODO、长期决策或规范正文。未解决、未验证或只是一时异常的问题，不应写成 active Pitfall。
...
4. V2 `23-Pitfall-踩坑经验.md` 的后续字段迁移判断。
