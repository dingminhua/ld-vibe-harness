...
> 编号归属：`docs/studies/` 项目评估与内部辅助文档，编号仅用于排序和引用便利，不属于 specs 正式规范编号体系
...
```markdown
请对 `/Users/dmh2002/trae_projects/ld-vibe-harness/specs` 根目录下的 Markdown 文件执行一次“自身规范符合性”检查，不检查 `docs/studies/` 和 `docs/sources/` 子目录，除非它们被根目录文件引用且必须作为背景读取。
...
4. 不要把 `docs/sources/` 的外部资料当作强制规则来源。
...
1. 本文模板如何与 LDVH Skill 机制联动待 Trae Solo 环境机制规范稳定后确定；
2. 本文模板如何纳入规范落地自动化待环境适配规范稳定后确定。
