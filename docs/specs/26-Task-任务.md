...
不形成 Task 的内容，应按性质留在当前执行上下文，或进入 Memo、docs、refs、research、现有 Task 字段或其他权威位置。
...
Human Gate 的具体环境实体由 04 系列环境投射和运行投影记录承接。本文只规定 Task 语境下需要确认的事实、影响范围和证据承接要求。
...
下表“消费方”表示读取、展示、校验或执行时会消费该字段，不表示 Web 写入授权。Task 当前所有字段在 Web 中均为只读；`status`、`source_intent`、`acceptance`、`verification`、`closure_evidence`、`deliverables` 等字段不得通过 Web 修改。
...
Web 不得在页面状态、缓存或数据库中维护独立 Task 权威状态。Task 当前不在 `docs/specs/08-Web信息同步规范.md` §8.2 Web 事实源写入白名单内，因此 Web 不得修改 Task 字段、状态、引用关系或文件内容；Task 变更必须通过 AI/Code/工作流程和 Human Gate 约束完成。

### 8.4 工作流程与环境投射

Task 执行、验证、关闭和审计的具体行动流程由后续 40-59 工作流程规范承接。本文只定义 Task 实例的事实规则和状态约束。

独立、专项或并行验证的环境投射方式由 04 系列环境投射和运行投影记录处理。环境不支持子 Agent 时，应记录降级方式，例如改用人工审查、Code 校验或单 Agent 分阶段自检；不得把未完成的独立验证表述为完整通过。
...
2. Task Web 展示可继续完善；Task Web 写入暂不开放，未来如需开放必须先更新 08 白名单、状态机、字段校验、测试和 Human Gate 影响评估；
