...
> 创建日期：2026-05-30
> 定位：定义 ADR 决策记录生产对象模型，包括对象定位、准入条件、事实源边界、状态机、对象关系、Human Gate、字段契约、适配原则、实践子文档规划和实例检查
...
本文只定义 ADR 对象模型。ADR 相关 Rules、Skill、Agent、Tools 辅助和 Web 展示实践由 §十二规划的实践子文档承载；实例初始化检查和实例审计检查由本文承载。

---

## 二、与 06 的关系

`specs/06-LDVH生产对象基础规范.md` 定义生产对象通用规则、文件命名、实践子文档命名和生产对象模型标准组成。本文依据 06 §4.2 定义 ADR 对象模型。
...
| ADR 实践子文档 | `specs/11.01-Rules.md` 至 `specs/11.05-Web.md` |
...
实践子文档含义和命名规则见 06 §4.5。ADR 实践子文档规划如下：
...
| 9 | 事实源边界 | ADR 模型、实例、实践子文档和工具展示未混淆事实源位置 |
...
4. ADR 相关 Rules、Skill、Agent、Tools 和 Web 实践待对应实践子文档创建时展开。
