...
v2 active 后，当前 active `specs/` 是正式规范事实源。v1 `specs/`、切换前事实源和相关 Git commit records 进入历史记录；历史记录可以追溯和提取价值，但不替代当前 active 事实源。
...
6. v2 active 切换前的历史阶段，应用 MIGRATION-MAP 或等价覆盖诊断逐条确认 v1 `09`、v1 `10` 的章节去向。
