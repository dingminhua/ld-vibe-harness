...
| lifecycle 验收闭环要求 | 安装检测通过但尚未 integrated 时，必须提供重启 App、新会话只读可见性探针、插件页面授权、真实触发或受控写入正反例的 Human 验收路径，并复跑 `install_verification.py` 形成本次验收总结；不得写入或消费长期过程状态 | 本文、01、Code audit、tests | 环境治理 | 环境插件安装检测通过但后续逻辑需要 integrated 时 |
