...
LDVH_INSTALL_ACTION_TEMPLATE_REQUIRED_ROWS = {
    "Context": ["用户目标", "目标环境", "LDVH 本体路径", "目标工作区根目录", "管辖项目候选", "LDVH-GOVERNED-PROJECTS.yaml", "ldvh-base/", "workcases/adrs/pitfalls/sparks/studies", "环境入口审计结果", "source_refs", "specs/10-管辖项目配置规范.md"],
    "Scenario": ["安装 LDVH", "接入 LDVH", "初始化 LDVH", "配置管辖项目", "旧插件 / 旧路径", "只回答 01/06/10 边界"],
    "Gate": ["Human Gate", "环境入口", "LDVH 插件 / 扩展包", "LDVH-GOVERNED-PROJECTS.yaml", "ldvh-base/", "事实源子目录", "LDVH 本体路径", "目标工作区根目录", "配置层级冲突", "integrated", "多项目", "用户告知清单", "安装方案预览", "最终确认"],
    "执行": ["bootstrap discovery", "有限、只读、有证据", "LDVH_ROOT", "候选路径和证据", "01", "支持 Hook", "LDVH 插件 / 扩展包 / package", "不直接写入环境 Hook 系统文件", "用户告知清单", "安装方案预览", "workcases/adrs/pitfalls/sparks/studies", "repo instruction", "manual entrypoint", "不恢复 Rules 顶层机制", "目标工作区根目录", "配置文件完整路径", "项目根目录、用户级目录和 LDVH 本体目录不得作为主选项", "配置层级检查", "目标项目内已存在配置", "10", "Git common-dir", "target-first resolver"],
...
LDVH_INSTALL_WIZARD_TERMS = [
    ...
    "AI 必须用表格展示 LDVH 本体路径、目标工作区根目录、配置文件完整路径、含义、限制和建议",
    "配置层级冲突",
    "先删除、迁移或明确保留其中一个配置文件",
    "执行、不执行、返回修改",
    "最终确认前",
