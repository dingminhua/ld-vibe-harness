...
def ldvh_bootstrap_baseline_build(workspace_root, checks, governed_issues, runtime_report, spec_report, remaining_gaps):
    ...
    required_assets = [
        ...
        (PROJECT_ROOT / "rules" / "LDVH-RUNTIME-PROTOCOL.md", "LDVH Runtime Protocol 统一入口", "环境承接"),
    ]
    asset_issues = [
    ...
    allowed_categories = {"规范", "Code", "Web", "WorkCase", "事实源", "环境承接", "Human Gate"}
    routing_issues = []
    for gap in remaining_gaps:
        if not gap.get("suggested_writeback"):
            routing_issues.append(ldvh_bootstrap_issue("BOOTSTRAP_GAP_ROUTING_MISSING", f"缺口缺少分流建议: {gap.get('id')}", category="WorkCase"))
    ...
    for category in unknown_categories:
        routing_issues.append(ldvh_bootstrap_issue("BOOTSTRAP_GAP_CATEGORY_UNKNOWN", f"未知缺口分类: {category}", category="WorkCase"))
    items.append(ldvh_bootstrap_baseline_item(
...
def ldvh_assurance_check_build(workspace_root=None):
    ...
    checks = [
        ...
        },
        {
            ...
            "suggested_writeback": "fact_yaml_fix_or_workcase",
        ...
        },
        {
            ...
            "suggested_writeback": "spec_fix_or_workcase",
