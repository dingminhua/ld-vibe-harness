...
| 入口可见要求 | AI 处理需要长期保留的调研报告时，应能定位 Study active 规范、本文和对应实例事实源 | `specs/README.md`、02 成员身份、知识地图输入、运行时入口；行动编排未接管前为人工降级检查 | AI 执行入口 | 报告创建、引用、吸收或归档时 |
...
1. 本文已迁入 Study 主要准入、状态机、frontmatter 字段、正文骨架、URL 结构和 Web 阅读边界，并已完成 Human 单篇确认；active 切换前的历史默认入口为 v1 历史 Study 规范为默认入口；
