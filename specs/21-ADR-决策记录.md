...
9. ADR 创建、升级、推翻和废弃流程中的编号生成、ADR YAML 写入、状态流转写入、关联字段回写、推翻或废弃字段回写和 Change 记录生成属于应评估的可机械化受控写入步骤。
10. 若 21.04-Tools.md 或 Tools 实现未覆盖上述受控写入步骤，应将缺口记录为 ADR 完整落地初始化的阻塞项或暂缓项；不得因只读、查询、统计或校验能力已存在而声称 ADR Tools 完整支撑对象模型落地。
...
11. 确认 ADR Tools 是否已覆盖编号生成、ADR 创建、状态流转写入、关联字段回写、推翻或废弃字段回写和 Change 记录生成等可机械化受控写入步骤；未覆盖时应记录为完整落地初始化阻塞项或暂缓项；
12. 确认 ADR Tools 已创建或更新对应自动化测试，并已执行测试验收、记录测试命令和结果；
13. 记录暂缓项和初始化产物。
...
14. ADR Tools 是否覆盖编号生成、ADR 创建、状态流转写入、关联字段回写、推翻或废弃字段回写和 Change 记录生成等可机械化受控写入步骤；未覆盖时是否已标记为阻塞项或暂缓项；
15. active-prep 和 not-created 子文档是否说明当前状态、原因、前提和检查项；
16. 识别初始化缺口、过期实践和需要整改的附件型实践子文档。
...
10. ADR Tools 契约式校验与执行适配、受控写入能力缺口识别及其测试验收是否符合本文 §十二和 `specs/12.01-Tools辅助规范.md`；
...
5. ADR 对象模型在本项目中的实际落地状态需重新执行落地前决策后确认；
...
9. ADR Tools 受控写入能力尚未实现，需补齐编号生成、ADR 创建、状态流转写入、关联字段回写、推翻或废弃字段回写、Change 记录生成和写入后验证能力后，才能声称完整支撑 ADR Skill 流程和对象模型完整落地初始化。
