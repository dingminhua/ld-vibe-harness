...
> **退役标注**：本规划承接的 Codex adapter / 环境薄引用形态已被 00 §8.2（commit f88694775）废除——LDVH 的环境接入形态只有薄 Skill 与 Git Gate，接入面不存在环境 Hook、插件或 adapter 层（09 §5.1）。本文作为 workcase-0007 的历史规划记录保留，不再作为实现依据；文中 "adapter"、"薄引用"、"8.2 环境 Hook 的薄引用与核心职责边界"（00 §8.2 旧标题，现标题为"8.2 薄 Skill 环境接入与核心职责边界"）等表述均为旧学说原文。`ldvh-work-context` 核心仍是 09 §5.4 规则引导的首选入口；新接入实现见 09 §5 与 33。本文引用的 `code/plugins/ldvh/` Codex 参考实现已于 2026-07-31 整体移除。
