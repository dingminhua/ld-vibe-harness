...
1. `00-03` 是全局上位层：理念与构成、保障与衔接、AI 行为、事实源与 Git 追溯；
...
事实源与 Git 追溯放在 `03`，是因为用户事实源和 Git 追溯是所有构成要素共同遵守的底层原则，且 commit 契约需要早于行动模板、Code validator、Web 展示和测试治理。
