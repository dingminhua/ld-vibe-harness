...
| 入口可见要求 | AI 处理未计划化但有保留价值的信息、发现、提醒、问题或缺口时，应能定位 Spark active 规范、本文和对应实例事实源 | 02 成员身份、Rules 入口、`v2-check` 只读诊断和知识地图输入；行动编排未接管前为人工降级检查 | AI 执行入口 | Spark 入口、成员身份、实例目录、Rules 入口、`v2-check` 或知识地图输入变化时 |
