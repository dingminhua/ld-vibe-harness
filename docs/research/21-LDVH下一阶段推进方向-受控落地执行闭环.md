...
> 来源：18、19、20 号历史调研文档、2026-06-11 关于安装、自动执行、CLI、长期状态源与平台映射的讨论；跨平台相关事项已收敛到 `docs/research/22-LDVH跨平台能力复用与薄投影落地计划.md`
...
不要把跨平台相关事项展开在受控执行闭环文档中；相关计划由 22 承接。
```
...
这条链路是 LDVH 当前最需要补齐的核心运行能力。它不是 ECC 式安装器，不引入安装状态库，不复制 ECC 的 profile、executor、repair store 或跨平台分发系统；但它承认一个现实问题：如果没有“安装/落地初始化/接入/补齐”这一步，LDVH 后续 Code、Web、CLI、Human Gate 和自动化都无法稳定推进。跨平台相关事项由 `docs/research/22-LDVH跨平台能力复用与薄投影落地计划.md` 单独承接。
...
### 3.6 跨平台相关事项由 22 单独承接

跨平台相关事项不在本文展开，统一见 `docs/research/22-LDVH跨平台能力复用与薄投影落地计划.md`。
...
| 跨平台事项需要独立收敛 | 如果继续放在本文，受控执行闭环主线会被稀释 | 不在本文处理，统一由 22 承接 |
...
跨平台相关事项不再作为本文的展开内容，统一由 `docs/research/22-LDVH跨平台能力复用与薄投影落地计划.md` 承接。
