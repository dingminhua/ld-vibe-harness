...
def deployment_entries_check_asset_metadata(root, asset_path_raw):
    ...
    if metadata is None:
        issues.append(Issue(asset_path, 1, f"固定运行时扩展缺少 ldvh_asset 自登记元信息: {asset_path_raw}", code="DEPLOYMENT_ENTRIES_ASSET_METADATA_MISSING"))
        return issues

    for field in DEPLOYMENT_ENTRIES_REQUIRED_METADATA_FIELDS:
        value = metadata.get(field)
        if value in (None, "", []):
            issues.append(Issue(asset_path, 1, f"固定运行时扩展登记缺少字段 {field}: {asset_path_raw}", code="DEPLOYMENT_ENTRIES_ASSET_METADATA_FIELD_MISSING"))
...
def deployment_entries_check(root=None):
    ...
    if not spec_path.exists():
        issues.append(Issue(spec_path, 1, f"缺少固定运行时扩展登记表: {DEPLOYMENT_ENTRIES_SPEC_PATH}", code="DEPLOYMENT_ENTRIES_SPEC_MISSING"))
        spec_text = ""
    else:
    ...
    for entry_type, expected_paths in DEPLOYMENT_ENTRIES_REQUIRED_ASSETS.items():
        ...
        if spec_text and entry_type not in spec_text:
            issues.append(Issue(spec_path, 1, f"固定运行时扩展登记缺少必备承载物类型: {entry_type}", code="DEPLOYMENT_ENTRIES_REQUIRED_TYPE_MISSING"))
        for expected_path in expected_paths:
            if spec_text and expected_path not in spec_text:
                issues.append(Issue(spec_path, 1, f"固定运行时扩展登记缺少必备承载物路径: {expected_path}", code="DEPLOYMENT_ENTRIES_REQUIRED_ASSET_MISMATCH"))
            if not (root / expected_path).exists():
                issues.append(Issue(root / expected_path, 1, f"缺少必备固定运行时扩展承载物: {expected_path}", code="DEPLOYMENT_ENTRIES_REQUIRED_ASSET_MISSING"))
    ...
    for forbidden_type in DEPLOYMENT_ENTRIES_FORBIDDEN_TYPES:
        ...
        if fixed_asset_section and forbidden_pattern in fixed_asset_section:
            issues.append(Issue(spec_path, 1, f"不得将支撑能力写成 Rules、Skill、Agent、Hook 同级固定运行时扩展类型: {forbidden_type}", code="DEPLOYMENT_ENTRIES_FORBIDDEN_TYPE"))
...
def deployment_entries_main(root=None):
    ...
    if issues:
        print(f"固定运行时扩展登记检查失败，共 {len(issues)} 个问题：")
        for issue in issues:
