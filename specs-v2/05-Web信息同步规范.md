...
v2_spec:
  ...
  related_specs:
    ...
    - "specs-v2/attachments/05.Att.01-DTO与API契约表.md"
    - "specs-v2/attachments/05.Att.02-页面与API映射矩阵.md"
    - "specs-v2/attachments/05.Att.03-轻写入白名单表.md"
    - "specs-v2/attachments/05.Att.04-Confirm-UI字段表.md"
    - "specs-v2/attachments/05.Att.05-Gate与Validate阶段边界矩阵.md"
    - "specs-v2/attachments/05.Att.06-Human-facing态势语义表.md"
    - "specs-v2/attachments/05.Att.07-提交记录展示矩阵.md"
    - "specs-v2/attachments/05.Att.08-缓存与同步状态矩阵.md"
    - "specs-v2/attachments/05.Att.09-Web回归矩阵.md"
    - "specs-v2/attachments/05.Att.10-Web差距审计模板.md"
    - "specs-v2/attachments/05.Att.11-Web能力删除核对表.md"
  migration_sources:
...
>
> Web v2 当前暂不实施。本文和 `05.Att.01` 至 `05.Att.11` 已承接 v1 `08` 主体规则和 Web 相关迁移线索；未经 Human 单篇确认前，不得作为 active 规范、Code 默认消费依据、Rules 入口依据或迁移完成结论。
...
|---|---|---|---|---|
| 上位约束承接要求 | Web 应承接 00、01、07、active v1 `11` 和 Human 确认后的 v2 08 测试治理，不反向定义事实模型、Code 或测试规则 | 本文、00、01、07、active v1 `11`、v2 08、Human Gate | Web 治理 | Web 展示、DTO、写入边界或事实源回指变化时 |
