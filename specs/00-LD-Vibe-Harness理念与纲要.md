...
> 原则：本文只定义产品理念、价值实现标准、架构构成和基础规范索引，不展开 specs 文档治理细则或具体对象实践细则
...
LD Vibe Harness 的事实源必须回到 Git 可追踪文件。事实源权威性判断、单一事实源原则、承载介质选择和工具输出回写规则，由 `specs/03-事实源边界与承载规范.md` 定义；本文只保留事实源闭环的顶层价值要求。
...
5. LDVH 工具基础规则由 `specs/05-LDVH工具基础规范.md` 定义；
6. LDVH 生产对象基础规则由 `specs/06-LDVH生产对象基础规范.md` 定义，并由 10-39 展开具体对象规范；
7. LDVH 行动模型基础规则由 `specs/07-LDVH行动模型基础规范.md` 定义，并由 40-69 展开具体行动规范。
...
1. 10-39 Harness 生产对象的具体对象规范待逐步创建；
2. 40-69 Harness 行动模型的具体行动规范待逐步创建；
