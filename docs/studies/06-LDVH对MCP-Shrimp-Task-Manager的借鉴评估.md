...
> 编号归属：`docs/studies/` 项目评估文档，编号仅用于排序和引用便利，不属于 specs 正式规范编号体系
> 调研边界：基于 `/Users/dmh2002/trae_projects/mcp-shrimp-task-manager` 本地仓库的 README、docs、src、code/task-viewer 与配置文件进行分析，重点评估其对 LD Vibe Harness 的可借鉴方向
...
6. 可选启动内嵌 WebGUI，或使用独立 `code/task-viewer` 查看和编辑任务。
...
| `src/code/task/` | 任务规划、分析、反思、拆分、执行、验证、查询、更新、删除等工具 |
| `src/code/research/` | 研究模式工具 |
| `src/code/thought/` | 思维处理工具 |
| `src/code/project/` | 项目规则初始化工具 |
...
| `code/task-viewer/` | 独立 React + Node 任务查看器和管理界面 |
...
第二套是独立 `code/task-viewer`，功能更完整。它包含 Node server、React 前端、TanStack Table、i18n、模板管理、Agent 管理、历史查看、任务详情、任务编辑、Chat Agent 等能力。其配置使用用户 home 下的设置文件和模板目录，并按 project profile 读取不同 `tasks.json`。
...
| research_mode | docs/studies、ADR、Memo、行动模型 | 借鉴研究状态整合和后续步骤约束 | 研究过程不等于结论，稳定结论需进入 research、ADR 或正式规范 |
...
Shrimp 的 `research_mode` 要求输入 topic、previousState、currentState 和 nextSteps，用于整合阶段性研究状态。这对 LDVH 的 `docs/studies/` 有直接借鉴价值。
...
建议将 `docs/studies/` 作为研究模式主要落点，并形成固定评估骨架：
...
| 研究模式行动模型或 research 模板 | `specs/50-79` 或 `docs/studies/` 模板 | 将外部调研转为可复用工程活动 | 若进入正式行动规范需确认 |
...
1. 本文结论如何影响 LDVH MCP 工具接入策略待工具规范稳定后确定；
2. 本文结论如何影响 Task 事实模型与 MCP 数据同步待事实模型规范稳定后确定。
