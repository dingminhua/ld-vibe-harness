...
| `suspended` | 暂停接入，暂不作为默认执行项目，但保留路径和记录 |
...
Profile 可以通过 `related_tasks` 关联项目接入、路径调整、初始化、审计或搬移 Task。Task 的准入、状态和字段契约由 `docs/specs/27-Task-任务.md` 定义。
...
|---|---|---|---|---|
| 上位约束承接要求 | Profile 实例和后续工作流程应遵守本文定义的准入、状态机、字段契约、环境记录边界和事实源边界 | 05、03.04、本文、20 集合索引、04.03、Human Gate | 工作模型治理 | 创建、修改、搬移、审计、暂停或归档 Profile 时 |
...
2. Profile 创建、激活、暂停、归档和项目切换的具体工作流程待 40-59 承接；
3. `project_kind` 是否需要扩展为更细的项目类型，待多项目实践后评估；
4. 多项目集中名册与各管辖项目自有 Profile 的关系，待产品初始化和产品审计流程稳定后进一步校准。
