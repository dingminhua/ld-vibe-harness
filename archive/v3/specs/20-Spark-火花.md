...
Spark 主要服务 V1、V2、V3、V7 和 V9：它帮助 AI 在信息尚不成熟时保留可定位入口、形成最低限度上下文、判断后续归口候选，按事实对象格式承载来源与演变，并推动议题分流为 WorkCase、ADR、Pitfall、Study、规范或其它承接位置。它不是完成对象，也不是任务计划、长期决策、经验库或研究报告。
...
4. 当前 V3 Spark 字段契约、实例校验和展示需求的后续补充判断；该判断不得以本地 V2 成员文件作为默认来源。
...
1. 后续如需补充 Spark 完整字段表、字段条件必填和实例样例，必须作为 V3-owned 字段契约、实例校验或明确 WorkCase 推进；
