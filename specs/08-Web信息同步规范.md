...
Web 的直接服务对象是 Human，但它仍服务 00 的 AI 第一目标：Web 通过让 Human 更好地确认、纠偏和验收，反过来给 AI 提供更清晰、可消费、可追溯的确认结果。

## 2. 权威依据

本文承接 `specs/00-理念与构成.md` 的 Web 构成要素定位、`specs/01-保障与衔接.md` 的证据和 Human Gate 呈现边界、`specs/02-AI行为规范.md` 的面向 Human 输出责任、`specs/03-事实源与Git追溯规范.md` 的非事实源边界，以及 `specs/04-Specs基础规范.md` 的结构规则。
...
Web 必须与 AI 和 Code 读取同一用户事实源。Web 可以使用 Code 输出、Git 查询、API 聚合或直接读取实现展示，但不得维护第二套字段契约、状态机、规则判断或事实源。
...
| 受控写入要求 | Web 写入必须有规范授权、验证和 Git 追溯 | 05、07、09、03 | 写入治理 | 新增或扩大 Web 写入能力时 |
