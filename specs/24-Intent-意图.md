...
本文定义 Intent 意图事实模型。Intent 是人的原始目标、范围、成功标准和约束，用于沉淀需要跨任务追踪的用户意图，为 Task 拆解和 ADR 决策提供上游来源。
...
`specs/13-LDVH事实模型基础规范.md` 定义事实模型通用规则、文件命名、附件型实践子文档命名和事实模型标准组成。本文依据 13 §4.2 定义 Intent 对象模型。
...
本文是 Intent 意图事实模型的权威事实源。本文定义 Intent 的准入条件、状态机、对象关系、Human Gate 和字段契约。
...
Intent 基础字段遵循 `specs/13-LDVH事实模型基础规范.md` §7.3 的字段契约原则。
