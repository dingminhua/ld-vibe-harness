...
构建时只从根级 `specs/` 当前单一来源生成包内资源，不在 Git 中维护第二份 Markdown 副本。目标资源逻辑位置为 `ldvh/_rule_snapshot/specs/**`，并带机器清单：
...
| `algorithm` | 固定 `sha256` |
| `files` | 按 canonical path 升序；成员字段闭集为 `path`、`size`、`sha256` |
| `snapshot_sha256` | 对不含本字段的规范化 manifest payload 计算的集合摘要 |

manifest 顶层字段闭集固定为上表六项。规范化 payload 使用除 `snapshot_sha256` 外的五项，`files` 先按 `path` 升序；随后用 Python JSON 等价规则执行 `sort_keys=true`、`ensure_ascii=false`、`separators=(",", ":")`，不附加换行，并对 UTF-8 字节计算 SHA-256。最终 `manifest.json` 使用相同 canonical JSON 序列化并只在文件末尾增加一个 LF。`path` 必须唯一、使用 POSIX 分隔符、匹配 §6.1 的两个候选位置，不得为绝对路径、包含空段、`.`、`..` 或反斜杠；`size` 为非负 integer；两个摘要均为 64 位小写十六进制。未知字段、重复 path、非规范排序、算法大小写变化或字段类型错误都使快照不可用。

sdist 必须包含根级 Specs、附件、构建逻辑和必要说明；由 sdist 再构建的 wheel 必须得到相同规则文件集合与集合摘要。运行时必须先核对 distribution、版本、候选文件闭集、长度、逐文件摘要和集合摘要，再解析规范；额外候选只检查 §6.1 两个目录和文件形状，不把 manifest 或包内非规则资源误判为候选。任何缺失、额外、篡改、清单错误或版本不一致都返回 `unavailable`，不得回退其它来源。
...
| `ldvh.specs.repository` | 对两种可信候选视图复用同一身份、结构、关系、字段和公开操作机械检查 |
...
2. **快照构建与定位**：新增构建生成、清单校验、包发现收紧、来源选择和单元/失败测试；
...
2. 普通 wheel 中规则文件和附件集合完整，且不出现顶层 `plugins`/`scripts` 伪包；
3. 快照文件缺失、额外、内容篡改、清单篡改、distribution/version 不一致和集合摘要不一致均 fail closed；
