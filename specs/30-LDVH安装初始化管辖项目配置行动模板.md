...
环境 Hook 或插件提示必须按当前目标环境命名。只有环境审计或 Human 明确目标环境为 Codex 时，才可在用户主界面写 `Codex/工具入口插件`；换成 Trae、IDE、Agent runner、CI、repo instruction 或未知环境时，必须改写为对应的目标环境名称，或使用环境无关的 `目标环境插件 / 工具入口插件`。AI 提示用户安装、升级、授权或验收环境插件时，必须说明目标环境、插件 / 扩展页面或入口位置、用户需要执行的授权 / trust 动作、新开窗口或新会话后应看到的预期现象、失败后应返回给 AI 的信息，以及 AI 后续诊断命令。未真实写入插件包、未进入插件页面或未获得授权证据前，不得写成“插件已安装”；只能写成 `待用户安装`、`需授权`、`可见 / 需验证` 或 `已写入但待用户授权` 等状态。
